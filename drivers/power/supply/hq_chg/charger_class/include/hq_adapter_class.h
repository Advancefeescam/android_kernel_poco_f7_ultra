...
struct adapter_dev;
struct adapter_ops {
	int (*handshake)(struct adapter_dev *adapter);
	int (*get_cap)(struct adapter_dev *adapter, struct adapter_cap *cap);
	int (*set_cap)(struct adapter_dev *adapter, uint8_t nr, uint32_t mv, uint32_t ma);
	int (*set_wdt)(struct adapter_dev *adapter, uint32_t ms);
	int (*reset)(struct adapter_dev *adapter);
	int (*get_temp)(struct adapter_dev *adapter, uint8_t *temp);
	int (*get_softreset)(struct adapter_dev *adapter);
	int (*set_softreset)(struct adapter_dev *adapter, bool val);
};
...
int adapter_set_cap(struct adapter_dev *adapter, uint8_t nr, uint32_t mv, uint32_t ma);
...
struct adapter_dev *adapter_register(char *name, struct device *parent, struct adapter_ops *ops, void *private);
