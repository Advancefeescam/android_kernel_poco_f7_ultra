...
static int fts_read_file(char *file_name, u8 **file_buf)
{
	...
	*file_buf = (u8 *)vmalloc(file_len);
	...
	set_fs(KERNEL_DS);
	pos = 0;
/*L19A code for HQ-203966  by gaoxue at 2022/4/25 start*/
#ifndef BUILD_USER
	ret = kernel_read(filp, *file_buf, file_len, &pos);
#endif
/*L19A code for HQ-203966  by gaoxue at 2022/4/25 end*/
	if (ret < 0)
		FTS_ERROR("read file fail");
	FTS_INFO("file len:%d read len:%d pos:%d", (u32)file_len, ret, (u32)pos);
	filp_close(filp, NULL);
...
/*L19 code for HQ-159452 by gaoxue at 2022/1/5 start*/
...
/*L19 code for HQ-159452 by gaoxue at 2022/1/5 end*/
