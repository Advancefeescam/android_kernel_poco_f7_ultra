...
static u32 g_acpu_base_addr[ACPU_CRG_ID_MAX];
...
static int dpu0_notifier_callback(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	dpu_reset = devm_ioremap(perf_mon_dev->dev,
		g_acpu_base_addr[ACPU_M1_CRG_ID] + Media1_CRG_SOFT_RESETS, CRG_LONGTH);
	dpu_clk = devm_ioremap(perf_mon_dev->dev,
		g_acpu_base_addr[ACPU_M1_CRG_ID] + Media1_CRG_CLK_OFFSET, CRG_LONGTH);
...
{
	if (event == REGULATOR_EVENT_ENABLE)
		perf_crg(g_acpu_base_addr[ACPU_M1_CRG_ID] + Media1_CRG_SOFT_RESETS, g_acpu_base_addr[ACPU_M1_CRG_ID] +
...
{
	if (event == REGULATOR_EVENT_ENABLE)
		perf_crg(g_acpu_base_addr[ACPU_M2_CRG_ID] + ISP_CRG_SOFT_RESETS, g_acpu_base_addr[ACPU_M2_CRG_ID] + ISP_CRG_CLK_OFFSET,
...
{
	if (event == REGULATOR_EVENT_ENABLE)
		perf_crg(g_acpu_base_addr[ACPU_M2_CRG_ID] + VENC_CRG_SOFT_RESETS, g_acpu_base_addr[ACPU_M2_CRG_ID] + VENC_CRG_CLK_OFFSET,
...
{
	if (event == REGULATOR_EVENT_ENABLE)
		perf_crg(g_acpu_base_addr[ACPU_GPU_CRG_ID] + GPU_CRG_CLK_OFFSETS, g_acpu_base_addr[ACPU_GPU_CRG_ID] + GPU_CRG_SOFT_RESETS,
...
static int perf_regulator(struct device *dev)
{
	...
	ret = of_property_read_u32_index(np, "acpu_media1_crg", 0, &g_acpu_base_addr[ACPU_M1_CRG_ID]);
	ret += of_property_read_u32_index(np, "acpu_m2_crg", 0, &g_acpu_base_addr[ACPU_M2_CRG_ID]);
	ret += of_property_read_u32_index(np, "acpu_gpu_crg", 0, &g_acpu_base_addr[ACPU_GPU_CRG_ID]);
	if (ret) {
		pr_err("Failed to get acpu_base_addr from dts!\n");
		return ret;
	}
...
static int perf_monitor_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32_index(np, "acpu_lpis_actrl", 0, &acpu_lpis_actrl);
	if (ret) {
		dev_err(dev, "failed to get acpu_lpis_actrl from dts!\n");
		return ret;
	}
