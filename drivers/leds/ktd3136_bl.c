...
int get_backlight_id(void)
{
	...
	return videolfb_tag->bkl_id;
}
EXPORT_SYMBOL(get_backlight_id);
...
EXPORT_SYMBOL(ktd3137_brightness_set);
			...
			const struct i2c_device_id *id)
{
	...
	struct ktd3137_chip *chip = NULL;
	//struct device_node *np = client->dev.of_node;

	if(24 != get_backlight_id())
		return -ENODEV;
