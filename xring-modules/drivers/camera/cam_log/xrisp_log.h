...
#define XRISP_PR_FORCE(fmt, ...) \
	(cam_log_get_level() >= XRISP_LOG_FORCE ? pr_info(fmt, ##__VA_ARGS__) : 0)
...
#define XRISP_DEV_FORCE(fmt, ...) \
	(cam_log_get_level() >= XRISP_LOG_FORCE ? dev_info(fmt, ##__VA_ARGS__) : 0)
