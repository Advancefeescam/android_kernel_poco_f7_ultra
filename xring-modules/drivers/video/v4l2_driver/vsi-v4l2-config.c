...
int vsi_set_Level(struct vsi_v4l2_ctx *ctx, int type, int level)
{
	...
	/* repair fuzz issue */
	if (level >= BITWIDTH_32 || level < 0) {
		v4l2_klog(LOGLVL_ERROR, "level(%d) out of bounds", level);
		return -EINVAL;
	}
...
int vsi_set_profile(struct vsi_v4l2_ctx *ctx, int type, int profile)
{
	...
	/* repair fuzz issue */
	if (profile >= BITWIDTH_32 || profile < 0) {
		v4l2_klog(LOGLVL_ERROR, "profile(%d) out of bounds", profile);
		return -EINVAL;
	}
...
{
	/* repair fuzz issue */
	if (codingmode >= BITWIDTH_32 || codingmode < 0) {
		v4l2_klog(LOGLVL_ERROR, "codingmode(%d) out of bounds", codingmode);
		return -EINVAL;
	}
