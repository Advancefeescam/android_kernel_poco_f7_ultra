...
static int xrisp_rproc_start(struct rproc *rproc)
{
	...
	if (ret) {
		XRISP_PR_FORCE("boot completion wait %llu ms",
...
static void xrisp_rproc_kick(struct rproc *rproc, int vqid)
{
	...
	u64 kick_start_time = xr_timestamp_gettime() / 1000; //us
	u64 mbox_use_time;

	memcpy(&msg[2], &kick_start_time, 8);
#endif
...
#ifdef IPC_TRACE_AP2FW_OPEN
	mbox_use_time = xr_timestamp_gettime() / 1000 - kick_start_time;
	if (mbox_use_time > IPC_MBOX_MAX_TIMEOUT_US)
		XRISP_PR_WARN("ipc trace: ipc trace: mbox ap2fw long time %lluus\n", mbox_use_time);
#endif
}
...
void xrisp_rproc_rpmsg_work(struct work_struct *work)
{
	...
	if (kick_use > 10)
		XRISP_PR_WARN("rpmsg work sched long time %lld ms", kick_use);
}
...
int xrisp_rpmsg_wait_complete(uint32_t times)
{
	...
	if (timeout)
		XRISP_PR_FORCE("rpmsg complete use time %d ms", times - jiffies_to_msecs(timeout));
	else {
...
int xrisp_rproc_boot(uint32_t scene)
{
	...
	XRISP_PR_FORCE("xrisp rproc scene = %d, state = %d, power_cnt=%d, attach_mode = %d", scene,
...
int xrisp_rproc_shutdown(void)
{
	...
	XRISP_PR_FORCE("xrisp rproc scene =%d, state = %d, power_cnt=%d, attach_mode = %d",
