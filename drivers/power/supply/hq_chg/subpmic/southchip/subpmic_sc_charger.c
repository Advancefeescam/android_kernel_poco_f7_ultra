...
static int subpmic_chg_set_qc_term_vbus(struct subpmic_chg_device *sc)
{
	if (sc->qc_result == QC2_MODE && (sc->qc_vbus != 5000)) {
		subpmic_chg_write_byte(sc, SUBPMIC_REG_DPDM_QC_CTRL, 0x03);
		sc->qc_vbus = 5000;
		dev_info(sc->dev, "QC charge done, drop qc_vbus = %d\n", sc->qc_vbus);
	}
...
static int subpmic_chg_get_input_volt_lmt(struct subpmic_chg_device *sc, uint32_t *mv)
{
	...
	ret = subpmic_chg_field_read(sc, F_VINDPM);
	if (ret < 0)
		return ret;
					...
					uint32_t *chg_status)
{
	...
	state = __subpmic_chg_get_chg_status(sc);
	if (state < 0) {
		dev_err(sc->dev, "failed to get charge status, ret = %d\n", state);
		return state;
	}
...
static int subpmic_chg_set_input_curr_lmt(struct subpmic_chg_device *sc, int ma)
{
	...
	dev_info(sc->dev, "subpmic_chg_set_input_curr_lmt = %d\n", ma);
...
__maybe_unused
static int subpmic_chg_get_rechg_vol(struct subpmic_chg_device *sc, int *mv)
{
	int ret = 0;

	ret = subpmic_chg_field_read(sc, F_VRECHG);
	if (ret < 0)
		return ret;
...
__maybe_unused
static int subpmic_chg_get_vindpm_state(struct subpmic_chg_device *sc, int *vindpm_state)
{
	u8 val[3] = {0};
	int ret = 0;
	int int_flg = 0;
	...
	dev_info(sc->dev, "Buck INT STAT : 0x%x\n", state);

	*vindpm_state = !!(state & SUBPMIC_BUCK_STATE_VINDPM);
	dev_info(sc->dev, "chip vindpm_state = 0x%x\n", *vindpm_state);
	...
	dev_info(sc->dev, "chip vindpm_flag = 0x%x\n", vindpm_flag);
...
static irqreturn_t subpmic_chg_dpdm_alert_handler(int irq, void *data)
{
	...
	if (!sc->use_soft_bc12) {
		...
		if (val & SUBPMIC_DPDM_BC12_DETECT_DONE) {
			...
			#endif
			ret = subpmic_chg_get_input_curr_lmt(sc, &new_icl);
			if (ret < 0) {
				dev_info(sc->dev, "get icl error\n");
			} else {
				if (new_icl != sc->icl_val) {
					dev_info(sc->dev, "icl update  after bc12 detect\n");
					subpmic_chg_set_input_curr_lmt(sc, sc->icl_val);
				}
			}
			charger_changed(sc->sc_charger);
		}
...
static irqreturn_t subpmic_chg_buck_alert_handler(int irq, void *data)
{
	...
	ret = subpmic_chg_bulk_read(sc, SUBPMIC_REG_CHG_FLT_STAT, val, 2);
	if (ret < 0) {
		return ret;
	}
	...
	if (flt_stat != 0) {
		dev_info(sc->dev, "Buck_flt state : 0x%x\n", flt_stat);
		if ((val[1] & 0x04) == 0x04) {
			hq_charger_notifier_call_chain(CHARGER_EVENT_CHG_TIMEOUT, NULL);
			dev_err(sc->dev, "charge timeout!\n");
		}
	}
	if (((val[0] & 0x10) == 0x10) &&
				((flt_stat & 0xFFEF) == 0)) {
			sc->state.conv_ocp = 1;
			//dev_info(sc->dev, "conv_ocp triggered without other fault");
	} else {
			sc->state.conv_ocp = 0;
			//dev_info(sc->dev, "conv_ocp NOT triggered");
	}
...
static int subpmic_pd_hw_init(struct subpmic_chg_device *sc)
{
	...
	uint8_t bmc_opera[4][2] = {{0xE8, 0x86},{0xE9, 0x3D},{0xEA, 0x3C},{0xEB, 0x16}};
...
static int subpmic_chg_hw_init(struct subpmic_chg_device *sc)
{
	...
	if (sc->tcpc) {
		...
		dev_err(sc->dev, "tcpci_set_continuous_time !!!\n");
	}
	...
	ret |= subpmic_chg_write_byte(sc, 0x39, 0x53);
	if (ret < 0)
		return ret;
			...
			enum chg_adc_channel channel, uint32_t *value)
{
	...
	case CHG_ADC_TDIE:
		*value = adc_val;
		break;
	default:
		dev_err(sc->dev, "charger adc channel %d not support\n");
		ret = -1;
		break;
	}
...
static int sc_chg_get_rechg_vol(struct charger_dev *charger, int *mv)
{
	struct subpmic_chg_device *sc = charger_get_private(charger);

	return subpmic_chg_get_rechg_vol(sc, mv);
}
...
static int sc_disable_dpdm_block(struct charger_dev *charger)
{
	struct subpmic_chg_device *sc = charger_get_private(charger);
	int ret = 0;
	...
	if (ret < 0) {
		dev_err(sc->dev, "dpdm block disable fail \n");
	}

	return ret;
}

static int sc_chg_get_vindpm_state(struct charger_dev *charger, int *vindpm_state)
{
	struct subpmic_chg_device *sc = charger_get_private(charger);

	return subpmic_chg_get_vindpm_state(sc, vindpm_state);
}

static int sc_chg_dump_registers(struct charger_dev *charger)
{
	struct subpmic_chg_device *sc = charger_get_private(charger);

	return subpmic_chg_dump_regs(sc, NULL);
}

static int sc_chg_en_cid(struct charger_dev *charger, bool en)
{
	struct subpmic_chg_device *sc = charger_get_private(charger);

	return subpmic_chg_field_write(sc, F_CID_EN, en);
}
...
static struct charger_ops charger_ops = {
	...
	.disable_dpdm_block = sc_disable_dpdm_block,
	...
	.get_rechg_vol = sc_chg_get_rechg_vol,
	...
	.dump_registers = sc_chg_dump_registers,
	.en_cid = sc_chg_en_cid,
};
				...
				struct device_node *np)
{
	...
	const struct {
		...
		{"sc,fast-chg-safety-timer", &(sc->buck_init.safety_timer)},
	};
...
static const struct of_device_id subpmic_chg_of_match[] = {
	{.compatible = "southchip,subpmic_sc_charger",},
	{},
};
...
MODULE_DESCRIPTION("southchip subpmic platform driver");
