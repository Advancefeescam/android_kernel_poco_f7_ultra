...
static void tee_shm_release(struct tee_device *teedev, struct tee_shm *shm)
{
	if (shm->flags & TEE_SHM_POOL) {
		...
		if (shm->flags & TEE_SHM_USER_KERNEL_MAPPED)
			vfree(shm->kaddr);
	}
				 ...
				 size_t length, u32 flags)
{
	...
	const u32 req_user_kernel_flags = TEE_SHM_DMA_BUF | TEE_SHM_KERNEL_MAPPED | TEE_SHM_USER_KERNEL_MAPPED;
	struct tee_shm *shm;
	unsigned long start;
	size_t num_pages;
	void *ret;
	...
	if (flags != req_user_flags && flags != req_kernel_flags && flags != req_user_kernel_flags) {
...
static int tee_shm_fop_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (shm->flags & TEE_SHM_USER_KERNEL_MAPPED) {
		for (int i = 0; i < shm->num_pages; i++) {
			ret = remap_pfn_range(vma, vma->vm_start + i * PAGE_SIZE, page_to_pfn(shm->pages[i]),
			       PAGE_SIZE, vma->vm_page_prot);
			if (ret) {
				pr_err("remap_pfn_range is error\n");
				return ret;
			}
		}
		return 0;
