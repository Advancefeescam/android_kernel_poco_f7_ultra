...
static void Nanosic_i2c_specified_packets_detect(char *data)
{
    ...
    else if (command == 0xA4 && source == FIELD_176X && object == FIELD_HOST)
    {
        ...
        if(hall_n != 0xff && i2c_client->sensor_dev && i2c_client->notifier_handler)
        {
            if(hall_n)
...
int Nanosic_i2c_parse(char *data, int datalen)
{
    ...
    {
        /*null packet*/
        dbgprint(ERROR_LEVEL, "third_byte is null, invalid data\n");
...
int Nanosic_i2c_read_handler(void *data)
{
    ...
    if (IS_ERR_OR_NULL(i2c_client))
    {
        dbgprint(ERROR_LEVEL, "Nanosic_i2c_read_handler, null\n");
        ret = -1;
        goto out;
    }
...
int Nanosic_i2c_read_version(struct i2c_client *client)
{
    ...
    while (retry++ < 30)
    {
        ...
        if (ret < 0)
        {
            dbgprint(INFO_LEVEL, "i2c write cmd failed time %d\n", retry);
            msleep(100);
            continue;
        }
        dbgprint(INFO_LEVEL, "i2c write OK %d\n", ret);
        msleep(2);
        ret = Nanosic_i2c_read_boot(client, rsp, sizeof(rsp));
        dbgprint(INFO_LEVEL, "i2c read %d\n", ret);
        if (ret > 0)
        {
...
int Nanosic_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
    ...
    {
        dbgprint(ERROR_LEVEL, "======return=====\n");
        return -ENODEV;
    }

    dbgprint(INFO_LEVEL, "%s: addr = %x\n", __func__, client->addr);
