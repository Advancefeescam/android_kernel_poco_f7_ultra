			 ...
			 void *arg)
{
	...
	for (i = 0; i < MAX_MULTIPLEX_CLIENT; i++) {
		...
		if (packet->bit_flag & index) {
			...
			if (res > 0) {
				if (packet->wakeup) {
					if (atomic_read(&this->wakeup[i]) == 0) {
						atomic_inc(&this->wakeup[i]);
						__pm_stay_awake(this->event_ws[i]);
					}
					atomic_inc(&this->wakeup[i]);
					pr_debug("hal client %u wakeup %d awake cnt %lu relax cnt %lu!\n",
						i,
						atomic_read(&this->wakeup[i]),
						this->event_ws[i]->active_count,
						this->event_ws[i]->relax_count);
				}
			      ...
			      const uint32_t rx_buf_size, const int service_id)
{
	...
	*this = tmp;
	...
	for (i = 0; i < MAX_MULTIPLEX_CLIENT; i++) {
		char wakesource_name[WAKESOURCE_NAME_LEN];

		snprintf(wakesource_name, sizeof(wakesource_name), "approuter_event_wakeup%d", i);
		tmp->event_ws[i] = wakeup_source_register(NULL, wakesource_name);
	}
	return 0;
...
int app_router_multiplex_deinit(struct app_router_multiplex *this)
{
	...
	for (i = 0; i < MAX_MULTIPLEX_CLIENT; i++) {
		...
		wakeup_source_unregister(this->event_ws[i]);
	}
	kfree(this);
	return 0;
}
				 ...
				 uint32_t index)
{
	...
	if (err)
		goto fail;
	atomic_set(&this->wakeup[index], 0);
	mutex_unlock(&this->lock);
	return err;
