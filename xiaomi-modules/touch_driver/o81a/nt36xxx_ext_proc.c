...
#define XM_HTC_MC_CALIBRATION_EN "xm_htc_mc_calibration_en"
...
#define XM_HTC_BASE_REFRESH_ENABLE "xm_htc_base_refresh_enable"
#define XM_HTC_THP_BASE_TRACE_FLAG "xm_htc_thp_base_trace_flag"
#define XM_HTC_IDLE_HIGH_BASE_EN "xm_htc_idle_high_base_en"
#define XM_HTC_IDLE_HIGH_BASE_KEEP_TIME "xm_htc_idle_high_base_keep_time"
...
static struct proc_dir_entry *XM_HTC_proc_mc_calibration_en_entry;
...
static struct proc_dir_entry *XM_HTC_proc_base_refresh_enable_entry;
static struct proc_dir_entry *XM_HTC_proc_thp_base_trace_flag_entry;
static struct proc_dir_entry *XM_HTC_proc_idle_high_base_en_entry;
static struct proc_dir_entry *XM_HTC_proc_idle_high_base_keep_time_entry;
...
static ssize_t xm_htc_report_rate_proc_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	static int finished = 0;
	int32_t cnt = 0;
	int32_t len = 0;
...
static ssize_t xm_htc_report_rate_proc_write(struct file *filp,const char __user *buf, size_t count, loff_t *f_pos)
{
	int32_t ret;
	int32_t tmp;
	int16_t report_rate;
...
int32_t nvt_xm_htc_set_mc_calibration_en(int16_t mc_calibration_en)
{
	int32_t ret = 0;

	NVT_LOG("++\n");

	NVT_LOG("set mc_calibration_en %d\n", mc_calibration_en);
	ret = nvt_set_extend_custom_cmd(0x0C, mc_calibration_en);
    if (ret < 0) {
        NVT_ERR("nvt_set_extend_custom_cmd fail! ret=%d\n", ret);
        goto out;
    }
...
int32_t nvt_xm_htc_get_mc_calibration_en(int16_t *mc_calibration_en)
{
	int32_t ret = 0;

	NVT_LOG("++\n");

	ret = nvt_get_extend_custom_cmd(0x0C, mc_calibration_en);
    if (ret < 0) {
        NVT_ERR("nvt_get_extend_custom_cmd fail! ret=%d\n", ret);
        goto out;
    }
	NVT_LOG("get mc_calibration_en: %d\n", *mc_calibration_en);
...
static ssize_t xm_htc_mc_calibration_en_proc_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	static int finished = 0;
	int32_t cnt = 0;
	int32_t len = 0;
	int16_t mc_calibration_en;
	char tmp_buf[64];
	...
	*/
	...
	nvt_xm_htc_get_mc_calibration_en(&mc_calibration_en);

	mutex_unlock(&ts->lock);

	cnt = snprintf(tmp_buf, sizeof(tmp_buf), "mc_calibration_en: %d\n", mc_calibration_en);
	if (copy_to_user(buf, tmp_buf, sizeof(tmp_buf))) {
		NVT_ERR("copy_to_user() error!\n");
		return -EFAULT;
	}
...
static ssize_t xm_htc_mc_calibration_en_proc_write(struct file *filp,const char __user *buf, size_t count, loff_t *f_pos)
{
	int32_t ret;
	int32_t tmp;
	int16_t mc_calibration_en;
	...
	mc_calibration_en = (int16_t)tmp;
	NVT_LOG("mc_calibration_en = %d\n", mc_calibration_en);
	...
	nvt_xm_htc_set_mc_calibration_en(mc_calibration_en);
...
#ifdef HAVE_PROC_OPS
static const struct proc_ops  xm_htc_mc_calibration_en_fops = {
    .proc_read = xm_htc_mc_calibration_en_proc_read,
    .proc_write = xm_htc_mc_calibration_en_proc_write,
};
#else
static const struct file_operations xm_htc_mc_calibration_en_fops = {
	.owner = THIS_MODULE,
	.read  = xm_htc_mc_calibration_en_proc_read,
	.write = xm_htc_mc_calibration_en_proc_write,
};
#endif
...
int32_t nvt_xm_htc_set_base_refresh_enable(int16_t base_refresh_enable)
{
	int32_t ret = 0;

	NVT_LOG("++\n");

	NVT_LOG("set base_refresh_enable: %d\n", base_refresh_enable);
	ret = nvt_set_extend_custom_cmd(0x1C, base_refresh_enable);
	if (ret < 0) {
		NVT_ERR("nvt_set_extend_custom_cmd fail! ret=%d\n", ret);
		goto out;
	}
...
int32_t nvt_xm_htc_get_base_refresh_enable(int16_t *base_refresh_enable)
{
	int32_t ret = 0;

	NVT_LOG("++\n");

	ret = nvt_get_extend_custom_cmd(0x1C, base_refresh_enable);
	if (ret < 0) {
		NVT_ERR("nvt_get_extend_custom_cmd fail! ret=%d\n", ret);
		goto out;
	}
	NVT_LOG("get base_refresh_enable: %d\n", *base_refresh_enable);
...
static ssize_t xm_htc_base_refresh_enable_proc_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	static int finished = 0;
	int32_t cnt = 0;
	int32_t len = 0;
	int16_t base_refresh_enable;
	char tmp_buf[64];
	...
	*/
	...
	nvt_xm_htc_get_base_refresh_enable(&base_refresh_enable);

	mutex_unlock(&ts->lock);

	cnt = snprintf(tmp_buf, sizeof(tmp_buf), "base_refresh_enable: %d\n", base_refresh_enable);
	if (copy_to_user(buf, tmp_buf, sizeof(tmp_buf))) {
		NVT_ERR("copy_to_user() error!\n");
		return -EFAULT;
	}
...
static ssize_t xm_htc_base_refresh_enable_proc_write(struct file *filp,const char __user *buf, size_t count, loff_t *f_pos)
{
	int32_t ret;
	int32_t tmp;
	int16_t base_refresh_enable;
	...
	base_refresh_enable = (int16_t)tmp;
	NVT_LOG("base_refresh_enable = %d\n", base_refresh_enable);
	...
	nvt_xm_htc_set_base_refresh_enable(base_refresh_enable);
...
#ifdef HAVE_PROC_OPS
static const struct proc_ops  xm_htc_base_refresh_enable_fops = {
	.proc_read = xm_htc_base_refresh_enable_proc_read,
	.proc_write = xm_htc_base_refresh_enable_proc_write,
};
#else
static const struct file_operations xm_htc_base_refresh_enable_fops = {
	.owner = THIS_MODULE,
	.read  = xm_htc_base_refresh_enable_proc_read,
	.write = xm_htc_base_refresh_enable_proc_write,
};
#endif

int32_t nvt_xm_htc_set_thp_base_trace_flag(int16_t thp_base_trace_flag)
{
	int32_t ret = 0;

	NVT_LOG("++\n");

	NVT_LOG("set thp_base_trace_flag: %d\n", thp_base_trace_flag);
	ret = nvt_set_extend_custom_cmd(0x1D, thp_base_trace_flag);
	if (ret < 0) {
		NVT_ERR("nvt_set_extend_custom_cmd fail! ret=%d\n", ret);
		goto out;
	}
...
int32_t nvt_xm_htc_get_thp_base_trace_flag(int16_t *thp_base_trace_flag)
{
	int32_t ret = 0;

	NVT_LOG("++\n");

	ret = nvt_get_extend_custom_cmd(0x1D, thp_base_trace_flag);
	if (ret < 0) {
		NVT_ERR("nvt_get_extend_custom_cmd fail! ret=%d\n", ret);
		goto out;
	}
	NVT_LOG("get thp_base_trace_flag: %d\n", *thp_base_trace_flag);
...
static ssize_t xm_htc_thp_base_trace_flag_proc_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	static int finished = 0;
	int32_t cnt = 0;
	int32_t len = 0;
	int16_t thp_base_trace_flag;
	char tmp_buf[64];
	...
	*/
	...
	nvt_xm_htc_get_thp_base_trace_flag(&thp_base_trace_flag);

	mutex_unlock(&ts->lock);

	cnt = snprintf(tmp_buf, sizeof(tmp_buf), "thp_base_trace_flag: %d\n", thp_base_trace_flag);
	if (copy_to_user(buf, tmp_buf, sizeof(tmp_buf))) {
		NVT_ERR("copy_to_user() error!\n");
		return -EFAULT;
	}
...
static ssize_t xm_htc_thp_base_trace_flag_proc_write(struct file *filp,const char __user *buf, size_t count, loff_t *f_pos)
{
	int32_t ret;
	int32_t tmp;
	int16_t thp_base_trace_flag;
	...
	thp_base_trace_flag = (int16_t)tmp;
	NVT_LOG("thp_base_trace_flag = %d\n", thp_base_trace_flag);
	...
	nvt_xm_htc_set_thp_base_trace_flag(thp_base_trace_flag);
...
#ifdef HAVE_PROC_OPS
static const struct proc_ops  xm_htc_thp_base_trace_flag_fops = {
	.proc_read = xm_htc_thp_base_trace_flag_proc_read,
	.proc_write = xm_htc_thp_base_trace_flag_proc_write,
};
#else
static const struct file_operations xm_htc_thp_base_trace_flag_fops = {
	.owner = THIS_MODULE,
	.read  = xm_htc_thp_base_trace_flag_proc_read,
	.write = xm_htc_thp_base_trace_flag_proc_write,
};
#endif
...
int32_t nvt_xm_htc_set_idle_high_base_keep_time(int16_t idle_high_base_keep_time)
{
	int32_t ret = 0;

	NVT_LOG("++\n");

	NVT_LOG("set idle_high_base_keep_time: %d\n", idle_high_base_keep_time);
	ret = nvt_set_extend_custom_cmd(0x26, idle_high_base_keep_time);
	if (ret < 0) {
		NVT_ERR("nvt_set_extend_custom_cmd fail! ret=%d\n", ret);
		goto out;
	}
...
int32_t nvt_xm_htc_get_idle_high_base_keep_time(int16_t *idle_high_base_keep_time)
{
	int32_t ret = 0;

	NVT_LOG("++\n");

	ret = nvt_get_extend_custom_cmd(0x26, idle_high_base_keep_time);
	if (ret < 0) {
		NVT_ERR("nvt_get_extend_custom_cmd fail! ret=%d\n", ret);
		goto out;
	}
	NVT_LOG("get idle_high_base_keep_time: %d\n", *idle_high_base_keep_time);
...
static ssize_t xm_htc_idle_high_base_keep_time_proc_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	static int finished = 0;
	int32_t cnt = 0;
	int32_t len = 0;
	int16_t idle_high_base_keep_time;
	char tmp_buf[64];
	...
	*/
	...
	nvt_xm_htc_get_idle_high_base_keep_time(&idle_high_base_keep_time);

	mutex_unlock(&ts->lock);

	cnt = snprintf(tmp_buf, sizeof(tmp_buf), "idle_high_base_keep_time: %d\n", idle_high_base_keep_time);
	if (copy_to_user(buf, tmp_buf, sizeof(tmp_buf))) {
		NVT_ERR("copy_to_user() error!\n");
		return -EFAULT;
	}
...
static ssize_t xm_htc_idle_high_base_keep_time_proc_write(struct file *filp,const char __user *buf, size_t count, loff_t *f_pos)
{
	int32_t ret;
	int32_t tmp;
	int16_t idle_high_base_keep_time;
	...
	idle_high_base_keep_time = (int16_t)tmp;
	NVT_LOG("idle_high_base_keep_time = %d\n", idle_high_base_keep_time);
	...
	nvt_xm_htc_set_idle_high_base_keep_time(idle_high_base_keep_time);
...
#ifdef HAVE_PROC_OPS
static const struct proc_ops  xm_htc_idle_high_base_keep_time_fops = {
	.proc_read = xm_htc_idle_high_base_keep_time_proc_read,
	.proc_write = xm_htc_idle_high_base_keep_time_proc_write,
};
#else
static const struct file_operations xm_htc_idle_high_base_keep_time_fops = {
	.owner = THIS_MODULE,
	.read  = xm_htc_idle_high_base_keep_time_proc_read,
	.write = xm_htc_idle_high_base_keep_time_proc_write,
};
#endif
...
int32_t nvt_extra_proc_init(void)
{
	...
	XM_HTC_proc_report_rate_entry = proc_create(XM_HTC_REPORT_RATE, 0666, NULL, &xm_htc_report_rate_fops);
	...
	XM_HTC_proc_mc_calibration_en_entry = proc_create(XM_HTC_MC_CALIBRATION_EN, 0666, NULL, &xm_htc_mc_calibration_en_fops);
	if ( XM_HTC_proc_mc_calibration_en_entry == NULL) {
		NVT_ERR("create proc/%s Failed!\n", XM_HTC_MC_CALIBRATION_EN);
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/%s Succeeded!\n", XM_HTC_MC_CALIBRATION_EN);
	}
	...
	XM_HTC_proc_base_refresh_enable_entry = proc_create(XM_HTC_BASE_REFRESH_ENABLE, 0666, NULL, &xm_htc_base_refresh_enable_fops);
	if ( XM_HTC_proc_base_refresh_enable_entry == NULL) {
		NVT_ERR("create proc/%s Failed!\n", XM_HTC_BASE_REFRESH_ENABLE);
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/%s Succeeded!\n", XM_HTC_BASE_REFRESH_ENABLE);
	}

	XM_HTC_proc_thp_base_trace_flag_entry = proc_create(XM_HTC_THP_BASE_TRACE_FLAG, 0666, NULL, &xm_htc_thp_base_trace_flag_fops);
	if ( XM_HTC_proc_thp_base_trace_flag_entry == NULL) {
		NVT_ERR("create proc/%s Failed!\n", XM_HTC_THP_BASE_TRACE_FLAG);
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/%s Succeeded!\n", XM_HTC_THP_BASE_TRACE_FLAG);
	}
	...
	XM_HTC_proc_idle_high_base_keep_time_entry = proc_create(XM_HTC_IDLE_HIGH_BASE_KEEP_TIME, 0666, NULL, &xm_htc_idle_high_base_keep_time_fops);
	if ( XM_HTC_proc_idle_high_base_keep_time_entry == NULL) {
		NVT_ERR("create proc/%s Failed!\n", XM_HTC_IDLE_HIGH_BASE_KEEP_TIME);
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/%s Succeeded!\n", XM_HTC_IDLE_HIGH_BASE_KEEP_TIME);
	}
...
void nvt_extra_proc_deinit(void)
{
	...
	if (XM_HTC_proc_mc_calibration_en_entry != NULL) {
		remove_proc_entry(XM_HTC_MC_CALIBRATION_EN, NULL);
		XM_HTC_proc_mc_calibration_en_entry = NULL;
		NVT_LOG("Removed /proc/%s\n", XM_HTC_MC_CALIBRATION_EN);
	}
	...
	if (XM_HTC_proc_base_refresh_enable_entry != NULL) {
		remove_proc_entry(XM_HTC_BASE_REFRESH_ENABLE, NULL);
		XM_HTC_proc_base_refresh_enable_entry = NULL;
		NVT_LOG("Removed /proc/%s\n", XM_HTC_BASE_REFRESH_ENABLE);
	}

	if (XM_HTC_proc_thp_base_trace_flag_entry != NULL) {
		remove_proc_entry(XM_HTC_THP_BASE_TRACE_FLAG, NULL);
		XM_HTC_proc_thp_base_trace_flag_entry = NULL;
		NVT_LOG("Removed /proc/%s\n", XM_HTC_THP_BASE_TRACE_FLAG);
	}
	...
	if (XM_HTC_proc_idle_high_base_keep_time_entry != NULL) {
		remove_proc_entry(XM_HTC_IDLE_HIGH_BASE_KEEP_TIME, NULL);
		XM_HTC_proc_idle_high_base_keep_time_entry = NULL;
		NVT_LOG("Removed /proc/%s\n", XM_HTC_IDLE_HIGH_BASE_KEEP_TIME);
	}
