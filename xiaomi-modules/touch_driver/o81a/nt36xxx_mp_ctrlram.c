...
#define MP_LIMIT_CSV_FILE "MP_Setting_Criteria" /* Ex. MP_Setting_Criteria_5F86.csv*/
...
struct mp_criteria_item mp_criteria_item_table[] = {
...
#endif
	{"PS_Config_Lmt_Short_Rawdata_P:", &PS_Config_Lmt_Short_Rawdata_P, TYPE_ARRAY},
	{"PS_Config_Lmt_Short_Rawdata_N:", &PS_Config_Lmt_Short_Rawdata_N, TYPE_ARRAY},
	{"PS_Config_Lmt_Open_Rawdata_P:", &PS_Config_Lmt_Open_Rawdata_P, TYPE_ARRAY},
	{"PS_Config_Lmt_Open_Rawdata_N:", &PS_Config_Lmt_Open_Rawdata_N, TYPE_ARRAY},
	{"PS_Config_Lmt_FW_Rawdata_P:", &PS_Config_Lmt_FW_Rawdata_P, TYPE_ARRAY},
	{"PS_Config_Lmt_FW_Rawdata_N:", &PS_Config_Lmt_FW_Rawdata_N, TYPE_ARRAY},
	...
	{"PS_Config_Lmt_PenTipX_FW_Raw_P:", &PS_Config_Lmt_PenTipX_FW_Raw_P, TYPE_ARRAY_PEN_X},
	{"PS_Config_Lmt_PenTipX_FW_Raw_N:", &PS_Config_Lmt_PenTipX_FW_Raw_N, TYPE_ARRAY_PEN_X},
	{"PS_Config_Lmt_PenTipY_FW_Raw_P:", &PS_Config_Lmt_PenTipY_FW_Raw_P, TYPE_ARRAY_PEN_Y},
	{"PS_Config_Lmt_PenTipY_FW_Raw_N:", &PS_Config_Lmt_PenTipY_FW_Raw_N, TYPE_ARRAY_PEN_Y},
	{"PS_Config_Lmt_PenRingX_FW_Raw_P:", &PS_Config_Lmt_PenRingX_FW_Raw_P, TYPE_ARRAY_PEN_X},
	{"PS_Config_Lmt_PenRingX_FW_Raw_N:", &PS_Config_Lmt_PenRingX_FW_Raw_N, TYPE_ARRAY_PEN_X},
	{"PS_Config_Lmt_PenRingY_FW_Raw_P:", &PS_Config_Lmt_PenRingY_FW_Raw_P, TYPE_ARRAY_PEN_Y},
	{"PS_Config_Lmt_PenRingY_FW_Raw_N:", &PS_Config_Lmt_PenRingY_FW_Raw_N, TYPE_ARRAY_PEN_Y},
	{"PS_Config_Lmt_PenTipX_FW_Diff_P:", &PS_Config_Lmt_PenTipX_FW_Diff_P, TYPE_ARRAY_PEN_X},
	{"PS_Config_Lmt_PenTipX_FW_Diff_N:", &PS_Config_Lmt_PenTipX_FW_Diff_N, TYPE_ARRAY_PEN_X},
	{"PS_Config_Lmt_PenTipY_FW_Diff_P:", &PS_Config_Lmt_PenTipY_FW_Diff_P, TYPE_ARRAY_PEN_Y},
	{"PS_Config_Lmt_PenTipY_FW_Diff_N:", &PS_Config_Lmt_PenTipY_FW_Diff_N, TYPE_ARRAY_PEN_Y},
	{"PS_Config_Lmt_PenRingX_FW_Diff_P:", &PS_Config_Lmt_PenRingX_FW_Diff_P, TYPE_ARRAY_PEN_X},
	{"PS_Config_Lmt_PenRingX_FW_Diff_N:", &PS_Config_Lmt_PenRingX_FW_Diff_N, TYPE_ARRAY_PEN_X},
	{"PS_Config_Lmt_PenRingY_FW_Diff_P:", &PS_Config_Lmt_PenRingY_FW_Diff_P, TYPE_ARRAY_PEN_Y},
	{"PS_Config_Lmt_PenRingY_FW_Diff_N:", &PS_Config_Lmt_PenRingY_FW_Diff_N, TYPE_ARRAY_PEN_Y},
...
#if TOUCH_THP_SUPPORT
	nvt_read_get_num_mdata(ts->mmap->PEN_2D_RAW_TIP_X_ADDR, RawData_PenTipX_Raw, ts->x_num * ts->y_gang_num);
	nvt_read_get_num_mdata(ts->mmap->PEN_2D_RAW_TIP_Y_ADDR, RawData_PenTipY_Raw, ts->x_gang_num * ts->y_num);
	nvt_read_get_num_mdata(ts->mmap->PEN_2D_RAW_RING_X_ADDR, RawData_PenRingX_Raw, ts->x_num * ts->y_gang_num);
	nvt_read_get_num_mdata(ts->mmap->PEN_2D_RAW_RING_Y_ADDR, RawData_PenRingY_Raw, ts->x_gang_num * ts->y_num);
#else /* #if TOUCH_THP_SUPPORT */
...
int32_t nvt_read_fw_open(int32_t *xdata)
{
...
int32_t nvt_read_fw_short(int32_t *xdata)
{
	uint32_t raw_pipe_addr = 0;
	uint8_t *rawdata_buf = NULL;
	uint32_t x = 0;
...
*******************************************************/
int32_t RawDataTest_SinglePoint_Sub(int32_t rawdata[], uint8_t RecordResult[], uint8_t x_ch, uint8_t y_ch, int32_t Rawdata_Limit_Postive[], int32_t Rawdata_Limit_Negative[])
{
	int32_t i = 0;
	int32_t j = 0;
#if TOUCH_KEY_NUM > 0
...
void nvt_print_selftest_result(int32_t TestResult, uint8_t RecordResult[], int32_t rawdata[], uint8_t x_len, uint8_t y_len)
{
	int32_t i = 0;
...
int32_t parse_mp_setting_criteria_item(char **ptr, const char *name, int32_t *item_value) {
    char *tmp = NULL;
...
int32_t parse_mp_setting_ain_array(char **ptr, const char *name,
...
int32_t parse_mp_criteria_item_array(char **ptr, const char *name,
		int32_t *item_array, int32_t x_ch, int32_t y_ch) {
	char *tmp = NULL;
	int32_t i = 0;
	int32_t j = 0;
	int32_t ret = 0;
	...
	*ptr = tmp;
	...
	for (i = 0; i < y_ch; i++) {
		...
		while ((token = strsep(&tok_ptr, ", \t\r\0"))) {
			if (strlen(token) == 0)
				continue;
			item_array[i * x_ch + j] = (int32_t) simple_strtol(token, NULL, 10);
	...
	while ((token = strsep(&tok_ptr, ", \t\r\0"))) {
		if (strlen(token) == 0)
			continue;
		item_array[y_ch * x_ch + i] = (int32_t) simple_strtol(token, NULL, 10);
...
int32_t nvt_load_mp_setting_criteria_from_csv(void){
	...
	while ( ptr && (ptr < (fbufp + f_file->size))) {
		...
		switch (mp_criteria_item_table[item].type) {
			case TYPE_ITEM:
				if (parse_mp_setting_criteria_item(&ptr, mp_criteria_item_table[item].name,
							(int32_t *) mp_criteria_item_table[item].data) < 0) {
					ret = -1;
					goto exit_free;
				}
				break;
			...
			case TYPE_AIN:
				if (!strcmp(mp_criteria_item_table[item].name, "AIN_X:"))
					len = IC_X_CFG_SIZE;
				else if (!strcmp(mp_criteria_item_table[item].name, "AIN_Y:"))
					len = IC_Y_CFG_SIZE;
				else if (!strcmp(mp_criteria_item_table[item].name, "AIN_KEY:"))
					len = IC_KEY_CFG_SIZE;

				if (parse_mp_setting_ain_array(&ptr, mp_criteria_item_table[item].name,
							mp_criteria_item_table[item].data, len) < 0) {
					ret = -1;
					goto exit_free;
				}
				break;
			...
			case TYPE_ARRAY:
				if (parse_mp_criteria_item_array(&ptr, mp_criteria_item_table[item].name,
		...
		if (item == sizeof(mp_criteria_item_table) / sizeof(struct mp_criteria_item)) {
...
int nvt_ic_self_test_short(void)
{
	...
	int32_t *RawData_Short = NULL;
	uint8_t *RecordResult_Short = NULL;
	...
	if (nvt_update_firmware(MP_UPDATE_FIRMWARE_NAME) < 0) {
		...
		xiaomi_touch_mievent_report_int(TOUCH_EVENT_FWLOAD_ERR, 0, "TpFirmwareLoadFail", "novatek", ERROR_REGULATOR_INIT);
#endif
	...
	printk("Short Test");
	nvt_print_selftest_result(TestResult_Short, RecordResult_Short, RawData_Short, X_Channel, Y_Channel);
...
int nvt_ic_self_test_open(void)
{
	...
	int32_t *RawData_Open = NULL;
	uint8_t *RecordResult_Open = NULL;
	...
	printk("Open Test");
	nvt_print_selftest_result(TestResult_Open, RecordResult_Open, RawData_Open, X_Channel, Y_Channel);
