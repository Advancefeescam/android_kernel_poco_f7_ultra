...
DECL_TCPC_TIMEOUT(TYPEC_RT_TIMER_LEGACY_STABLE, 30*MSEC_PER_SEC),
DECL_TCPC_TIMEOUT(TYPEC_RT_TIMER_LEGACY_RECYCLE, 300*MSEC_PER_SEC),
...
void tcpc_enable_lpm_timer(struct tcpc_device *tcpc, bool en)
{
	...
	if (en) {
		...
		TCPC_INFO("%s tout = %dms\n", __func__, tout / USEC_PER_MSEC);
