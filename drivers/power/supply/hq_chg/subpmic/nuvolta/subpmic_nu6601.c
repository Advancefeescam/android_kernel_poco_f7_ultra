...
static int nu6601_disable_dpdm_block(struct charger_dev *subpmic_dev)
{
	struct nu6601_charger *chg = charger_get_private(subpmic_dev);
	int val;

	val = DPDM_DISABLE << DPDM_EN_SHIFT;
	dev_info(chg->dev, "%s: dpdm block disable\n", __func__);

	return nu6601_reg_update_bits(chg, DPDM_I2C, NU6601_REG_BC12_CTRL, DPDM_EN_MASK, val);
}
...
__maybe_unused static int nu6601_chg_en_cid(struct charger_dev *subpmic_dev, bool en)
{
	struct nu6601_charger *chg = charger_get_private(subpmic_dev);

	return nu6601_cid_enable(chg, en);
}
...
static int nu6601_bc12_det_enable(struct nu6601_charger *chg, bool en)
{
	...
	dev_info(chg->dev, "%s: enable bc12 detect\n", __func__);
#endif
...
static int nu6601_get_rechg_volt(struct charger_dev *subpmic_dev, int *mv)
{
	struct nu6601_charger *chg = charger_get_private(subpmic_dev);
	u8 volt;
	int ret;

	ret = nu6601_reg_read(chg, CHG_I2C, NU6601_REG_RECHG_CFG, &volt);
	if (ret)
		return ret;

	volt = (volt & VBAT_RECHG_MASK) >> VBAT_RECHG_SHIFT;
...
static int nu6601_get_chip_chg_status(struct charger_dev *subpmic_dev, uint32_t *chg_status)
{
	struct nu6601_charger *chg = charger_get_private(subpmic_dev);
	int ret;

	ret = _nu6601_get_chg_type(chg);
	*chg_status = 0;
...
static int _nu6601_set_input_current_limit(struct nu6601_charger *chg, int mA, int id)
{
	...
	dev_info(chg->dev, "%s ma =%d input_suspend = %d", __func__, mA, chg->input_suspend_status);
	if (!chg->input_suspend_status) {
		if (mA <= 100) {
			ret = nu6601_reg_update_bits(chg, CHG_I2C, NU6601_REG_IIN_FINAL,
				IIN_FINAL_FORCE_0MA_MASK, 1 << IIN_FINAL_FORCE_0MA_SHIFT);
			return ret;
		} else {
			ret = nu6601_reg_update_bits(chg, CHG_I2C, NU6601_REG_IIN_FINAL,
				IIN_FINAL_FORCE_0MA_MASK, 0 << IIN_FINAL_FORCE_0MA_SHIFT);
		}
	}
...
static int nu6601_request_dpdm(struct charger_dev *subpmic_dev, bool en)
{
	...
	if (!chg->dpdm_reg && of_get_property(chg->dev->of_node, "dpdm-supply", NULL)) {
		...
		if (IS_ERR(chg->dpdm_reg)) {
			...
			dev_err(chg->dev, "Couldn't get dpdm regulator rc=%d\n", rc);
			chg->dpdm_reg = NULL;
			return rc;
		}
	}
	...
	if (en) {
		if (chg->dpdm_reg && !chg->dpdm_enabled) {
			dev_info(chg->dev, "%s: enabling DPDM regulator\n", __func__);
			rc = regulator_enable(chg->dpdm_reg);
			if (rc < 0)
				dev_err(chg->dev, "%s: Couldn't enable dpdm regulator rc=%d\n",
					__func__, rc);
			else
		...
		if (chg->dpdm_reg && chg->dpdm_enabled) {
			dev_info(chg->dev, "%s: disabling DPDM regulator\n", __func__);
			rc = regulator_disable(chg->dpdm_reg);
			if (rc < 0)
				dev_err(chg->dev, "%s: Couldn't disable dpdm regulator rc=%d\n",
					__func__, rc);
			else
...
static struct charger_ops nu6601_ops = {
	...
	.get_chip_chg_status = nu6601_get_chip_chg_status,
	...
	.disable_dpdm_block = nu6601_disable_dpdm_block,
	...
	.get_rechg_vol = nu6601_get_rechg_volt,
	...
	for (reg = 0x10; reg < 0x7f; reg++) {
		...
		if (buf)
			count = snprintf(buf + count, PAGE_SIZE - count,
					"[0x%x] -> 0x%x\n", reg, val);
		dev_info(chg->dev, "nu6601 reg 0x31: [0x%x] -> 0x%x\n", reg, val);
	}
	nu6601_charger_key(chg, false);
...
static irqreturn_t nu6601_bc1p2_det_done_irq_handler(int irq, void *data)
{
	...
	ret = nu6601_get_input_current_limit(chg->subpmic_dev, &new_icl);
	if (ret < 0) {
		dev_info(chg->dev, "get icl error\n");
	} else {
		if (new_icl != chg->icl_val) {
			dev_info(chg->dev, "icl update  after bc12 detect\n");
			nu6601_set_input_current_limit(chg->subpmic_dev, chg->icl_val);
		}
	}
...
static irqreturn_t nu6601_boost_fail_irq_handler(int irq, void *data)
{
	...
	if (chg->request_otg && (reg58 == 0x80 || reg58 == 0xA0)) {
		if (chg->boost_fault_times < 3) {
			...
			nu6601_get_otg_status(chg->subpmic_dev, &otg_st);
			if(!otg_st) {
				nu6601_pd_request_otg(chg->subpmic_dev, true);
...
static int nu6601_charger_init_setting(struct nu6601_charger *chg)
{
	...
	ret |= nu6601_adc_enable(chg->subpmic_dev, false);
	...
	ret |= nu6601_reg_update_bits(chg, CHG_I2C, NU6601_REG_BOOST_PORT_CFG,
			VBAT_LOW_MASK, VBAT_LOW_2P6V << VBAT_LOW_SHIFT); //set default vbat low 2.6V
