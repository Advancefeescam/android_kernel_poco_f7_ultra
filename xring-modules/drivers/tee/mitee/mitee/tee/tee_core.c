...
#define TEE_SHM_ALLOC_KERNEL_MAGIC 0x006D656D  // MEM
/*
			       ...
			       struct tee_ioctl_shm_alloc_data __user *udata)
{
	...
	if (data.flags == TEE_SHM_ALLOC_KERNEL_MAGIC) {
		vaddr = vmalloc(data.size);
		if (!vaddr) {
			pr_err("vaddr is error\n");
			return -ENOMEM;
		}
		flags = TEE_SHM_DMA_BUF | TEE_SHM_KERNEL_MAPPED | TEE_SHM_USER_KERNEL_MAPPED;
		shm = tee_shm_register(ctx, (unsigned long)vaddr, data.size, flags);
		if (IS_ERR(shm)) {
			vfree(vaddr);
...
static int auth_ca_name(void)
{
	...
	if (current->active_mm->exe_file == NULL) {
		pr_err("current->active_mm->exe_file is NULL\n");
		return -1;
	}
	...
	capath_len = strlen(ca_path);
	for (int index = 0; index < ARRAY_SIZE(white_list); index++) {
