...
int force_get_tbat_internal(struct mtk_battery *gm, bool update)
{
	...
	gm->battery_psy = power_supply_get_by_name("battery");
	if (IS_ERR(gm->battery_psy)) {
		pr_err("Couldn't get battery power supply\n");
		return 25;
	}

	rc = power_supply_get_property(gm->battery_psy, POWER_SUPPLY_PROP_TEMP, &val);
	if (rc < 0) {
		pr_err("Couldn't get battery temp\n");
		return 25;
	}
