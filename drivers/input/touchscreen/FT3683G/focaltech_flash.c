...
/* N6 code for HQ-330297 by liaoxianguo at 2023/9/20 start */
    if (upg->ts_data->fw_loading) {
        FTS_INFO("fw upgrade in process, no need to upgrade again");
        return -EINVAL;
    }
...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    /*O6 code for HQ-443403 by xiongdejun at 2025/1/9 start*/
    ret = fts_init_lockdown_info(upg->ts_data->lockdown_info);
    if (ret) {
        FTS_ERROR("init lockdown info fail!");
    }
    /*O6 code for HQ-443403 by xiongdejun at 2025/1/9 end*/
    upg->ts_data->fw_loading = 1;
    fts_irq_disable();
    fts_esdcheck_switch(upg->ts_data, DISABLE);
...
static int32_t fts_tp_info_show(struct seq_file *m, void *v)
{
        ...
        seq_printf(m, "[Vendor]:Tianma [TP-IC]:FT3683G [FW]:0x%x\n", fw_version);
        return 0;
}
...
int fts_hw_info_add(struct fts_ts_data *ts_data)
{
    ...
    sprintf(tp_version_info, "[Vendor]Tianma [TP-IC]:FT3683G [FW]0x%x\n", fw_version);
...
int lct_get_lockdown_info(void)
{
    ...
    if (ts_data->fw_loading) {
        FTS_INFO("fw upgrade in process, can't read lockdown");
        return -EINVAL;
    }
    /* N6 code for HQ-330297 by liaoxianguo at 2023/9/20 end */
