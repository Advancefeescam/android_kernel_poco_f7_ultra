...
int platform_class_cp_get_pogo_online(unsigned int role, int *online)
{
	struct platform_cp_class_ops_data *temp_data = platform_cp_class_get_ic_ops(role);

	if (platform_cp_ops_invalid(temp_data, cp_get_pogo_online))
		return -1;

	return temp_data->ops->cp_get_pogo_online(online, temp_data->data);

}
EXPORT_SYMBOL(platform_class_cp_get_pogo_online);

int platform_class_cp_set_pogo_plugin(unsigned int role, bool en)
{
	struct platform_cp_class_ops_data *temp_data = platform_cp_class_get_ic_ops(role);

	if (platform_cp_ops_invalid(temp_data, cp_set_pogo_plugin))
		return -1;

	return temp_data->ops->cp_set_pogo_plugin(en, temp_data->data);

}
EXPORT_SYMBOL(platform_class_cp_set_pogo_plugin);

int platform_class_cp_set_keyboard_plugin(unsigned int role, bool en)
{
	struct platform_cp_class_ops_data *temp_data = platform_cp_class_get_ic_ops(role);

	if (platform_cp_ops_invalid(temp_data, cp_set_keyboard_plugin))
		return -1;

	return temp_data->ops->cp_set_keyboard_plugin(en, temp_data->data);

}
EXPORT_SYMBOL(platform_class_cp_set_keyboard_plugin);

int platform_class_cp_get_keyboard_plugin(unsigned int role, int *enable)
{
	struct platform_cp_class_ops_data *temp_data = platform_cp_class_get_ic_ops(role);

	if (platform_cp_ops_invalid(temp_data, cp_get_keyboard_plugin))
		return -1;

	return temp_data->ops->cp_get_keyboard_plugin(enable, temp_data->data);

}
EXPORT_SYMBOL(platform_class_cp_get_keyboard_plugin);
...
struct mca_sysfs_attr_info cp_sysfs_field_tbl[] = {
	...
	mca_sysfs_attr_ro(cp_sysfs, 0440, POGO_CONNECT, pogo_connect),
};
		...
		struct device_attribute *attr, char *buf)
{
	...
	case CP_PROP_TDIE:
		...
		break;
	case POGO_CONNECT:
		platform_class_cp_get_pogo_online(CP_ROLE_MASTER, &(val.intval));
		count = scnprintf(buf, PAGE_SIZE, "%d\n", val.intval);
		break;
	default:
		break;
