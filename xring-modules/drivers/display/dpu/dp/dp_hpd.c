...
#define WAIT_SCREEN_OFF_TIMEOUT_US                    5000000
...
static int dp_handle_sink_request(struct dp_display *display)
{
	...
	if (vec->auto_test_flag) {
	...
	} else if (!link->funcs->check_link_status(link)) {
		...
		if (ktime_to_ms(ktime_sub(now, display->hpd->last_irq_time)) < 5000 &&
				memcmp((u8 *)display->panel->edid + 8, display->hpd->sink_id, SINK_ID_SIZE) == 0)
			display->hpd->irq_downgrade_flag = true;

		display->hpd->last_irq_time = now;
		memcpy(display->hpd->sink_id, (u8 *)display->panel->edid + 8, SINK_ID_SIZE);

		if (!display->hpd->irq_reconn_flag) {
			display->hpd->irq_reconn_flag = true;
...
static void dp_hpd_wait_for_screen_off(struct dp_hpd *hpd)
{
	 ...
	 * case 2 here.
	 */
	if (atomic_read(&display->status.is_connector_enabled) == 0) {
		kthread_cancel_delayed_work_sync(&display->lp_delayed_work.base);
		/* case 2.2 and 2.3 */
		if (display->power->is_lpm_enable) {
			hpd->last_off = false;
...
static void dp_hpd_handle_irq(struct dp_hpd *hpd)
{
	...
	mutex_lock(&hpd->irq_lock);
	ret = dp_handle_sink_request(display);
	mutex_unlock(&hpd->irq_lock);
	if (ret)
