...
static void *g_dis_panel_node_register_cookie;
...
static void notifier_blank_unblank_callcack(enum xring_panel_event_tag tag,
	struct xring_panel_event_notification *notification, void *client_data)
{
	...
	case DRM_PANEL_EVENT_BLANK:
		ret = sh_ipc_send_msg_sync(AP_SR_IPC_CHN_SH, TAG_SH_PM_SCREEN_EVENT, CMD_SH_PM_SCREEN_OFF, NULL, 0);
		if (ret < 0)
			pr_err("screen on notify failed,ret=%x\n", ret);
		break;
	case DRM_PANEL_EVENT_UNBLANK:
		ret = sh_ipc_send_msg_sync(AP_SR_IPC_CHN_SH, TAG_SH_PM_SCREEN_EVENT, CMD_SH_PM_SCREEN_ON, NULL, 0);
		if (ret < 0)
			pr_err("screen off notify failed,ret=%x\n", ret);
		break;
	default:
		pr_err("unsupported notifier type: %d\n", notification->type);
	}
}
...
{
	int ret = 0;
	struct device_node *dsi_panel_node = NULL;
	...
	dsi_panel_node = get_dsi_panel_node();
	if (!dsi_panel_node) {
		pr_err("dsi_panel_node is NULL!\n");
		goto _err_node;
	}

	g_dis_panel_node_register_cookie = xring_panel_event_notifier_register(
		XRING_PANEL_EVENT_TAG_PRIMARY, XRING_PANEL_EVENT_CLIENT_SENSORHUB_NOTIFIER, dsi_panel_node,
		notifier_blank_unblank_callcack, NULL);
...
static int shub_boot_remove(struct platform_device *pdev)
{
	...
	xring_panel_event_notifier_unregister(g_dis_panel_node_register_cookie);
