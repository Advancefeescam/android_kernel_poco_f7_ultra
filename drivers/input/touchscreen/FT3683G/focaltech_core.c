...
/*N6 code for HQ-302671 by dingying at 2023/08/08 end*/
/*P6 code for BUGP6-605 by p-zhaobeidou3 at 2025/7/21 start*/
static int fts_set_board_temp(int temp, bool force);
/*P6 code for BUGP6-605 by p-zhaobeidou3 at 2025/7/21 end*/
...
{
/* N6 code for HQ-330297 by liaoxianguo at 2023/9/20 start */
    struct fts_ts_data *ts_data = fts_data;
    FTS_DEBUG("tp reset");
    if (ts_data->fw_loading) {
        FTS_INFO("fw upgrade in process, can't reset");
        return -EINVAL;
    }
/* N6 code for HQ-330297 by liaoxianguo at 2023/9/20 end */
...
*/
    ...
    if (enable) {
        if (ts_data->power_disabled) {
            ...
            /*O6 code for HQ-437781 by xiongdejun at 2025/1/8 start*/
            mdelay(1);
            ...
            udelay(1500);
            gpio_direction_output(ts_data->pdata->iovdd_gpio, 1);
            /*O6 code for HQ-437781 by xiongdejun at 2025/1/8 end*/
            msleep(1);
...
/* N6 code for HQ-304378 by zhangyundan at 2023/07/12 end */
/* P6 code for BUGP6-4682 by zhaobeidou at 2025/09/01 start */
    if (ts_data->finger_in_fod) {
        input_report_key(ts_data->input_dev, BTN_INFO, 0);
        update_fod_press_status(0);
        input_sync(ts_data->input_dev);
        FTS_INFO("suspend Report_0x152 UP for FingerPrint\n");
    }
/* P6 code for BUGP6-4682 by zhaobeidou at 2025/09/01 end */
    fts_release_all_finger();
...
/* N6 code for HQ-310178 by zhangyundan at 2023/08/10 start */
    ...
    /* N6 code for HQ-330297 by liaoxianguo at 2023/9/20 start */
    if (ts_data->fw_loading) {
        FTS_INFO("fw upgrade in process, do nothing");
        return 0;
    }
    /* N6 code for HQ-330297 by liaoxianguo at 2023/9/20 end */
...
{
    u8 cmd_shift = 0;
/*O6 code for HQ-438680 by liaoxianguo at 2024/12/30 start*/
    if (bit == GESTURE_AOD || bit == GESTURE_FOD || bit == GESTURE_DOUBLETAP)
        cmd_shift = bit;
/*O6 code for HQ-438680 by liaoxianguo at 2024/12/30 end*/
...
/*O6 code for HQ-437764 by xiongdejun at 2024/12/18 start*/
    if (mode == Touch_Doubletap_Mode && value >= 0) {
        FTS_INFO("Mode:Ddoubletap  status = %d", value);
        fts_data->double_click = value != 0 ? true : false;
        fts_data->gesture_support = value != 0 ? true : false;
        fts_update_gesture_state(fts_data, GESTURE_DOUBLETAP, value != 0 ? true : false);
        return 0;
    }
/*O6 code for HQ-437764 by xiongdejun at 2024/12/18 end*/
...
/*O6 code for HQ-438680 by liaoxianguo at 2024/12/30 start*/
    if (mode == Touch_Fod_Enable && value >=0){
        ...
        if (fts_data->fod_status != 0 && fts_data->fod_status != -1) {
            if (fts_data->suspended) {
                fts_reset_proc(50);
                msleep(100);
                fts_irq_enable();
...
/*O6 code for HQ-438680 by liaoxianguo at 2024/12/30 end*/
/*P6 code for BUGP6-2407 by p-zhaobeidou3 at 2025/08/04 start*/
    if (mode == Touch_Is_In_Input_Method && value >=0 && !fts_data->suspended) {
        lct_set_input_mode(value);
        return 0;
    }
/*P6 code for BUGP6-2407 by p-zhaobeidou3 at 2025/08/04 end*/
/*N6 code for HQ-302671 by dingying at 2023/08/08 start*/
...
/*O6 code for HQ-437761 by liaoxianguo at 2024/12/26 start*/
static int fts_set_mode_long_value(int mode, int len, int *buf)
{
	int i = 0;
	int customize = FTS_CUST_EDGE_MIN;
	if (len <= 0)
		return -EIO;
	...
	for (i = 0; i < len; i = i + 8) {
		if (i == FTS_SET_LONG_VALUE) { //defult
			if (xiaomi_touch_interfaces.long_mode_value[i + 4] == FTS_EDGE_FILTER_DEF) {  //defult
				fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, FTS_NOMAL_EDGE_DEF_CMD);
				FTS_INFO("fts_set_mode_long_value is defult");
			} else if (xiaomi_touch_interfaces.long_mode_value[i + 4] == FTS_EDGE_FILTER_STRONG) {  //strong
				fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, FTS_NOMAL_EDGE_STRONG_CMD);
				FTS_INFO("fts_set_mode_long_value is strong");
			} else if (xiaomi_touch_interfaces.long_mode_value[i + 4] == FTS_EDGE_FILTER_WEAK) {  //weak
				fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, FTS_NOMAL_EDGE_WEAK_CMD);
				FTS_INFO("fts_set_mode_long_value is weak");
			} else {  //Customize
				fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, FTS_NOMAL_EDGE_CUST_CMD);
...
/*O6 code for HQ-437761 by liaoxianguo at 2024/12/26 end*/
...
static int fts_set_board_temp(int temp, bool force)
{
    ...
    struct fts_ts_data *ts_data = fts_data;
    if (!ts_data) {
        FTS_ERROR("NO FTS_DATA,ERROR SET TEMPERARURE");
        return ret;
    }
    if (ts_data->suspended) {
        FTS_INFO("suspended no need set temp");
        return ret;
    }

    ret = fts_write_reg(FTS_TEMP_REG, (u8)temp);
    if (ret < 0)
...
/*P6 code for HQFEAT-118700 by p-zhaobeidou3 at 2025/6/27 end*/
    ...
    /*N6 code for HQ-326418 by liaoxianguo at 2023/09/25 end*/
    /*P6 code for BUGP6-605 by p-zhaobeidou3 at 2025/7/21 start*/
    xiaomi_touch_interfaces.set_thermal_temp = fts_set_board_temp;
    /*P6 code for BUGP6-605 by p-zhaobeidou3 at 2025/7/21 end*/
