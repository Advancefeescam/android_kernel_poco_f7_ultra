...
/*cpufreq_limit node in sysfs  for performace profile*/
...
static DEFINE_PER_CPU(struct cpu_qos_data, cpu_qos);

#define SHOW_FREQ(name) \
static ssize_t name##_freq_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf) \
{ \
	struct cpu_qos_data *data = container_of(kobj, struct cpu_qos_data, kobj); \
	s32 value; \
	\
	value = data->req_##name.pnode.prio; \
	return sprintf(buf, "%d\n", value); \
}

#define STORE_FREQ(name) \
static ssize_t name##_freq_store(struct kobject *kobj, struct kobj_attribute *attr, \
...
{ \
	...
	if (kstrtou32(buf, 0, &value)) \
		return -EINVAL; \
	\
	ret = freq_qos_update_request(&data->req_##name, value); \
	if (ret) \
		return -EINVAL; \
	\
	return count; \
...
ATTRIBUTE_GROUPS(qos);

static struct kobj_type qos_ktype = {
	.sysfs_ops = &kobj_sysfs_ops,
	.default_groups = qos_groups,
	.release = NULL,
};

static int qos_cpu_init(int cpu)
{
	struct cpu_qos_data *data = &per_cpu(cpu_qos, cpu);
	...
	ret = kobject_init_and_add(&data->kobj, &qos_ktype, &cpu_dev->kobj, "qos");
	if (ret)
		goto error;
	...
	if (policy) {
		ret = freq_qos_add_request(&policy->constraints,
						&data->req_max, FREQ_QOS_MAX,
						U32_MAX);
		if (ret < 0)
			goto remove_min_qos;

		ret = freq_qos_add_request(&policy->constraints,
						&data->req_min, FREQ_QOS_MIN,
						0);
		if (ret < 0)
			goto remove_max_qos;
...
remove_max_qos:
	freq_qos_remove_request(&data->req_max);

remove_min_qos:
	freq_qos_remove_request(&data->req_min);
...
static void qos_cpu_exit(int cpu)
{
	struct cpu_qos_data *data = &per_cpu(cpu_qos, cpu);
...
static int cpu_qos_init(void)
{
	int ret = 0;
	int cpu = -1;

	for_each_possible_cpu(cpu) {
		ret = qos_cpu_init(cpu);
		if (ret)
			goto unwind;
	}
