...
void dsi_cont_display_res_force_release(struct drm_device *drm_dev)
{
	...
	if (unlikely(drm_conn == NULL)) {
		DPU_ERROR("drm connector is NULL\n");
		return;
	}

	dsi_conn = to_dsi_connector(drm_conn);
	display = dsi_conn->display;
	dsi_display_initialized_set(display, false);
	display->is_panel_power_up = false;
	dsi_display_esd_ctrl_locked(display, false);
	display->panel->initialized = false;
	DSI_INFO("force release dsi res for lastclose\n");
}
...
static void dsi_underflow_work(struct work_struct *work)
{
	...
	if (connector) {
		if (!dsi_connector_is_cmd_mode(connector)) {
			...
			dpu_exception_dump(virt_pipe, crtc->state);
		} else {
