...
static void arch_timer_ipi_handler(void *unused, struct pt_regs *regs)
{
	u64 ctrl, cval, cnt;
	bool state[2];
	int ret;
	...
	ret = irq_get_irqchip_state(irq, IRQCHIP_STATE_PENDING, &state[0]);
	if (ret < 0)
		pr_err("irqchip get pending state failed\n");

	ret = irq_get_irqchip_state(irq, IRQCHIP_STATE_MASKED, &state[1]);
	if (ret < 0)
		pr_err("irqchip get mask state failed\n");

	desc = irq_to_desc(irq);
	if (desc)
		action = desc->action;

	pr_err("ctrl=%llu cnt=%llu cval=%llu; irq=%s pending=%d mask=%d\n",
			ctrl, cnt, cval, action ? action->name : "null",
			state[0], state[1]);
}
...
static void register_walt_hooks(void)
{
	...
	register_trace_android_vh_ipi_stop(arch_timer_ipi_handler, NULL);
}
