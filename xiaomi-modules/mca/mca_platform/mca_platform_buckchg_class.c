...
int platform_class_buckchg_set_typec_orientation_ready(unsigned int role, bool ready)
{
	struct platform_class_buckchg_data *temp_data = platform_class_buckchg_get_ops_data(role);

	if (platform_class_buckchg_invalid_ops(temp_data, set_typec_orientation_ready))
		return -1;

	return temp_data->ops->set_typec_orientation_ready(temp_data->data, ready);
}
EXPORT_SYMBOL(platform_class_buckchg_set_typec_orientation_ready);
