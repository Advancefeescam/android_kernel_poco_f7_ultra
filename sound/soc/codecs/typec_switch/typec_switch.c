...
#include "../../drivers/power/supply/hq_chg/charger_class/include/hq_charger_class.h"
...
#define RES_DET_THRESHOLD	0x32 // Threshold for moisture detection with current source disabled
...
extern void mt6358_accdet_eint_callback_wrapper(unsigned int plug_status);
				      ...
				      unsigned long evt, void *ptr)
{
	...
	if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
		...
		switch (ts_priv->vendor_type) {
			case SWITCH_DIO4485:
			        //I2C reset
				regmap_write(ts_priv->regmap, SWITCH_SETTINGS, 0x00);
		...
		mt6358_accdet_eint_callback_wrapper(0);
...
static void ts_enable_callback(struct work_struct *work)
{
	...
	switch (global_data->vendor_type) {
		...
		case SWITCH_DIO4485:
			if (jack_status == 0x01) {
				...
				regmap_write(global_data->regmap, SWITCH_CONTROL, 0x00);
				mdelay(20);
				regmap_write(global_data->regmap, FUNCTION_ENABLE, 0x08);
				regmap_read(global_data->regmap, FUNCTION_ENABLE, &function_enbale);
				pr_info("%s function_enbale reg:%x\n", __func__, function_enbale);
				mdelay(20);
	...
	mt6358_accdet_eint_callback_wrapper(1);
	ts_dump_register();
}
...
#if IS_ENABLED(CONFIG_RUST_DETECTION)
int ts_moisture_detection_enable(struct charger_dev *chgdev, int en)
{
	...
	ts_priv = charger_get_private(chgdev);
	if (!ts_priv) {
...
int ts_moisture_detection_read_res(struct charger_dev *chgdev)
{
	u32 flag_status = 0;
	u8 cnt = 10;
	u32 cc_val = 0;
...
static struct charger_ops ts_chg_ops = {
...
{
	pr_info("%s\n", __func__);
	ddata->chgdev = charger_register("typec_switch_chg", ddata->dev,
						 &ts_chg_ops, ddata);
...
static int ts_remove(struct i2c_client *i2c)
{
	struct ts_priv *ts_priv =
