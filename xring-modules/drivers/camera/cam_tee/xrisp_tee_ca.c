					  ...
					  uint64_t *fixiova, bool ismap)
{
	...
	mutex_lock(&g_xrisp_pvt->nsbuf_map_mtx);
	ret = xrisp_nsbuf_ssmmu_map_core(sg_pa, 1, TA_SMMU_MMAP_TYPE_FIX_IOVA, fixiova, ismap);
	mutex_unlock(&g_xrisp_pvt->nsbuf_map_mtx);
	if (ret) {
					  ...
					  bool is_map)
{
	...
	sgt_pa = xrisp_pvt->sglist_shm;
	if (!sgt_pa) {
		XRISP_PR_ERROR("sgt_pa is NULL, sg_num=%d", sgt->nents);
		ret = -ENOMEM;
		goto unlock_map_mtx;
	}
	...
	memset(sgt_pa, 0, sizeof(struct sg_table_pa) * sgt->nents);
...
int xrisp_ca_shm_reserve(struct xrisp_ca_private *xrisp_pvt)
{
	int retry_cnt = 0;

	while (retry_cnt++ < CA_KMALLOC_RETRY_TIMES) {
		xrisp_pvt->sglist_shm = kzalloc(SGLIST_SHM_RESERVE_SIZE, GFP_KERNEL);
		if (xrisp_pvt->sglist_shm)
			break;
	...
	if (!xrisp_pvt->sglist_shm) {
		XRISP_PR_ERROR("ca_shm reserve failed");
		return -ENOMEM;
	}
...
void xrisp_ca_shm_free(struct xrisp_ca_private *xrisp_pvt)
{
	kfree(xrisp_pvt->sglist_shm);
	xrisp_pvt->sglist_shm = NULL;
}
...
int xrisp_ca_init(void)
{
	...
	ret = xrisp_ca_shm_reserve(xrisp_pvt);
	if (ret)
		goto free_mem;
...
free_shm:
	xrisp_ca_shm_free(xrisp_pvt->sglist_shm);
free_mem:
	kfree(xrisp_pvt);
	xrisp_pvt = NULL;
	return ret;
...
void xrisp_ca_exit(void)
{
	...
	xrisp_ca_shm_free(g_xrisp_pvt->sglist_shm);
