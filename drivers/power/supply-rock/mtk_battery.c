...
/*L19A HQ-193945 add battery node by tongjiacheng at 2022/04/07 start*/
static enum mtk_battery_property mtk_battery_properties[] = {
...
/*L19A HQ-194258 add shutdown delay 30s by gengyifei at 2022/4/29 start*/
	MTK_BATTERY_PROP_SHUTDOWN_DELAY,
/*L19A HQ-194258 add shutdown delay 30s by gengyifei at 2022/4/29 end*/
...
/*L19A HQ-194258add shutdown delay 30s by gengyifei at 2022/4/29 start*/
#define SHUTDOWN_DELAY_VOL	3350
bool enable_notify_shutdown;
/*L19A HQ-194258add shutdown delay 30s by gengyifei at 2022/4/29 end*/
static int mtk_battery_get_prop(struct mtk_battery_core *bsy,
                                            ...
                                            union mtk_battery_propval *val)
{
	...
	static struct power_supply *psy1;
	/*L19A HQHW-1761 fix disconnect charging by miaozhichao at 2022/06/06 start*/
	union power_supply_propval usb_type = {0};
	/*L19A HQHW-1761 fix disconnect charging by miaozhichao at 2022/06/06 end*/
	int ret = 0;
	int type;

	psy1 = power_supply_get_by_name("battery");
	if (IS_ERR_OR_NULL(psy1)) {
		pr_err("%s: failed to get battery device\n", __func__);
		return PTR_ERR(psy1);
	}

	gm = power_supply_get_drvdata(psy1);
	...
	/*ret = bq2589x_get_usb_type(bq, &type);
	if (ret) {
		pr_err("%s: failed to get bq usb type\n", __func__);
		return ret;
	}*/
	/*L19A HQHW-1761 fix disconnect charging by miaozhichao at 2022/06/06 start*/
	...
	type = usb_type.intval;
	/*L19A HQHW-1761 fix disconnect charging by miaozhichao at 2022/06/06 end*/
	switch (bsp) {
		case MTK_BATTERY_PROP_BATTERY_ID:
			val->intval =  fgauge_get_profile_id();
		break;
		case MTK_BATTERY_PROP_BATTERY_ID_VOLTAGE:
			val->intval = (battery_get_bat_id_voltage())*1000;
		break;
		case MTK_BATTERY_PROP_BATTERY_VENDOR:
			val->intval = fgauge_get_profile_id();
		break;
		case MTK_BATTERY_PROP_USB_OTG:
			val->intval = bq->otg_enabled;
		break;
		case MTK_BATTERY_PROP_TYPEC_CC_ORIENTATION:
			val->intval = typec_cc_orientation_handler(tcpc);
		break;
		case MTK_BATTERY_PROP_REAL_TYPE:
			switch (bq->psy_desc.type) {
				case POWER_SUPPLY_TYPE_UNKNOWN:
					val->intval = MTK_BATTERY_CHARGER_TYPE_UNKNOWN;
				break;
				case POWER_SUPPLY_TYPE_USB:
					if (type == POWER_SUPPLY_USB_TYPE_SDP)
						val->intval = MTK_BATTERY_CHARGER_TYPE_SDP;
					else if (type == POWER_SUPPLY_USB_TYPE_DCP)
						val->intval = MTK_BATTERY_CHARGER_TYPE_FLOAT;
				break;
				case POWER_SUPPLY_TYPE_USB_CDP:
					val->intval = MTK_BATTERY_CHARGER_TYPE_CDP;
				break;
				case POWER_SUPPLY_TYPE_USB_DCP:
					val->intval = MTK_BATTERY_CHARGER_TYPE_DCP;
				break;
				case POWER_SUPPLY_TYPE_USB_PD:
					val->intval = MTK_BATTERY_CHARGER_TYPE_HV_DCP;
				break;
				default:
					val->intval = MTK_BATTERY_CHARGER_TYPE_FLOAT;
				break;
		...
		break;
/*L19A HQ-194739 modify input_suspend by gengyifei at 2022/5/12 start*/
		case MTK_BATTERY_PROP_INPUT_SUSPEND:
/*L19A HQ-194739 modify input_suspend by gengyifei at 2022/5/12 end*/
/*L19A HQ-193822 modify charging enable node by miaozhichao at 2022/4/18 start*/
		case MTK_BATTERY_PROP_CHARGING_ENABLED:
			val->intval = bq->is_input_suspend;
		break;
/*L19A HQ-193822 modify charging enable node by miaozhichao at 2022/4/18 end*/
/*L19A HQ-209466 fix typec mode node by tongjiacheng at 2022/05/09 start*/
		case MTK_BATTERY_PROP_TYPEC_MODE:
			tcpci_get_mode(tcpc);
			switch(tcpc->typec_mode) {
				case 1:
					val->intval = MTK_BATTERY_TYPEC_SOURCE_DEFAULT;
				break;
				case 2:
					val->intval = MTK_BATTERY_TYPEC_SINK;
				break;
				case 0:
					val->intval = MTK_BATTERY_TYPEC_NONE;
				break;
				default:
					val->intval = MTK_BATTERY_TYPEC_NONE;
				break;
		...
		break;
/*L19A HQ-194258 add shutdown delay 30s by gengyifei at 2022/4/29 start*/
		case MTK_BATTERY_PROP_SHUTDOWN_DELAY:
			val->intval= gm->shutdown_delay;
		break;
/*L19A HQ-194258 add shutdown delay 30s by gengyifei at 2022/4/29 end*/
/*L19A HQ-209466 fix typec mode node by tongjiacheng at 2022/05/09 end*/
/*L19A HQ-194773 charge logger resistance by tongjiacheng 2022/05/31 start*/
		case MTK_BATTERY_PROP_RESISTANCE:
			val->intval = 75000;
		break;
/*L19A HQ-194773 charge logger resistance by tongjiacheng 2022/05/31 end*/
/*L19AT HQ-254910 charge_counter by zhaohan 2022/10/18 start*/
		case MTK_BATTERY_PROP_CHARGE_COUNTER:
...
static int mtk_battery_set_prop(struct mtk_battery_core *bsy,
                                            enum mtk_battery_property bsp,
                                            union mtk_battery_propval *val)
{
	int rc = 0;
...
/*L19A HQ-193822 modify charging enable node by miaozhichao at 2022/4/18 start*/
		case MTK_BATTERY_PROP_CHARGING_ENABLED:
			bq2589x_enable_hz(bq->chg_dev, val->intval);
		break;
/*L19A HQ-193822 modify charging enable node by miaozhichao at 2022/4/18 end*/
		default:
			rc = -EINVAL;
...
static int mtk_battery_is_writeable(struct mtk_battery_core *bsy,
                        enum mtk_battery_property bsp)
{
	int rc;
...
/*L19A HQ-193945 add battery node by tongjiacheng at 2022/04/07 end*/
...
int fgauge_get_profile_id(void)
{
	...
	pr_err("[%s] id_voltage is %d",__func__,id_volt);
	...
	union power_supply_propval *val)
{
	...
	int vbat_mv;
	static char *envp[1];
	/*L19A HQ-194258add shutdown delay 30s by gengyifeiat 2022/4/29 end*/
...
/*L19A HQ-159006 add cycle count by tongjiacheng at 2022/04/29 start*/
		val->intval = gm->bat_cycle;
/*L19A HQ-159006 add cycle count by tongjiacheng at 2022/04/29 end*/
		break;
		...
		if(mtk_shutdown_delay_enable) {
			if (val->intval  <= 1){
				...
				if ( vbat_mv <= SHUTDOWN_DELAY_VOL
					...
					val->intval = 1;
				} else if (bs_data->bat_status == POWER_SUPPLY_STATUS_CHARGING
					&& gm->shutdown_delay) {
...
/*L19A HQ-194874 charge_full  node by gengyifei at 2022/4/21 start*/
		pr_err("gm.algo_qmax:%d gm.aging_factor:%d\n", gm->algo_qmax, gm->aging_factor);
		val->intval = gm->algo_qmax * gm->aging_factor / 100;
		break;
/*L19A HQ-194874 charge_full  node by gengyifei at 2022/4/21 end*/
...
/*L19A HQ-194874 charge_full_design  node by gengyifei at 2022/4/21 start*/
		val->intval = 5000000;
		break;
/*L19A HQ-194874 charge_full_design  node by gengyifei at 2022/4/21 end*/
...
static void mtk_battery_external_power_changed(struct power_supply *psy)
{
	...
	if (IS_ERR_OR_NULL(chg_psy)) {
		...
		pr_info("%s ----name:%s  %d  %d\n", __func__,chg_psy->desc->name, online.intval, status.intval);
...
static char *battery_psy_supplied_to[] = {
	"charger",
	"mtk-master-charger",
};
...
/*L19A L19A-15 add thermal limit current by tongjiacheng at 2022/04/26 end*/
	...
	bs_data->psy_cfg.supplied_to = battery_psy_supplied_to;
	bs_data->psy_cfg.num_supplicants = ARRAY_SIZE(battery_psy_supplied_to);

	bs_data->bat_status = POWER_SUPPLY_STATUS_DISCHARGING,
	bs_data->bat_health = POWER_SUPPLY_HEALTH_GOOD,
	bs_data->bat_present = 1,
	/*L19A HQ-194874 technology  node by gengyifei at 2022/4/22 start*/
	bs_data->bat_technology = POWER_SUPPLY_TECHNOLOGY_LIPO,
	/*L19A HQ-194874 technology  node by gengyifei at 2022/4/22 end*/
...
/* ============================================================ */
/*L19A HQ-198771 modify battery ntc by tongjiacheng at 2022/04/06 start*/
...
/*L19A HQ-198771 modify battery ntc by tongjiacheng at 2022/04/06 end*/
...
/*L19A HQ-194293 add recharge ways by tongjiacheng at 2022/04/20  start*/
	...
	charger_dev_is_charging_done(chg_dev, &chg_done);
	/*HQ-223242 modify 2 Celsius charge by miaozhichao at 2022/07/27 start*/
	if (chg_done) {
		if (bat_data->bat_capacity == 100 && bat_data->bat_status != POWER_SUPPLY_STATUS_DISCHARGING
		 && bat_data->bat_batt_temp < 45 && bat_data->bat_batt_temp > 10) {
			bat_data->bat_status = POWER_SUPPLY_STATUS_FULL;
			bm_err("battery_update set FULL! ui:%d chr:%d done:%d\n",
							bat_data->bat_capacity, bat_data->bat_status, chg_done);
		}
/*L19AT code for HQ-267704 by zhaohan at 2022/12/07 start*/
		if (bat_data->bat_batt_temp <= 45 && bat_data->bat_status != POWER_SUPPLY_STATUS_DISCHARGING) {
			bat_data->bat_status = POWER_SUPPLY_STATUS_FULL;
			wakeup_fg_algo(gm, FG_INTR_CHR_FULL);
		}
/*L19AT code for HQ-267704 by zhaohan at 2022/12/07 end*/
	}
	/*HQ-223242 modify 2 Celsius charge by miaozhichao at 2022/07/27 end*/
	bm_err("battery_update  ui:%d chr:%d done:%d\n",
					bat_data->bat_capacity, bat_data->bat_status, chg_done);
/*L19A HQ-194293 add recharge ways by tongjiacheng at 2022/04/20  end*/
...
/*L19A HQ-194258 add shutdown delay 30s by gengyifeiat 2022/4/29 start*/
						//down_to_low_bat = 1;
...
/*L19A HQ-193945 add battery node by tongjiacheng at 2022/04/07 start*/
	gm->bsy_desc.name = "battery";
	gm->bsy_desc.type = MTK_BATTERY_TYPE_BATTERY;
	gm->bsy_desc.properties = mtk_battery_properties;
	gm->bsy_desc.num_properties = ARRAY_SIZE(mtk_battery_properties);
	gm->bsy_desc.get_property = mtk_battery_get_prop;
	gm->bsy_desc.set_property = mtk_battery_set_prop;
	gm->bsy_desc.property_is_writeable = mtk_battery_is_writeable;
	gm->bsy_cfg.drv_data = gm;

	gm->bsy = mtk_battery_class_register(&(pdev->dev), &gm->bsy_desc, &gm->bsy_cfg);
	if (IS_ERR(gm->bsy)) {
		pr_err("%s, fail to register mtk battery device\n", __func__);
		return PTR_ERR(gm->bsy);
	}
/*L19A HQ-193945 add battery node  ntc by tongjiacheng at 2022/04/07 end*/
