...
static void xring_pcie_set_target_speed(struct dw_pcie *pci, u32 link_gen)
{
	...
	dev_info(pci->dev, "set target speed to %d\n", link_speed);
}
...
static int xring_pcie_resume_noirq(struct device *dev)
{
	...
	xring_pcie_set_target_speed(pci, PCI_EXP_LNKCTL2_TLS_2_5GT);
...
static int xring_pcie_resume_early(struct device *dev)
{
	...
	/* Set TLS = CLS */
	offset = dw_pcie_find_capability(pci, PCI_CAP_ID_EXP);
	linksta = dw_pcie_readw_dbi(pci, offset + PCI_EXP_LNKSTA);
	xring_pcie_set_target_speed(pci, FIELD_GET(PCI_EXP_LNKSTA_CLS, linksta));

	xring_pcie_clkreq_sw_mux_set(xring_pcie, 0);
	return 0;
}

static void xring_pcie_resume_complete(struct device *dev)
{
	struct platform_device *pdev;
	struct xring_pcie *xring_pcie;
	struct dw_pcie *pci;
	...
	bus = pci_find_bus(xring_pcie->port_num, 1);
	if (!bus) {
		dev_err(dev, "can not find bus>>>>%s\n", __func__);
		return;
	}

	pci_dev = pci_get_slot(bus, 0);
	if (!pci_dev) {
		dev_err(dev, "can not find pci_dev>>>>%s\n", __func__);
		return;
	}

	xring_pcie_set_speed(pci_dev, pci->link_gen);
...
static int xring_pcie_runtime_resume(struct device *dev)
{
	...
	xring_pcie_resume_complete(dev);
...
static const struct dev_pm_ops xring_pcie_pm_ops = {
	...
	.complete = xring_pcie_resume_complete,
};
