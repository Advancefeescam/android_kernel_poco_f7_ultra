...
* Static function
*****************************************************************************/
/*O6 code for HQ-437780 by xiongdejun at 2024/12/19 start*/
...
/*O6 code for HQ-437780 by xiongdejun at 2024/12/19 end*/
...
//P6 code for HQFEAT-109228 by p-zhaobeidou3 at 2025/07/11 start
static int ft5672_check_reset(void)
{
    int i = 0;
    int ret = 0;
    u8 value = 0;
    fts_reset_proc(50);
    for (i = 0; i < 3; i++) {
        ret = fts_test_read_reg(0x9f, &value);
        if (ret < 0) {
            FTS_TEST_SAVE_ERR("read reg9f fail,ret=%d\n", ret);
        }
        FTS_TEST_INFO("read reg9f=%x", value);
        if ((value == 0x72) || (value == 0x83)) {
            return 0;
        }
        msleep(50);
    }
...
static bool ft5672_ex_rst_test(void)
{
    u8 value_1 = 0;
    u8 value_2 = 0;
    u8 value_3 = 0;
    ...
    FTS_TEST_SAVE_INFO("\n============ Test Item: RST Test\n");
    ret = ft5672_check_reset();
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("check reset failed,ret=%d\n", ret);
        goto out;
    }
    ...
    FTS_TEST_INFO("value_1=%x,value_2=%x\n", value_1, value_2);
    if (value_2 != (value_1 + 1)) {
        FTS_TEST_SAVE_ERR("regad: read(%d)!=write(%d)", value_2, (value_1 + 1));
        goto restore_reg;
    }
...
restore_reg:
    fts_test_write_reg(0xad, value_1);
out:
    return !!tmp_result;
}
...
static int ft5672_rst_test(struct fts_test *tdata, bool *test_result)
{
    ...
    tmp_result1 = ft5672_ex_rst_test();
...
//P6 code for HQFEAT-109228 by p-zhaobeidou3 at 2025/07/11 end
...
/*P6 code for HQFEAT-118700 by p-zhaobeidou3 at 2025/6/27 start*/
            ...
            xiaomi_touch_mievent_report_int(TOUCH_EVENT_OPENTEST_FAIL, 1, "TpOpenTestFail", "focal", ret);
#endif
...
/*P6 code for HQFEAT-118700 by p-zhaobeidou3 at 2025/6/27 start*/
#if IS_ENABLED(CONFIG_MIEV)
            xiaomi_touch_mievent_report_int(TOUCH_EVENT_SHORTTEST_FAIL, 1, "TpShortTestFail", "focal", ret);
#endif
/*P6 code for HQFEAT-118700 by p-zhaobeidou3 at 2025/6/27 end*/
        } else {
    ...
    //P6 code for HQFEAT-109228 by p-zhaobeidou3 at 2025/07/11 start
    /* Reset Pin test */
    if (true == test_item->reset_pin_test) {
        ret = ft5672_rst_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
            fts_ftest->test_item[FTS_RESET_TEST].testresult = RESULT_NG;
        } else {
            fts_ftest->test_item[FTS_RESET_TEST].testresult = RESULT_PASS;
        }
    }
    //P6 code for HQFEAT-109228 by p-zhaobeidou3 at 2025/07/11 end
