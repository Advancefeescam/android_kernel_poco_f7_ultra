			 ...
			 struct goodix_ts_event *ts_event)
{
	...
	ts_debug("large_touch_status1 = %d\n", large_touch_status);
	...
	ts_debug("palm_status = %d\n", cd->palm_status);
	if (cd->palm_status) {
		ts_debug("large_touch_status2 = %d\n", large_touch_status);
		if (large_touch_status & GOODIX_LRAGETOUCH_EVENT) {
			ts_info("cd->palm_status");
			update_palm_sensor_value(1);
			ts_info("palm_sensor report");
			return ret;
		}
