...
static void xrisp_rproc_mbox_recv_callback(void *rx_buff, void *arg)
{
	...
	if (msg_type >= XRISP_RPROC_MSG_MAX || msg_type <= XRISP_RPROC_MSG_INVALED) {
	...
	if (msg_type == XRISP_RPROC_MSG_KICK) {
		...
		uint32_t *data = rx_buff;
		u64 kick_time_start;
		u64 kick_time_delta;

		memcpy(&kick_time_start, &data[2], 8);
		kick_time_delta = xr_timestamp_gettime() / 1000 - kick_time_start;
		if (kick_time_delta > IPC_KICK_MAX_TIMEOUT_US)
			XRISP_PR_WARN("ipc trace: kick fw2ap long time %lluus", kick_time_delta);
#endif
