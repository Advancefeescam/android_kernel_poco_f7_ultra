...
static struct notifier_block mdr_ap_die_block = {
	.notifier_call = mdr_ap_die_notify,
	.priority = INT_MIN,
};
...
int mdr_ap_die_notify(struct notifier_block *nb, unsigned long event, void *buf)
{
	pr_err("===> enter die notify!\n");
...
int mdr_ap_adapter_init(void)
{
	...
	ret = register_die_notifier(&mdr_ap_die_block);
	if (ret) {
		pr_err("register_die_notifier fail, ret = [%d]\n", ret);
		return ret;
	}
