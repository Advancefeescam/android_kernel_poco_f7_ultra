...
/*L19A code for HQ-211785 by gaoxue at 2022/5/20 start*/
#ifdef PROJECT_ROCK
bool tp_charger_status1;
extern bool tp_charger_status;
#endif
/*L19A code for HQ-211785 by gaoxue at 2022/5/20 end*/
...
/*L19A L19A-15 add thermal limit current by tongjiacheng at 2022/04/26 start*/
/*L19A L19A-15 add thermal limit current by miaozhichao at 2022/05/20 start*/
...
static int thermal_mitigation_dcp[THERMAL_MAX] = {
	...
	2000000, 2000000, 2000000, 2000000, 2000000,
	2000000, 2000000, 2000000, 2000000, 2000000 };
/*L19A L19A-64 add thermal limit current by miaozhichao at 2022/06/29 start*/
/*L19A L19A-69 add thermal limit current by liunianliang at 2022/07/07 start*/
static int thermal_mitigation_qc2[THERMAL_MAX] = {
	2000000,1800000, 1400000, 1200000, 1000000, 1000000,
	900000, 800000, 600000, 600000, 600000,
	600000, 600000, 1600000, 1400000, 1300000,
	1100000, 1000000, 1000000, 1000000, 900000,
	900000, 900000, 800000, 800000 };
/*L19A L19A-69 add thermal limit current by liunianliang at 2022/07/07 end*/
/*L19A L19A-64 add thermal limit current by miaozhichao at 2022/06/29 end*/
static int thermal_mitigation_dcp_india[THERMAL_MAX] = {
	...
	2000000, 2000000, 2000000, 2000000, 2000000,
	2000000, 2000000, 2000000, 2000000, 2000000 };
/*L19A L19A-43 add thermal limit current by miaozhichao at 2022/06/13 start*/
/*L19A L19A-64 add thermal limit current by miaozhichao at 2022/06/29 start*/
static int thermal_mitigation_qc2_india[THERMAL_MAX] = {
	2000000,1800000, 1400000, 1200000, 1000000, 900000,
	900000, 900000, 800000, 800000, 700000,
	700000, 600000, 1600000, 1400000, 1300000,
	1100000, 1000000, 1000000, 1000000, 900000,
	900000, 900000, 800000, 800000 };
/*L19A L19A-64 add thermal limit current by miaozhichao at 2022/06/29 end*/
/*L19A L19A-43 add thermal limit current by miaozhichao at 2022/06/13 end*/
/*L19A L19A-15 add thermal limit current by miaozhichao at 2022/05/20 end*/
bool is_india;
struct mtk_charger *pinfo;
static void hqsys_pcba_set_thermal_current_limit(void)
{
	struct PCBA_MSG* pm;
	PCBA_INFO pcba_id = PCBA_INFO_UNKNOW;

	pm = get_pcba_msg();
	if (IS_ERR(pm)) {
		pr_err("%s: failed to read pcba info\n", __func__);
		return;
	}
	pcba_id = pm->huaqin_pcba_config;

	if (pcba_id == PCBA_L19C_P0_1_IN || pcba_id == PCBA_L19A_P0_1_IN ||
		pcba_id == PCBA_L19C_P1_IN || pcba_id == PCBA_L19A_P1_IN ||
		pcba_id == PCBA_L19C_P1_1_IN || pcba_id ==PCBA_L19A_P1_1_IN ||
		pcba_id ==  PCBA_L19C_P2_IN || pcba_id == PCBA_L19A_P2_IN ||
		pcba_id == PCBA_L19C_MP_IN || pcba_id == PCBA_L19A_MP_IN)
		is_india = true;
	else
		is_india = false;

	pr_err("thermal pcba is %d, is india: %d\n", pcba_id, is_india);
}

int charger_manager_get_system_temp_level(void)
{
	if (IS_ERR(pinfo)) {
		pr_err("%s: failed to get charger info\n", __func__);
		return PTR_ERR(pinfo);
	}

	return pinfo->system_temp_level;
}
EXPORT_SYMBOL(charger_manager_get_system_temp_level);

void charger_manager_set_system_temp_level( int temp_level)
{
	int thermal_icl_ua = 0;
	struct charger_data *pdata;
	enum power_supply_type usb_type = 0;
	...
	usb_type = get_charger_type(pinfo);

	switch(usb_type) {
		case POWER_SUPPLY_TYPE_USB_PD:				//hvdcp
			thermal_icl_ua = is_india ? thermal_mitigation_qc2_india[pinfo->system_temp_level] :
																	thermal_mitigation_qc2[pinfo->system_temp_level];
			break;
		case POWER_SUPPLY_TYPE_USB_DCP:
		default:
			thermal_icl_ua = is_india ? thermal_mitigation_dcp_india[pinfo->system_temp_level] :
																	thermal_mitigation_dcp[pinfo->system_temp_level];
			break;
	}
...
EXPORT_SYMBOL(charger_manager_set_system_temp_level);
/*L19A L19A-15 add thermal limit current by tongjiacheng at 2022/04/26 end*/
...
/*L19A HQ-194293 add recharge ways by tongjiacheng at 2022/04/20 start*/
	if (of_property_read_u32(np, "recharger_uisoc_limit", &val) >= 0)
		info->data.recharger_uisoc_limit = val;
	else {
		chr_err("use default RECHARGER_UISOC_LIMIT: %d\n",
			RECHARGER_UISOC_LIMIT);
		info->data.recharger_uisoc_limit =
					RECHARGER_UISOC_LIMIT;
	}
...
/*L19A HQ-194878 add jeita ways by miaozhichao at 2022/04/27 start*/
	...
	if (info->battery_temp >= info->data.temp_t5_thres) {
			...
			chr_err("[SW_JEITA] Battery Temperature between %d and %d !!\n",
				...
				info->data.temp_t5_thres);
/*L19A HQ-230035 modify High temperature overcharge problem by miaozhichao at 2022/8/24 start*/
			vbat_val = get_battery_voltage(info);
			pr_err("tmp:%d,vbat:%d,bq->part_no:%d\n",info->battery_temp,vbat_val,bq->part_no);
			if( bq->part_no == 1) {
				if (vbat_val > 4100) {
					sw_jeita->charging = false;
					cv_uvlo_flag = 1;
...
/*L19A code for HQHW-1761 by tongjiacheng at 2022/06/07 start*/
...
/*L19A code for HQHW-1761 by tongjiacheng at 2022/06/07 end*/
...
static void kpoc_power_off_check(struct mtk_charger *info)
{
	...
	if (boot_mode == 8 || boot_mode == 9) {
		vbus = get_vbus(info);
/*L19A HQ-194100 modify charge down by miaozhichao at 2022/04/28 start*/
		if (vbus >= 0 && vbus < 2700 && !mtk_is_charger_on(info) && !info->pd_reset) {
			chr_err("Unplug Charger/USB in KPOC mode, vbus=%d, shutdown\n", vbus);
/*L19A HQ-194100 modify charge down by miaozhichao at 2022/04/28 end*/
			while (1) {
				if (counter >= 20000) {
					chr_err("%s, wait too long\n", __func__);
/*L19A L19A-30 kpoc plug out display by miaozhichao at 2022/05/16 start*/
					msleep(4000);
/*L19A L19A-30 kpoc plug out display by miaozhichao at 2022/05/16 end*/
					kernel_power_off();
					break;
				}
...
/*L19A L19A-30 kpoc plug out display by miaozhichao at 2022/05/16 start*/
					msleep(4000);
/*L19A L19A-30 kpoc plug out display by miaozhichao at 2022/05/16 end*/
/*L19A HQ-221230 modify CDP charge by gengyifei at 2022/07/5 start*/
					vbus = get_vbus(info);
					if (vbus > 4000){
						break;
					}
/*L19A HQ-221230 modify CDP charge by gengyifei at 2022/07/5 end*/
					kernel_power_off();
				} else {
...
static void charger_status_check(struct mtk_charger *info)
{
	...
	if (chg_psy == NULL) {
		chr_err("%s, Couldn't get chg_psy,retry\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	switch(info->chr_type) {
		case POWER_SUPPLY_TYPE_USB:					//sdp or float
		case POWER_SUPPLY_TYPE_USB_CDP:
		case POWER_SUPPLY_TYPE_USB_DCP:
			res = 0;
		break;
...
/*L19A HQ-226260 add real_type by miaozhichao at 2022/08/02 start*/
static ssize_t real_type_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int res = MTK_BATTERY_CHARGER_TYPE_UNKNOWN;
	struct power_supply *psy;
	struct mtk_charger *info;
	...
	switch(info->chr_type) {
		case POWER_SUPPLY_TYPE_UNKNOWN:
			res = MTK_BATTERY_CHARGER_TYPE_UNKNOWN;
		break;
		case POWER_SUPPLY_TYPE_USB:
			if (info->usb_type == POWER_SUPPLY_USB_TYPE_SDP)
					res = MTK_BATTERY_CHARGER_TYPE_SDP;
			else if (info->usb_type == POWER_SUPPLY_USB_TYPE_DCP)
					res = MTK_BATTERY_CHARGER_TYPE_FLOAT;
		break;
		case POWER_SUPPLY_TYPE_USB_CDP:
			res = MTK_BATTERY_CHARGER_TYPE_CDP;
		break;
		case POWER_SUPPLY_TYPE_USB_DCP:
			res = MTK_BATTERY_CHARGER_TYPE_DCP;
		break;
		case POWER_SUPPLY_TYPE_USB_PD:
			res = MTK_BATTERY_CHARGER_TYPE_HV_DCP;
		break;
		default:
			res = MTK_BATTERY_CHARGER_TYPE_UNKNOWN;
		break;
...
/*L19A HQ-226260 add real_type by miaozhichao at 2022/08/02 end*/
...
{
	return sysfs_create_group(&info->psy_usb->dev.kobj,
								&usb_psy_attrs_group);
}
/*L19A HQ-194263 add  quick charge type by tongjiacheng at 2022/05/10 end*/
...
static void notify_plug_out(void)
{
	...
	int ret;
	struct power_supply *charger_psy = power_supply_get_by_name("mtk-master-charger");
	if (charger_psy == NULL)
		return;
		...
		unsigned long event, void *data)
{
	...
	struct mtk_charger *pinfo =
			container_of(nb, struct mtk_charger, tcpc_nb);
	bool vbus_on;
	...
	case TCP_NOTIFY_SOURCE_VBUS:
		...
		charger_dev_enable_otg(pinfo->chg1_dev, vbus_on);
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			//pr_err(dev, "Type-C SINK plug in\n");
		...
		//	mutex_unlock(&pinfo->pd_lock);
			...
			/*L19A code for HQ-211785 by gaoxue at 2022/5/20 start*/
			#ifdef PROJECT_ROCK
			tp_charger_status1 = true;
			tp_charger_status = tp_charger_status1;
			#endif
