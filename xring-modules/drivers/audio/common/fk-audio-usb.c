	...
	struct snd_pcm_hw_params *params)
{
	...
	list_for_each_entry(as, &chip->pcm_list, list) {
		if (!as) {
			AUD_LOG_INFO(AUD_COMM, "as(%d) null", dir);
			return NULL;
		}
	...
	int dir, struct dai_format dai_fmt)
{
	...
	if (dir == STRM_DIR_PLAYBACK) {
		if ((audev[card_id].jack_type & SND_JACK_HEADPHONE) == 0) {
			AUD_LOG_INFO(AUD_COMM, "jack type(%d) unmatch", audev[card_id].jack_type);
			ret = -1;
			goto out;
		}
	} else {
		if ((audev[card_id].jack_type & SND_JACK_MICROPHONE) == 0) {
			AUD_LOG_INFO(AUD_COMM, "jack type(%d) unmatch", audev[card_id].jack_type);
			ret = -1;
			goto out;
		}
...
static void audio_usb_connect(struct snd_usb_audio *chip)
{
	...
	AUD_LOG_INFO(AUD_COMM, "prepare jack(%d) report.", audev[card_num].jack_type);
