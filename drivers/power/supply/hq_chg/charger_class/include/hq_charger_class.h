...
struct charger_ops {
	...
	int (*get_vbus_type)(struct charger_dev *charger, enum vbus_type *vbus_type);
	...
	int (*get_chg_status)(struct charger_dev *charger, uint32_t *chg_state,
							uint32_t *chg_status);
	int (*disable_dpdm_block)(struct charger_dev *charger);
	...
	int (*set_chg)(struct charger_dev *charger, bool en);
	int (*get_chg)(struct charger_dev *charger);
	...
	int (*get_rechg_vol)(struct charger_dev *charger, int *mv);
	...
	int (*dump_registers)(struct charger_dev *charger);
	int (*en_cid)(struct charger_dev *charger, bool en);
#if IS_ENABLED(CONFIG_RUST_DETECTION)
	int (*rust_detection_enable)(struct charger_dev *charger, int en);
	int (*rust_detection_read_res)(struct charger_dev *charger);
#endif
};
