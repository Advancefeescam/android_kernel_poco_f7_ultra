...
#define SY6976_1P1_DEVICE_ID        0x67
#define SY6976A_DEVICE_ID           0x67
...
enum {
    SUBPMIC_SLAVE_HK = 0,
    SUBPMIC_SLAVE_CHG = 0,
#ifdef CONFIG_SY_SUBPMIC_SY6976
    SUBPMIC_SLAVE_LED = 0,
#endif /* CONFIG_SY_SUBPMIC_SY6976 */
    SUBPMIC_SLAVE_DPDM = 0,
    SUBPMIC_SLAVE_VOOC = 0,
    SUBPMIC_SLAVE_UFCS,
#ifdef CONFIG_SY_SUBPMIC_SYXXXX
    SUBPMIC_SLAVE_LED,
#endif /* CONFIG_SY_SUBPMIC_SYXXXX */
    SUBPMIC_SLAVE_MAX,
};

static const u8 subpmic_slave_addr[] = {
    0x3D, 0x3F, //YCX TBD 0x64,
};
...
static int subpmic_check_chip(struct subpmic_device *sc)
{
    ...
    if (ret < 0 || (did != SY6976_DEVICE_ID && did != SY6976_1P1_DEVICE_ID && did != SY6976A_DEVICE_ID)) {
        return -ENODEV;
    }
    return 0;
}
...
static int subpmic_probe(struct i2c_client *client)
{
    ...
    sc->sy6976_irq = of_get_named_gpio(client->dev.of_node, "intr-gpio", 0);
    if (ret < 0) {
        dev_err(sc->dev, "%s no intr_gpio info\n", __func__);
        return ret;
    } else {
        dev_info(sc->dev, "%s intr_gpio infoi %d\n", __func__, sc->sy6976_irq);
    }
    ret = gpio_request(sc->sy6976_irq, "sy6976 irq pin");
    if (ret) {
        dev_err(sc->dev, "%s: %d gpio request failed\n", __func__, ret);
        return ret;
    }
    dev_info(sc->dev, "%s gpio_irq=%d\n", __func__, sc->sy6976_irq);
    sc->irqn = gpio_to_irq(sc->sy6976_irq);
