...
int dp_display_tmg_state_get(struct dp_display *display)
{
	 ...
	 */
	if (atomic_read(&display->status.is_connector_enabled) == 1)
...
void dp_display_prepare_link_param(struct dp_display *display)
{
	...
	/* irq downgrade trigger */
	if (display->hpd->irq_downgrade_flag &&
			memcmp((u8 *)display->panel->edid + 8, display->hpd->sink_id, SINK_ID_SIZE) == 0) {
		DP_INFO("irq link_rate downgrade triggered\n");
		display->hpd->irq_downgrade_flag = false;
		dst_param->init_link_rate = (dst_param->init_link_rate == DP_LINK_RATE_RBR) ?
				DP_LINK_RATE_RBR : dst_param->init_link_rate - 1;
	}
}
...
static void dp_display_tmg_video_stream_on(struct dp_display *display)
{
	struct dp_hw_tmg *hw_tmg = display->hw_modules.hw_tmg;
	struct dp_panel_video_info *video_info = &display->panel->video_info;
	struct dp_panel_dsc_info *dsc_info = &display->panel->dsc_info;
...
static void dp_display_dptx_video_stream_on(struct dp_display *display)
{
...
{
	dp_display_tmg_video_stream_on(display);

	dp_display_dptx_video_stream_on(display);

	/* update status */
	display->status.timing = &display->panel->video_info.timing;
	display->status.is_dsc_enabled = display->panel->dsc_info.dsc_en;
...
static void dp_display_lp_delayed_work_handle(struct kthread_work *work)
{
	...
	if (display->power->state == DP_POWER_STATE_OFF) {
		DP_INFO("dp power is off\n");
		return;
	}

	if (display->status.hpd == DP_HPD_EVENT_PLUG_OUT) {
		DP_INFO("dp hpd is plugged-out\n");
		return;
	}
...
void dp_display_lp_exit(struct dp_display *display)
{
	...
	/* link is not ok, retraining */
	mutex_lock(&display->hpd->irq_lock);
	ret = ctrl->funcs->start_link_training(ctrl);
	mutex_unlock(&display->hpd->irq_lock);
	if (ret) {
		/* training failed, keep tmg video stream on, re-connect will depend on a irq event */
		dp_display_tmg_video_stream_on(display);
		return;
	}
	...
	/* if bandwidth is downgrade */
	if (video_info->timing.pixel_clock_khz > display->status.maxpclk_khz) {
		dp_display_tmg_video_stream_on(display);
		xr_usb_queue_dp_hpd(display->xr_usb, DP_HPD_EVENT_PLUG_OUT);
		xr_usb_queue_dp_hpd(display->xr_usb, DP_HPD_EVENT_PLUG_IN);
		return;
	}
...
int dp_display_init(struct platform_device *pdev, struct dp_display *display)
{
	...
	atomic_set(&display->status.is_connector_enabled, 0);
