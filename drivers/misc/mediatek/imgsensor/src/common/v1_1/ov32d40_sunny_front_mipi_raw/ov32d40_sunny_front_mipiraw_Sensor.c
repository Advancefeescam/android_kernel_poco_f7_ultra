...
#include "ov32d40_sunny_front_mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV32D40_SUNNY_FRONT_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	...
	.pre = {
		...
		.framelength = 2604,			//record different mode's framelength
	...
	.hs_video = {
		.pclk = 100000000,				//record different mode's pclk
		.linelength  = 591,				//record different mode's linelength
	...
	.custom3 = {
		.pclk = 100000000,				//record different mode's pclk
		.linelength  = 591,				//record different mode's linelength
		.framelength = 2820,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width  = 1280,		//record different mode's width of grabwindow
		.grabwindow_height = 720,		//record different mode's height of grabwindow
	...
	.max_gain = 62 * BASEGAIN, //real again is 15.5x
	.min_gain_iso = 50,
	.gain_step = 1,
	.gain_type = 1,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[] = {
	{ 6592, 4960, 0, 0,   6592, 4960, 3296, 2480,   16, 16,  3264, 2448, 0, 0, 3264, 2448 },// Preview
	{ 6592, 4960, 0, 0,   6592, 4960, 3296, 2480,   16, 16,  3264, 2448, 0, 0, 3264, 2448 },//capture
	{ 6592, 4960, 0, 304, 6592, 4352, 3296, 2176,   16, 168,  3264, 1840, 0, 0, 3264, 1840 },//normal-video
	{ 6592, 4960, 336, 496,   5920, 3968, 1480, 992,  100, 136, 1280, 720, 0, 0, 1280, 720 },//hs-video
	{ 6592, 4960, 0, 0,   6592, 4960, 3296, 2480,   16, 16,  3264, 2448, 0, 0, 3264, 2448 }, // slim video
	{ 6592, 4960, 672, 684,   5248, 3592, 2624, 1796,   352, 358,  1920, 1080, 0, 0, 1920, 1080 },//custom1
	{ 6592, 4960, 0, 0,   6592, 4960, 3296, 2480,   16, 16,  3264, 2448, 0, 0, 3264, 2448 },//custom2
	{ 6592, 4960, 336, 496,   5920, 3968, 1480, 992,  100, 136, 1280, 720, 0, 0, 1280, 720 },//custom3
	{ 6592, 4960, 336, 496,   5920, 3968, 1480, 992,  100, 136, 1280, 720, 0, 0, 1280, 720 },//custom4
};
...
static kal_uint32 return_sensor_id(void)
{
	...
	printk("[%s] OV32D40_SUNNY_FRONT sensor_id: 0x%x vendor_id: 0x%x sensorinfo_id: 0x%x", __func__, sensor_id, imgsensor.vendor_id, imgsensor_info.sensor_id);
	if ((VENDOR_ID == imgsensor.vendor_id) && sensor_id == imgsensor_info.sensor_id)
	{
		sensor_id = OV32D40_SUNNY_FRONT_SENSOR_ID;
		printk("[%s] OV32D40_SUNNY_FRONT sensor_id: 0x%x", __func__, sensor_id);
	}
	else
		sensor_id = 0;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain > 3968) {
		write_cmos_sensor(0x3508, 0x3e);//62x analog
		write_cmos_sensor(0x3509, 0x00);

	}if (gain < 72){
		write_cmos_sensor(0x3508, 0x01);//62x analog
		write_cmos_sensor(0x3509, 0x20);
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		...
		write_cmos_sensor(0x3729, 0x7c);
		write_cmos_sensor(0x0100, 0x01);
		mdelay(5);
	} else {
		write_cmos_sensor(0x3716, 0x08);
		write_cmos_sensor(0x3717, 0x20);
		write_cmos_sensor(0x3729, 0x7e);
...
#define ENABLE_PDC_FUNCTION
#ifdef ENABLE_PDC_FUNCTION
/*****************add pdc */
#define PDC_SIZE   450 // 732 - 12
//#define XTALK_SIZE 288

#define FLAG_PDC_VALID_OFFSET          0x0F4C

#define PDC_EEPROM_OFFSET   (0x0F4D + 8)
#define PDC_SENSOR_OFFSET   0x5A20
...
{
	int i, ret;
	kal_uint8 pdc_flag_vaild          = 0;
	char pu_send_cmd[2]               = {0};
	char pdc_cali_array[PDC_SIZE]     = {0};
	//char xtalk_cali_array[XTALK_SIZE] = {0};

	pu_send_cmd[0] = (char)(FLAG_PDC_VALID_OFFSET >> 8);
	pu_send_cmd[1] = (char)(FLAG_PDC_VALID_OFFSET & 0xFF);
	/* Check I2C is normal */
	ret = iReadRegI2C(pu_send_cmd, 2, &pdc_flag_vaild, 1, EEPROM_SLAVE_ID);
	LOG_INF("enter read_cali_data_from_eeprom pdc_flag_vaild = %d",pdc_flag_vaild);
	if (ret != 0) {
		LOG_INF("iReadRegI2C error");
		return;
	}
	if ((Is_Read_Otp_Data != 1) && pdc_flag_vaild) {
		/*Multi-Read pdc data*/
		...
		for (i = 0; i < PDC_SIZE; i++) {
			...
			//LOG_INF("PDC EEPROM write add = 0x%x, data = 0x%x \n",pdc_cali_setting[2 * i],pdc_cali_setting[2 * i + 1]);
		}
        /*
...
static void sensor_init(void)
{
	...
	table_write_cmos_sensor(ov32d40sunny_init_setting,
			sizeof(ov32d40sunny_init_setting) / sizeof(kal_uint16));
}	/*	sensor_init  */
...
static void preview_setting(void)
{
	...
	table_write_cmos_sensor(ov32d40sunny_preview_setting,
		sizeof(ov32d40sunny_preview_setting) / sizeof(kal_uint16));
}	/*	preview_setting  */
...
static void capture_setting(void)
{
	...
	table_write_cmos_sensor(ov32d40sunny_capture_setting,
		sizeof(ov32d40sunny_capture_setting) / sizeof(kal_uint16));
}	/*	preview_setting  */

static void normal_video_setting(void)
{
	printk("loading normal_video_setting\n");
	table_write_cmos_sensor(ov32d40sunny_normal_video_setting,
		sizeof(ov32d40sunny_normal_video_setting) / sizeof(kal_uint16));
...
static void hs_video_setting(void)
{
	...
	table_write_cmos_sensor(ov32d40sunny_hs_video_setting,
		sizeof(ov32d40sunny_hs_video_setting) / sizeof(kal_uint16));
}
...
{
	LOG_INF("E\n");
	table_write_cmos_sensor(ov32d40sunny_custom1_setting,
		sizeof(ov32d40sunny_custom1_setting) / sizeof(kal_uint16));
}

static void custom2_setting(void)
{
	LOG_INF("E\n");
	table_write_cmos_sensor(ov32d40sunny_custom2_setting,
		sizeof(ov32d40sunny_custom2_setting) / sizeof(kal_uint16));
}
static void custom3_setting(void)
{
	LOG_INF("E\n");
	table_write_cmos_sensor(ov32d40sunny_custom3_setting,
		sizeof(ov32d40sunny_custom3_setting) / sizeof(kal_uint16));
}
...
static void custom4_setting(void)
{
	...
	table_write_cmos_sensor(ov32d40sunny_custom4_setting,
		sizeof(ov32d40sunny_custom4_setting) / sizeof(kal_uint16));
}
/*************************************************************************
* FUNCTION
*	get_imgsensor_id
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	LOG_INF("OV32D40SUNNY get_imgsensor_id in\n");
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
	//O6 code for HQ-409961 geyanjie at 2024/08/20 start
	...
	//O6 code for HQ-409961 geyanjie at 2024/08/20 end
...
static kal_uint32 ana_gain_table_16x[] = {
	...
	1792, 1856, 1920, 1984, 2048, 2176, 2304, 2432, 2560, 2688,
	2816, 2944, 3072, 3200, 3328, 3456, 3584, 3712, 3840, 3968,
    4096, 4352, 4608, 4864, 5120, 5376, 5632, 5888, 6144, 6400,
	6656, 6912, 7168, 7424, 7680, 7936, 8192, 8704, 9216, 9728,
	10240,10752,11264,11776,12288,12800,13312,13824,14336,14848,
	15360,15872,16384,17408,18432,19456,20480,21504,22528,23552,
	24576,25600,26624,27648,28672,29696,30720,31744,32768,34816,
	36864,38912,40960,43008,45056,47104,49152,51200,53248,55296,
...
{
    LOG_INF("OV32D40_SUNNY_FRONT_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	OV32D40_SUNNY_FRONT_MIPI_RAW_SensorInit	*/
