...
extern int32_t nvt_xm_htc_set_idle_high_base_en(int16_t idle_high_base_en);
...
void nvt_ts_pen_wakeup_gesture_report(uint8_t pen_gesture_id)
{
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	/* read id pin value, 1 is CSOT, 0 is TM */
	ts->lcd_id_value = gpio_get_value(ts->lcd_id_gpio);
	NVT_LOG("lcd_id_value = %d, this panel is %s", ts->lcd_id_value, ts->lcd_id_value ? "CSOT" : "TM");
#endif // #if 0
...
static void nvt_ts_parse_data_func(void *data)
{
	...
	uint16_t write_cmd;
	uint8_t *frame_data;
	uint8_t *reserved_big_buf;
	uint16_t water_bit_map_len;
	uint8_t pen_hopping_index;
	uint16_t pen_tip_scan_freq;
	uint16_t pen_ring_scan_freq;
	...
	uint16_t pen_tip_noise_r0;
	uint16_t pen_tip_noise_r1;
	uint16_t pen_ring_noise_r0;
	uint16_t pen_ring_noise_r1;
	#if FRAME_DATA_PACKET_DEBUG_PRINT
	uint8_t *pen_data;
	uint32_t offset;
	uint8_t *hand_data;
	...
	if ((checksum_bar != ((uint16_t)(~checksum))) || (crc_len_bar != ~crc_len) || (checksum_cal != checksum)) {
		// checksum or crc_len wrong!
		NVT_ERR("checksum wrong(checksum=0x%04X, crc_len=0x%08X, checksum_bar=0x%04X, ~crc_len=0x%08X, checksum_cal=0x%04X)!\n",
	...
	head_cnt = *((uint16_t *)(frame_data_packet + 2));
	...
	cur_frame_len = *((uint16_t *)(frame_data_packet + 44));
	next_frame_len = *((uint16_t *)(frame_data_packet + 46));
	...
	debug_buf_size = frame_data_packet[59];
	reserved_big_buf_size = *((uint16_t *)(frame_data_packet + 60));
	write_cmd = *((uint16_t *)(frame_data_packet + 62));
	frame_data = frame_data_packet + 64;
	reserved_big_buf = frame_data_packet + cur_frame_len - reserved_big_buf_size;
	water_bit_map_len = *((uint16_t *)(reserved_big_buf + 4));
	pen_hopping_index = reserved_big_buf[6 + water_bit_map_len];
	pen_tip_scan_freq = *((uint16_t *)(reserved_big_buf + 7 + water_bit_map_len));
	pen_ring_scan_freq = *((uint16_t *)(reserved_big_buf + 9 + water_bit_map_len));
	#if FRAME_DATA_PACKET_DEBUG_PRINT
	#if NVT_CRC_DATA_ENABLE
	printk("nvt-ts:      frame_data_type=%d, head_cnt=%d, scan_mode=%d, frame_no=%d, drop_frame_no=%d, checksum=0x%04X, crc_len=0x%08X, checksum_bar=0x%04X, ~crc_len=0x%08X, checksum_cal=0x%04X\n",
			frame_data_type, head_cnt, scan_mode, frame_no, drop_frame_no, checksum, crc_len, checksum_bar, crc_len_bar,  checksum_cal);
	#else /* #if NVT_CRC_DATA_ENABLE */
	printk("nvt-ts:      frame_data_type=%d, head_cnt=%d, scan_mode=%d, frame_no=%d, drop_frame_no=%d\n",
			frame_data_type, head_cnt, scan_mode, frame_no, drop_frame_no);
	#endif /* #if NVT_CRC_DATA_ENABLE */
	...
	printk("nvt-ts:      cur_frame_len=%d, pen_hopping_index=%d, pen_tip_scan_freq=%d, pen_ring_scan_freq=%d\n", cur_frame_len, pen_hopping_index, pen_tip_scan_freq, pen_ring_scan_freq);
	#endif /* #if FRAME_DATA_PACKET_DEBUG_PRINT */
	#if FRAME_DATA_PACKET_DEBUG_PRINT
	...
	if (frame_data_type == 0 || frame_data_type == 3 || frame_data_type == 14) {
		#if FRAME_DATA_PACKET_DEBUG_PRINT
		if (print_debug_data) {
		...
		if ((checksum2_bar != ((uint16_t)(~checksum2))) || (checksum2_cal != checksum2)) {
			NVT_ERR("nvt-ts:     checksum2 wrong(checksum2=0x%04X, crc2_len=0x%08X, checksum2_bar=0x%04X, checksum2_cal=0x%04X)!\n",
		...
		#else // NVT_CRC_DATA_ENABLE
		printk("PEN: drop_frame_no=%d, frame_no=%d, pressure=%d, btn1=%d, btn2=%d, battery=%d, hover=%d.\n",
				pen_drop_frame_no, pen_frame_no, pen_pressure, pen_btn1, pen_btn2, pen_battery, pen_hover_status);
		#endif // NVT_CRC_DATA_ENABLE
		/* pen data */
		...
		pen_tip_scan_freq = *((uint16_t *)(frame_data + 20));
		pen_tip_noise_r0 = *((uint16_t *)(frame_data + 22));
		pen_tip_noise_r1 = *((uint16_t *)(frame_data + 24));
		pen_ring_scan_freq = *((uint16_t *)(frame_data + 26));
		pen_ring_noise_r0 = *((uint16_t *)(frame_data + 28));
		pen_ring_noise_r1 = *((uint16_t *)(frame_data + 30));
		#if NVT_CRC_DATA_ENABLE
		...
		if ((checksum2_bar != ((uint16_t)(~checksum2))) || (crc2_len_bar != ~crc2_len) || (checksum2_cal != checksum2)) {
			NVT_ERR("nvt-ts:     checksum2 wrong(checksum2=0x%04X, crc2_len=0x%08X, checksum2_bar=0x%04X, ~crc2_len=0x%08X, checksum2_cal=0x%04X)!\n",
		...
		#else // NVT_CRC_DATA_ENABLE
		printk("nvt-ts:     PEN: drop_frame_no=%d, frame_no=%d, pressure=%d, btn1=%d, btn2=%d, battery=%d, hover=%d.\n",
				pen_drop_frame_no, pen_frame_no, pen_pressure, pen_btn1, pen_btn2, pen_battery, pen_hover_status);
		#endif // NVT_CRC_DATA_ENABLE
		printk("nvt-ts:     PEN: pen_scan_rate=%d, pen_tip_scan_freq=%d, pen_tip_noise_r0=%d, pen_tip_noise_r1=%d, pen_ring_scan_freq=%d, pen_ring_noise_r0=%d, pen_ring_noise_r1=%d\n",
				pen_scan_rate, pen_tip_scan_freq, pen_tip_noise_r0, pen_tip_noise_r1, pen_ring_scan_freq, pen_ring_noise_r0, pen_ring_noise_r1);
		/* pen data */
...
#if FRAME_DATA_PACKET_DEBUG_PRINT
out:
#endif // FRAME_DATA_PACKET_DEBUG_PRINT
	return;
}
#endif //NVT_PARSE_DATA_ENABLE
...
static irqreturn_t nvt_ts_work_func(int irq, void *data)
{
	...
	if (touch_task == NULL) {
		...
		/*if (set_mi_rq_balance_irq_task(touch_task)) {
		printk("nvt-ts:     set touch_irq_thread as rq balance vip task ");
		}*/
	}
	...
	if (ts->dev_pm_suspend) {
		...
		if (!ret) {
			printk("nvt-ts:     system(bus) can't finished resuming procedure, skip it\n");
			goto XFER_ERROR;
		}
	}
	...
	if (bTouchIsAwake == 0) {
		...
		if (ts->pen_support) {
			...
			nvt_ts_pen_wakeup_gesture_report(pen_format_id);
		}
		goto XFER_ERROR;
	}
	...
	if (((crc_len != XM_HTC_DEFAULT_FINGER_PACKET_LEN && crc_len != XM_HTC_DEFAULT_STYLUS_PACKET_LEN) || (crc_r != ((uint16_t)(~crc))) || (crc_r_len != ~crc_len)) && protocol_type != 0xff ) {
...
#if NVT_PARSE_DATA_ENABLE
	nvt_ts_parse_data_func(frame_data_packet);
#endif //NVT_PARSE_DATA_ENABLE
...
int nvt_enable_touch_raw(int en)
{
	// if (en) {
		...
		add_common_data_to_buf(0, SET_CUR_VALUE, THP_HAL_CHARGING_STATUS, 1, &ts->is_usb_exist);
	// } else {
	// 	//cleanUp(1);
...
static void nvt_init_touchmode_data(void)
{
	...
	/* Touch_Stylus_Enable */
	touch_mode[Touch_Stylus_Enable][GET_MAX_VALUE] = 0x13;
...
static void nvt_ic_switch_mode(u8 gesture_type)
{
	if (gesture_type & GESTURE_DOUBLETAP_EVENT) {
		nvt_set_gesture_mode(ts->gesture_command | 0x01);
	} else {
		nvt_set_gesture_mode(ts->gesture_command & 0xFE);
	}
	if (gesture_type & GESTURE_STYLUS_SINGLETAP_EVENT) {
		nvt_set_gesture_mode(ts->pen_bluetooth_connect ? (ts->gesture_command | 0x02) : (ts->gesture_command & 0xFD));
	} else {
		nvt_set_gesture_mode(ts->gesture_command & 0xFD);
	}
	if (gesture_type & GESTURE_PAD_SINGLETAP_EVENT) {
		nvt_set_gesture_mode(ts->gesture_command | 0x04);
	} else {
		nvt_set_gesture_mode(ts->gesture_command & 0xFB);
	}
}

/* THP scheme this function will not be called */
static void nvt_cmd_mode_update(long mode_update_flag, int mode_value[Touch_Mode_NUM])
{
	int nvt_value;
...
static void nvt_set_cur_value(int mode, int *value)
{
	...
	if (nvt_mode == Touch_Stylus_Enable && ts) {
		...
		nvt_set_gesture_mode((ts->pen_bluetooth_connect && driver_get_touch_mode(TOUCH_ID, Touch_Stylus_Quick_Note_Mode)) ?
	...
	if (nvt_mode == Touch_Stylus_Gamemode_Enable) {
		...
		NVT_LOG("MIPP stylus game list update  whitelist_bak :%d, whitelist: %d ",ts->game_in_whitelist_bak,ts->game_in_whitelist);
		...
		if (ts->game_in_whitelist != ts->game_in_whitelist_bak) {
			ts->game_in_whitelist = ts->game_in_whitelist_bak;
			NVT_LOG("stylus whitelist update  whitelist_bak :%d, whitelist: %d ",ts->game_in_whitelist_bak,ts->game_in_whitelist);
			if (ts->pen_support) {
				mutex_lock(&ts->lock);
				update_pen_status(false);
				mutex_unlock(&ts->lock);
		...
		if (ts->enable_touch_raw && ts->ic_state >= NVT_IC_RESUME_OUT) {
			...
			switch (nvt_value) {
				...
				case 1:
					...
					break;
				default:
					NVT_LOG("THP_LOCK_SCAN_MODE not support value %d", nvt_value);
					break;
			}
			mutex_unlock(&ts->lock);
		}
...
static int nvt_ic_self_test(char *type, int *result)
{
	...
	if (!strncmp("short", type, 5)) {
		ret = nvt_ic_self_test_short();
			...
			xiaomi_touch_mievent_report_int(TOUCH_EVENT_SHORTTEST_FAIL, 0, "TpShortestFail", "novatek", ERROR_REGULATOR_INIT);
#endif
			}
...
#if IS_ENABLED(CONFIG_MIEV)
			xiaomi_touch_mievent_report_int(TOUCH_EVENT_OPENTEST_FAIL, 0, "TpOpentestFail", "novatek", ERROR_REGULATOR_INIT);
#endif
			}
...
/*static int nvt_fw_version_info_read(char *fw_version_buf)
{
	...
	cnt = snprintf(&fw_version_buf[cnt], 64 - cnt, "fw_ver:0x%02X  ", 0x0E);
	cnt += snprintf(&fw_version_buf[cnt], 64 - cnt, "fw_type:0x%02X  ", 0x01);
	cnt += snprintf(&fw_version_buf[cnt], 64 - cnt, "nvt_pid:0x%04X\n", 0x5981);
	cnt = snprintf(&fw_version_buf[cnt], 64 - cnt, "fw_ver:0x%02X  ", ts->fw_ver);
	cnt += snprintf(&fw_version_buf[cnt], 64 - cnt, "fw_type:0x%02X  ", ts->fw_type);
	cnt += snprintf(&fw_version_buf[cnt], 64 - cnt, "nvt_pid:0x%04X\n", ts->nvt_pid);
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
...
#if IS_ENABLED(CONFIG_MIEV)
        	xiaomi_touch_mievent_report_int(TOUCH_EVENT_TRANSFER_ERR, 0, "TpTransferErr", "novatek", ERROR_REGULATOR_INIT);
#endif
		ret = -EINVAL;
	...
	if (ts->pen_support) {
		...
		input_set_abs_params(ts->pen_input_dev, ABS_X, 0, PEN_MAX_WIDTH*SUPER_RESOLUTION_FACOTR - 1, 0, 0);
		input_set_abs_params(ts->pen_input_dev, ABS_Y, 0, PEN_MAX_HEIGHT*SUPER_RESOLUTION_FACOTR - 1, 0, 0);
		...
		ts->pen_input_dev->hint_events_per_packet = 128;
	...
	if (ts->lcd_id_value) {
		...
		memcpy(hardware_param.config_file_name, BOOT_UPDATE_CONFIG_NAME_TM, strlen(BOOT_UPDATE_CONFIG_NAME_TM));
	}
	memset(hardware_param.driver_version, 0, 64);
	memcpy(hardware_param.driver_version, NVT36536_DRIVER_VERSION, strlen(NVT36536_DRIVER_VERSION));
...
/*#if defined(TOUCH_PLATFORM_XRING)
	...
	add_common_data_to_buf(0, SET_LONG_VALUE, Touch_Grip_Mode, 96, (int *)edge_filter_params);
...
//err_register_suspend_resume_failed:
	///NVT_ERR("err_register_suspend_resume_failed.\n");
#if NVT_TOUCH_MP
	nvt_mp_proc_deinit();
...
static void suspend_mode_proc(enum suspend_state state)
{
	...
	add_common_data_to_buf(0, SET_CUR_VALUE, Touch_Suspend, 1, &suspend_status);
}
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	//add_common_data_to_buf(0, SET_CUR_VALUE, THP_HAL_STYLUS_STATIC_STATE, 1, &ts->pen_static_status);
...
#if IS_ENABLED(NVT_MSM_DRM_NOTIFY)
static int nvt_drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
...
static struct of_device_id nvt_match_table[] = {
	{ .compatible = "NVT_xiaomi_touch_TC",},
	{ },
};
#endif
