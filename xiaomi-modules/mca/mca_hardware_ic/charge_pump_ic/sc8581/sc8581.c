...
static int ops_cp_get_pogo_online(int *online, void *chg_dev)
{
	struct sc8581_device *bq = (struct sc8581_device *)chg_dev;
	int ret = 0;
	...
	*online = bq->pogo_plugin;
	//ret = sc8581_get_int_stat(bq, VWPC_PRESENT_STAT, (bool *)online);

	return ret;
}

static int ops_cp_set_pogo_plugin(bool enable, void *chg_dev)
{
	struct sc8581_device *bq = (struct sc8581_device *)chg_dev;
	int ret = 0;

	bq->pogo_plugin = enable;
	mca_log_info("pogo_plugin: %d", bq->pogo_plugin);
	return ret;
}

static int ops_cp_set_keyboard_plugin(bool enable, void *chg_dev)
{
	struct sc8581_device *bq = (struct sc8581_device *)chg_dev;
	int ret = 0, otg_boost_enable = 0, otg_gate_enable = 0;
	bool usb_present, otg_enable;

	bq->keyboard_plugin = enable;
	...
	otg_enable = (bool)(otg_boost_enable && otg_gate_enable);
	mca_log_info("keyboard_plugin: %d, otg_enable:%d", bq->keyboard_plugin, otg_enable);

	if (bq->keyboard_plugin && !bq->dis_keyboard_state) {
		bq->dis_keyboard_state = true;
		sc8581_enable_wpcgate(bq, false);
		if (!otg_enable)
			sc8581_enable_ovpgate(bq, true);
		sc8581_enable_acdrv_manual(bq, true);
		mca_log_info("move manual mode, keyboard_plugin: %d, dis_keyboard_state: %d",
			 bq->keyboard_plugin, bq->dis_keyboard_state);
	} else if (!bq->keyboard_plugin && bq->dis_keyboard_state) {
		if (otg_enable) {
			bq->dis_keyboard_state = false;
			sc8581_enable_wpcgate(bq, true);
			sc8581_enable_acdrv_manual(bq, true);
			mca_log_info("otg: %d, open wpc gate, keyboard_plugin: %d, dis_keyboard_state: %d",
			 otg_enable, bq->keyboard_plugin, bq->dis_keyboard_state);
		} else {
			sc8581_get_int_stat(bq, VUSB_PRESENT_STAT, &usb_present);
			if (!usb_present) {
				bq->dis_keyboard_state = false;
				sc8581_enable_acdrv_manual(bq, false);
				mca_log_info("move auto mode, keyboard_plugin: %d, dis_keyboard_state: %d",
					 bq->keyboard_plugin, bq->dis_keyboard_state);
			}
		}
	}
...
static int ops_cp_get_keyboard_plugin(int *enable, void *chg_dev)
{
	struct sc8581_device *bq = (struct sc8581_device *)chg_dev;
	int ret = 0;

	*enable = bq->keyboard_plugin;
...
static struct platform_class_cp_ops sc8581_chg_ops = {
	...
	.cp_get_pogo_online = ops_cp_get_pogo_online,
	.cp_set_pogo_plugin = ops_cp_set_pogo_plugin,
	.cp_set_keyboard_plugin = ops_cp_set_keyboard_plugin,
	.cp_get_keyboard_plugin = ops_cp_get_keyboard_plugin,
};
...
static void sc8581_irq_handler(struct work_struct *work)
{
	...
	bool usb_present, wpc_present;
	static bool last_usb_present;
	int wls_online = 0, otg_boost_enable = 0, otg_gate_enable = 0;
	...
	if (bq->cp_role == SC8581_MASTER && bq->pogo_support == 1) {
			sc8581_get_int_stat(bq, VWPC_PRESENT_STAT, &wpc_present);
			platform_class_buckchg_ops_get_otg_boost_enable_status(MAIN_BUCK_CHARGER, &otg_boost_enable);
			platform_class_buckchg_ops_get_otg_gate_enable_status(MAIN_BUCK_CHARGER, &otg_gate_enable);
			mca_log_info("cp_charger_int_handle pogo charge, otg_boost_enable: %d, otg_gate_enable:%d\n", otg_boost_enable, otg_gate_enable);
			if (bq->pogo_plugin && !bq->dis_pogo_plugin && usb_present && wpc_present && (!otg_gate_enable || !otg_boost_enable)) {
				bq->dis_pogo_plugin = true;
				sc8581_enable_wpcgate(bq, false);
				sc8581_enable_ovpgate(bq, true);
				sc8581_enable_acdrv_manual(bq, true);
				if (bq->pogo_plugin) {
				...
				mca_log_info("cp_charger_int_handle pogo charge, wpc_present: %d, usb_present:%d, dis_pogo_plugin:%d\n", wpc_present, usb_present, bq->dis_pogo_plugin);
			} else if (!usb_present) {
				if (bq->dis_pogo_plugin)
				{
					msleep(1000);
					sc8581_get_int_stat(bq, VUSB_PRESENT_STAT, &usb_present);
					if(!usb_present) {
						bq->dis_pogo_plugin = false;
						sc8581_enable_wpcgate(bq, true);
						sc8581_enable_ovpgate(bq, true);
						sc8581_enable_acdrv_manual(bq, false);
						if (wpc_present && !bq->keyboard_plugin) {
				...
				} else if (bq->dis_keyboard_state && !bq->keyboard_plugin){
						bq->dis_keyboard_state = false;
						sc8581_enable_acdrv_manual(bq, false);
				}
				mca_log_info("cp_charger_int_handle pogo charge, wpc_present: %d, usb_present:%d, dis_pogo_plugin:%d, dis_keyboard_state:%d, keyboard_plugin:%d\n", wpc_present, usb_present, bq->dis_pogo_plugin, bq->dis_keyboard_state, bq->keyboard_plugin);
			}
...
static int sc8581_parse_dt(struct sc8581_device *bq)
{
	...
	mca_parse_dts_u32(np, "pogo_support", &bq->pogo_support, 0);
