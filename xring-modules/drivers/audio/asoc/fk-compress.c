		    ...
		    struct snd_compr_stream *stream)
{
	...
	AUD_LOG_INFO(AUD_SOC, "sess_id=0x%x,bytes=%zu",
...
static int fk_compr_configure_dsp_for_capture(struct snd_compr_stream *stream)
{
	...
	AUD_LOG_INFO(AUD_SOC, "session_id=0x%x", cprtd->session_id);
		    ...
		    size_t count)
{
	...
	AUD_LOG_INFO(AUD_SOC, "write %zu bytes of buffer to adsp", count);
