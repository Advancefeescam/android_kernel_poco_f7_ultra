...
int32_t conn_mgr_i2c_backlight_on(struct platform_device *pdev, uint32_t id, void *value)
{
	dpu_pr_debug("+\n");
	pipeline_next_ops_handle(pdev, "i2c_backlight_power_on", id, value);
	dpu_pr_debug("-\n");
	return 0;
}
...
static struct ops_handle_table g_priv_handle[] = {
	...
	{"i2c_backlight_power_on", conn_mgr_i2c_backlight_on},
