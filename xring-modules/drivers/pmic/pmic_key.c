...
#include <dt-bindings/xring/common/mdr/include/mdr_pub.h>
...
#define KEY_PRESS_IRQ_INDEX 0
#define PMIC_CLEAR_PWR_KEY_INT 0x0f
#define PMIC_CLEAR_VOL_KEY_INT 0x3f

#define PWRKEY_PRESS_1S_DEBUG_REG 0x1505
#define PWRKEY_PRESS_6S_DEBUG_REG 0x1509
...
static irqreturn_t powerkey_press_hold_1s_handle(int irq, void *data)
{
	...
	regmap_bulk_read(info->regmap, TOP_REG_RTC_TIME_0, &rtc_secs, sizeof(rtc_secs));
	regmap_bulk_write(info->regmap, PWRKEY_PRESS_1S_DEBUG_REG, &rtc_secs, sizeof(rtc_secs));
...
static irqreturn_t powerkey_press_hold_6s_handle(int irq, void *data)
{
	...
	regmap_bulk_read(info->regmap, TOP_REG_RTC_TIME_0, &rtc_secs, sizeof(rtc_secs));
	regmap_bulk_write(info->regmap, PWRKEY_PRESS_6S_DEBUG_REG, &rtc_secs, sizeof(rtc_secs));
...
static int pmic_key_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < info->irq_count; i++) {
		...
		ret = xr_pmic_reg_write(TOP_REG_PMIC_LVL2INT_O1_IRQ_INT_RAW, PMIC_CLEAR_PWR_KEY_INT);
		if (ret)
			dev_err(dev, "failed to clear powerkey int status\n");

		ret = xr_pmic_reg_write(TOP_REG_PMIC_LVL2INT_O2_IRQ_INT_RAW, PMIC_CLEAR_VOL_KEY_INT);
		if (ret)
			dev_err(dev, "failed to clear volumekey int status\n");
