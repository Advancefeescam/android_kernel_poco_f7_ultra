...
static void update_usb_pd_type(struct business_charger *charger)
{
	...
	platform_class_cp_get_pogo_online(CP_ROLE_MASTER, &charger->pogo_online);
	mca_log_info("last_bc12_type = %d, bc12_type = %d, pd_type = %d, pogo_online:%d\n",
		last_type, charger->bc12_type, charger->pd_type, charger->pogo_online);
	...
	if (charger->pogo_online) {
		cur_type = XM_CHARGER_TYPE_POGO;
...
static void business_charger_report_quick_charge_type_work(struct work_struct *work)
{
	...
	} else if (charger->real_type == XM_CHARGER_TYPE_POGO) {
		charger->wired_power_max = 33;
		icon_type = ADP_ICON_TYPE_TURBO;
		mca_log_info("pogo_power_max: %d, icon_type: %d\n", charger->wired_power_max, icon_type);
		business_charger_report_wired_quick_charge_type(charger, icon_type);
	...
	unsigned int event)
{
	...
	platform_class_cp_get_pogo_online(CP_ROLE_MASTER, &charger->pogo_online);
	mca_log_info("online %d, pogo_online:%d\n", charger->bc12_active, charger->pogo_online);

	active_flag = charger->bc12_active || charger->pogo_online;
	if (!active_flag) {
		charger->pd_type = 0;
		charger->real_type = 0;
	...
	unsigned int event, void *data)
{
	...
	mca_strategy_func_process(STRATEGY_FUNC_TYPE_CHARGE_VERIFY_PROCESS_END, event, !verify_process_end);
...
static void business_charger_process_pogo_ibus_change(struct business_charger *charger,
	unsigned int event, void *data)
{
	int pogo_ibus = *((int *)data);

	mca_log_info("pogo_ibus: %d\n", pogo_ibus);
	mca_strategy_func_process(STRATEGY_FUNC_TYPE_QUICK_CHARGE, event, pogo_ibus);
	mca_strategy_func_process(STRATEGY_FUNC_TYPE_BUCK_CHARGE, event, pogo_ibus);
	...
	while (!list_empty(&charger->header)) {
		...
		list_for_each_entry_safe(event_node, temp_node, &charger->header, lnode) {
			...
			case MCA_EVENT_IBAT_OCP_CHANGE:
				...
				break;
			case MCA_EVENT_POGO_IBUS_CHANGE:
				business_charger_process_pogo_ibus_change(charger, event_node->event, event_node->data);
				break;
			case MCA_EVENT_CSD_SEND_PULSE:
