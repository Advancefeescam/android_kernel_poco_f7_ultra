...
extern struct mca_xring_usb *ppm_global;
...
static void usb_dwork_handler(struct work_struct *work)
{
	...
	struct mca_xring_usb *mca_xr_usb = ppm_global;
	...
	case DR_MAX:
		...
		stop_usb_host(ppm);
		if (NULL != mca_xr_usb) {
			mca_xr_usb->usb_dr = usb_dr;
			ret = xr_usb_queue_off(mca_xr_usb->xr_usb);
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			platform_class_buckchg_set_typec_orientation_ready(MAIN_BUCK_CHARGER, true);
			/*
			...
			platform_class_buckchg_set_typec_orientation_ready(MAIN_BUCK_CHARGER, false);
			/*
	...
	case TCP_NOTIFY_DR_SWAP:
		...
		if (noti->swap_state.new_role == PD_ROLE_UFP) {
			 ...
			 */
			if (ppm->dp_ready) {
				tcpm_dpm_vdm_discover_id(ppm->tcpc, NULL);
				mca_log_info("process DP events\n");
			}
			cancel_delayed_work_sync(&ppm->usb_dwork);
...
static int mca_dp_connect_process_event(int event, int value, void *data)
{
	struct protocol_pd_manager *ppm = data;
	int ret = 0;

	if (ppm == NULL)
		return -EINVAL;

	mca_log_info("%s:receive event %d, value %d\n", __func__, event, value);
	switch (event) {
...
static int protocol_pd_manager_probe(struct platform_device *pdev)
{
	...
	(void)mca_strategy_ops_register(STRATEGY_FUNC_TYPE_CHARGE_VERIFY_PROCESS_END, mca_dp_connect_process_event, NULL, NULL, ppm);
