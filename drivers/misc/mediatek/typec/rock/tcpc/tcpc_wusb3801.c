...
/*L19AT code for HQ-248674 by zhaohan at 2022/11/14 start*/
	msleep(100);
/*L19CT code for HQ-278972 by zhaohan at 2023/1/31 start*/
	ret = wusb3801_i2c_read8(chip->tcpc, WUSB3801_REG_INTERRUPT);
	dev_err(cdev, "%s 03 ret = [0x%02x] \n", __func__, ret);
	wusb3801_i2c_write8(chip->tcpc,
			WUSB3801_REG_CONTROL0, 0x24);
/*L19CT code for HQ-278972 by zhaohan at 2023/1/31 end*/
/*L19AT code for HQ-248674 by zhaohan at 2022/11/14 end*/
...
/*L19AT code for HQ-248674 by zhaohan at 2022/11/14 start*/
	if (int_sts & WUSB3801_INT_DETACH) {	
		...
		pr_err("%s: mtk_usb_extcon_set_role  USB_ROLE_NONE\n", __func__);
				...
				const struct i2c_device_id *id)
{
	...
	pr_err("mzc:chip_id=%d\n",chip_id);
	if (chip_id < 0) {
		chip_id = wusb3801_check_revision(client);
		if (chip_id < 0)
			return chip_id;
	}
pr_err("mzc:chip_id2=%d\n",chip_id);
