...
int slg_get_cycle_count(struct auth_device *auth_dev, u32* cycle_count)
{
	...
	if (get_count >= 29) {
		ret = slg_dc_get();
		if (ret < 0) {
			pr_err("%s read dc value failed: %d\n", __func__, ret);
			return ret;
		}
...
static int slg_set_ui_soh(struct auth_device *auth_dev, u8 *ui_soh_data, int len)
{
	int ret = 0, length = 0, i = 0;
	unsigned char buf[128];
...
static int slg_get_raw_soh(struct auth_device *auth_dev, int *raw_soh)
{
	int ret = 0, length = 0;
	uint8_t page_data[32];
	...
	pr_info("%s raw_soh: %d, len: %d\n", __func__, *raw_soh, length);
