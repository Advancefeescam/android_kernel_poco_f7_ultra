...
static struct imgsensor_info_struct imgsensor_info = {
	...
	// Dammin add custom2 for SW Remosaic.
	.custom2 = {
		.pclk = 560000000,
		.linelength = 8688,
		.framelength = 6400,
...
//		mdelay(5);
		...
		for (i = 0; i < 5; i++) {
			pr_err("main %s streaming check is %d", __func__,
...
/*L19A code for HQ-221460 by Zhangxiaodong at: Thu, 07 Jul 2022 start*/
...
static void set_shutter(kal_uint32 shutter)
{
    ...
    if (shutter > 0xFFF0) {
	    ...
	    LOG_INF("download  enter  shutter = %d\n", shutter);
    }else {
...
/*L19A code for HQ-221460 by Zhangxiaodong at: Thu, 07 Jul 2022 end*/
...
#define XTALK_DATA_SIZE	8006
#define XTALK_DATA_ADDR	0x1C94
#define HW_GGC_SIZE	346
#define HW_GGC_ADDR	0x3BDA
#define QSC_FLAG_ADDR	0x1C92
#define QSC_FLAG_VALID	0x01
#define CHECKSUM_ADDR	0x3D34
#define EEPROM_SLAVE_ID	0xA2
static char XTALK_DATA[XTALK_DATA_SIZE + 2];
static char HW_GGC_DATA[HW_GGC_SIZE];
static kal_bool is_read_cali_data = KAL_FALSE;

static void read_four_cell_data_from_eeprom(char *data)
{
        const kal_uint16 type_addr = 0x1C93;    // ofilm vendor-specific data, need by checksum.
        char is_samsung_sensor  = 0;
	int i;
	...
	char pu_send_cmd[2] = { };
        ...
        if (is_read_cali_data) {
		LOG_INF("no need to read calibration data from EEPROM.");
                if (data != NULL) {
                        memcpy(data, XTALK_DATA, XTALK_DATA_SIZE + 2);
                        LOG_INF("Already return xtalk data.\n");
	        }
                return;
        }

        pu_send_cmd[0] = (char)(QSC_FLAG_ADDR >> 8);
        pu_send_cmd[1] = (char)(QSC_FLAG_ADDR & 0xFF);
        iReadRegI2C(pu_send_cmd, 2, &calibration_flag, 1, EEPROM_SLAVE_ID);

        if (calibration_flag != QSC_FLAG_VALID) {
                LOG_INF("EEPROM data err: [0x%x] expect: 0x%x but: 0x%x",
                        QSC_FLAG_ADDR, QSC_FLAG_VALID, calibration_flag);
                LOG_INF("EEPROM data err: invalid calibration data !");
                return;
        }

        pu_send_cmd[0] = (char)(type_addr >> 8);
        pu_send_cmd[1] = (char)(type_addr & 0xFF);
        iReadRegI2C(pu_send_cmd, 2, &is_samsung_sensor, 1, EEPROM_SLAVE_ID);
        if (is_samsung_sensor != 0x01) {
                LOG_INF("EEPROM data err: [0x%x] expect: 0x01 but: 0x%x",
                        type_addr, is_samsung_sensor);
                LOG_INF("EEPROM data err: invalid type number !");
                return;
        }
        checksum_cal += is_samsung_sensor;

        XTALK_DATA[0] = (XTALK_DATA_SIZE & 0xFF);
        XTALK_DATA[1] = ((XTALK_DATA_SIZE >> 8) & 0xFF);

        for (i = 0; i < XTALK_DATA_SIZE; i++) {
                pu_send_cmd[0] = (char)((XTALK_DATA_ADDR + i) >> 8);
                pu_send_cmd[1] = (char)((XTALK_DATA_ADDR + i) & 0xFF);
                iReadRegI2C(pu_send_cmd, 2, &XTALK_DATA[i+2], 1, EEPROM_SLAVE_ID);
                checksum_cal += XTALK_DATA[i+2];
        }

        for (i = 0; i < HW_GGC_SIZE; i++) {
                pu_send_cmd[0] = (char)((HW_GGC_ADDR + i) >> 8);
                pu_send_cmd[1] = (char)((HW_GGC_ADDR + i) & 0xFF);
                iReadRegI2C(pu_send_cmd, 2, &HW_GGC_DATA[i], 1, EEPROM_SLAVE_ID);
                checksum_cal += HW_GGC_DATA[i];
        }

        checksum_cal = checksum_cal % 0xFF + 1;

        pu_send_cmd[0] = (char)(CHECKSUM_ADDR >> 8);
        pu_send_cmd[1] = (char)(CHECKSUM_ADDR & 0xFF);
        iReadRegI2C(pu_send_cmd, 2, &checksum_read, 1, EEPROM_SLAVE_ID);

        if (checksum_read != checksum_cal) {
                LOG_INF("EEPROM data err: checksum_read: 0x%x checksum_cal: 0x%x",
                        checksum_read, checksum_cal);
                LOG_INF("EEPROM data err: calibration data checksum err!");
                for (i = 0; i < XTALK_DATA_SIZE; i++)
                        LOG_INF("cali_data dump: [0x%x]: 0x%02x", XTALK_DATA_ADDR + i, XTALK_DATA[i+2]);
                for (i = 0; i < HW_GGC_SIZE; i++)
                        LOG_INF("cali_data dump: [0x%x]: 0x%02x", HW_GGC_ADDR + i, HW_GGC_DATA[i]);
                return;
        }

        is_read_cali_data = KAL_TRUE;
        LOG_INF("EEPROM calibration data read success!");
}

static void hw_ggc_setting(void)
{
        int i;
        if (is_read_cali_data != KAL_TRUE) {
                LOG_INF("hw_ggc_setting err: invalid calibration data !");
                return;
        }

        write_cmos_sensor(0x6028, 0x2400);
        write_cmos_sensor(0x602A, 0x0CFC);
        for (i = 0; i < HW_GGC_SIZE; i+=2)
                write_cmos_sensor(0x6F12, ((kal_uint16)HW_GGC_DATA[i])<<8 | HW_GGC_DATA[i+1]);

#if 0
        write_cmos_sensor(0x602C, 0x2400);
        write_cmos_sensor(0x602E, 0x0CFC);
        for (i = 0; i < HW_GGC_SIZE; i++) {
                LOG_INF("hw_ggc_setting [%d] HW_GGC[0x%02x] Read[0x%02x]",
                        i, HW_GGC_DATA[i], read_cmos_sensor_8(0x6F12));
        }
#endif
}
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_custom2_s5kjn1ofilm[] = {
	0x6028,0x2400,
	0x602A,0x1A28,
	0x6F12,0x4C00,
	0x602A,0x065A,
...
static void custom2_setting(void)
{
	LOG_INF("E\n");
	s5kjn1_multi_write_cmos_sensor(
		addr_data_pair_custom2_s5kjn1ofilm,
		sizeof(addr_data_pair_custom2_s5kjn1ofilm) / sizeof(kal_uint16));
}
			...
			*sensor_id = return_sensor_id() + 1;
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
...
#else
				{
					...
					/* preload 4cell data */
					read_four_cell_data_from_eeprom(NULL);
					return ERROR_NONE;
				}
			}
...
custom2(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *
		image_window, MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_INF("Setting custom2 for SW Remosaic.\n");
	spin_lock(&imgsensor_drv_lock);
...
/*L19A code for HQ-212412 by luling at 2022/6/13 start */
	0x6028, 0x4000,
	0x0600, 0x0001, //0x0001-test pattern solid color,0x0000-no pattern
	0x0602, 0x0000, //[9:0] greenR
	0x0604, 0x0000, //[9:0] red
	0x0606, 0x0000, //[9:0] blue
	0x0608, 0x0000, //[9:0] greenB
				...
				UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
		case MSDK_SCENARIO_ID_CUSTOM2:		/*full size*/
...
//cxc long exposure <
	...
	case SENSOR_FEATURE_GET_4CELL_DATA: {
		...
		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			...
			read_four_cell_data_from_eeprom(data);
