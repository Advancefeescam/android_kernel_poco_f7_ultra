...
int tcpci_get_mode(struct tcpc_device *tcpc)
{
	int ret;
	int mode;

	PD_BUG_ON(tcpc->ops->get_mode == NULL);

	ret = tcpc->ops->get_mode(tcpc, &mode);
	if (ret)
		return ret;

	tcpc->typec_mode = mode;

	return 0;
}
EXPORT_SYMBOL(tcpci_get_mode);
