...
#include "dt-bindings/xring/common/clk/include/clk_resource.h"
...
static int xring_o1_clocks_probe(struct platform_device *pdev)
{
	...
	*clk_hw_data = devm_kzalloc(dev, struct_size(*clk_hw_data, hws, XR_CLK_END), GFP_KERNEL);
	...
	g_hws[XR_CLK_PPLL0] = devm_xr_clk_hw_ppll(dev, "clk_ppll0", "clk_ppll0_fake", &ppll0_cfg,
		ppll0_rate_table, ARRAY_SIZE(ppll0_rate_table), 0,
		PLL_FLAGS_FIXED_RATE|PLL_FLAGS_DOZE_VOTE, VOTER_DOZAP_PPLL0);

	ppll1_cfg.base = base;
	g_hws[XR_CLK_PPLL1] = devm_xr_clk_hw_ppll(dev, "clk_ppll1", "clk_ppll1_fake", &ppll1_cfg,
		ppll1_rate_table, ARRAY_SIZE(ppll1_rate_table), 0,
		PLL_FLAGS_FIXED_RATE|PLL_FLAGS_DOZE_VOTE, VOTER_DOZAP_PPLL1);

	ppll2_cfg.base = base;
	g_hws[XR_CLK_PPLL2] = devm_xr_clk_hw_ppll(dev, "clk_ppll2", "clk_ppll2_fake", &ppll2_cfg,
		ppll2_rate_table, ARRAY_SIZE(ppll2_rate_table), 0,
		PLL_FLAGS_FIXED_RATE|PLL_FLAGS_DOZE_VOTE, VOTER_DOZAP_PPLL2);

	ppll2_dpu_cfg.base = base;
	g_hws[XR_CLK_PPLL2_DPU] = devm_xr_clk_hw_ppll(dev, "clk_ppll2_dpu", "clk_sys_peri",
		&ppll2_dpu_cfg, ppll2_rate_table, ARRAY_SIZE(ppll2_rate_table), 0,
		PLL_FLAGS_FIXED_RATE|PLL_FLAGS_DOZE_VOTE, VOTER_DOZAP_PPLL2_DPU);

	ppll3_cfg.base = base;
	g_hws[XR_CLK_PPLL3] = devm_xr_clk_hw_ppll(dev, "clk_ppll3", "clk_ppll3_fake", &ppll3_cfg,
		ppll3_rate_table, ARRAY_SIZE(ppll3_rate_table), 0,
		PLL_FLAGS_FIXED_RATE|PLL_FLAGS_DOZE_VOTE, VOTER_DOZAP_PPLL3);
	...
	g_hws[XR_CLK_PCIE_PLL] = devm_xr_clk_hw_ppll(dev, "clk_pcie_pll", "clk_pcie_pll_logic",
		...
		SAFE_PLL_PCIE_PLL, PLL_FLAGS_FIXED_RATE | PLL_FLAGS_SOFT_CTRL, 0);
...
/* ======== Generated Register Code Start,Do not modify by hand! ======== */
	...
	g_hws[XR_CLK_DPU_PLL] = devm_xr_clk_hw_ppll(dev, "clk_dpu_pll", "clkin_sys",
		&dpu_pll_cfg, NULL, 0, SAFE_PLL_DPU_PLL, PLL_FLAGS_DYNAMIC_RATE, 0);

	dp_pll_cfg.base = base;
	g_hws[XR_CLK_DP_PLL] = devm_xr_clk_hw_ppll(dev, "clk_dp_pll", "clkin_sys",
		&dp_pll_cfg, NULL, 0, SAFE_PLL_DP_PLL, PLL_FLAGS_DYNAMIC_RATE, 0);
