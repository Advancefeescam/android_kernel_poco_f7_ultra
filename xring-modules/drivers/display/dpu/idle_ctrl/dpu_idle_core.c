...
bool is_idle_enabled(void)
{
	...
	if (idle_ctx->pinfo && (idle_ctx->pinfo->lowpower_ctrl & DPU_LP_IDLE_CTRL_ENABLE) &&
...
void dpu_idle_fps_update(u32 fps)
{
	struct dpu_idle_ctx *idle_ctx = dpu_get_idle_ctx();

	if (!is_idle_enabled())
		return;
	if (dsi_connector_is_cmd_mode(idle_ctx->conn))
		return;

	mutex_lock(&idle_ctx->idle_lock);
	if ((idle_ctx->cur_fps == fps) || (fps == 0))
		goto exit;

	idle_ctx->cur_fps = fps;
	IDLE_DEBUG("current fps:%u\n", idle_ctx->cur_fps);

exit:
	mutex_unlock(&idle_ctx->idle_lock);
}

static u32 dpu_idle_expire_count_get(void)
{
	struct dpu_idle_ctx *idle_ctx = dpu_get_idle_ctx();
	u32 vsync_period;
	u32 expire_count;
	...
	if (dsi_connector_is_cmd_mode(idle_ctx->conn) ||
			(idle_ctx->pinfo->idle_enter_time == 0) || (idle_ctx->cur_fps == 0))
		return DEFAULT_IDLE_EXPIRE_FRAME_NUM;

	vsync_period = 1000 / idle_ctx->cur_fps;
	expire_count = idle_ctx->pinfo->idle_enter_time / vsync_period;
	if (expire_count < DEFAULT_IDLE_EXPIRE_FRAME_NUM)
		expire_count = DEFAULT_IDLE_EXPIRE_FRAME_NUM;
	IDLE_DEBUG("expire_count:%u, fps:%u vsync:%u\n", expire_count, idle_ctx->cur_fps, vsync_period);
	return expire_count;
}
...
static void dpu_idle_restart(struct dpu_idle_ctx *idle_ctx)
{
	...
	if (idle_ctx->active)
		idle_ctx->expire_count = dpu_idle_expire_count_get();
...
void dpu_idle_vote_restart(enum idle_vote_ch vote_ch)
{
	...
	if (idle_ctx->need_restore_clk && vote_ch == PRESENT_CH) {
		...
		dpu_hw_dvfs_enable(&idle_ctx->dpu_kms->core_perf->hw_dvfs_mgr);
...
int dpu_idle_ctx_init(struct dpu_kms *dpu_kms)
{
	...
	init_waitqueue_head(&idle_ctx->monitor_wq);
