...
int enter_factory_mode(void)
{
    ...
    if (ts_data->fw_loading) {
       FTS_INFO("fw upgrade in process, can't enter factory mode");
       return -EIO;
   }
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    if (ts_data->fw_loading) {
        FTS_INFO("fw upgrade in process, can't test");
        return 0;
    }
    /* N6 code for HQ-330297 by liaoxianguo at 2023/9/20 end */
...
static int fts_ito_test_show(struct seq_file *file, void *data)
{
    ...
    if ((fts_ftest->test_item[FTS_ENTER_FACTORY_MODE].testresult) == RESULT_PASS)
        ...
        seq_printf(file, "0F-1F-2F-3F-4F-5F\n");
        return 0;
    }
    //Rawdata result
...
//P6 code for HQFEAT-109228 by p-zhaobeidou3 at 2025/07/11 start
    //reset test result
    if (fts_ftest->test_item[FTS_RESET_TEST].testresult == RESULT_PASS)
        test_result_bmp[RESET_DATA] = 'P';
    else
        test_result_bmp[RESET_DATA] = 'F';
    seq_printf(file, "%s\n", fts_ftest->csv_file_buf);
    seq_printf(file, "0%c-1%c-2%c-3%c-4%c-5%c\n",
...
static int fts_ito_test_open(struct inode *inode, struct file *file)
{
    ...
    /*O6 code for HQ-437772 by liaoxianguo at 2024/1/17 start*/
    sprintf(fwname, "focaltech_mp_fw_tianma.ini");
    /*O6 code for HQ-437772 by liaoxianguo at 2024/1/17 end*/
