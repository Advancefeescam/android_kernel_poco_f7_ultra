...
{
	uint8_t value;
	int exponential_bl = backlight_level;
	int brightness = 0;
	...
	if(project == EFI_PROJECTINFO_TYPE_O81A) {
		if (exponential_bl <= BL_LEVEL_MAX) {
			...
			dpu_pr_err("ktz8866 backlight out of 4095 too large!!!\n");
		}
		...
		if (brightness < 0 || brightness > BL_LEVEL_MAX) {
			dpu_pr_err("ktz8866 backlight out of 2047 too large!!!\n");
		}

		if (brightness > 0) {
			ktz8866_i2c_write(KTZ8866_DISP_BL_ENABLE, 0x7f);
			dpu_pr_debug("ktz8866 backlight enable,dimming close");
		} else if (brightness == 0) {
			ktz8866_i2c_write(KTZ8866_DISP_BL_ENABLE, 0x3f);
			dpu_pr_debug("ktz8866 backlight disable,dimming close");
		}
		v[0] = (brightness >> 3) & 0xff;
		v[1] = brightness & 0x7;
		dpu_pr_info("ktz8866 get backlight_level: %d, exponential_bl: %d, reg: %d ,MSB: 0x%02x, LSB: 0x%02x\n",
				backlight_level, exponential_bl, brightness, v[0], v[1]);
		ktz8866_i2c_write(KTZ8866_DISP_BB_LSB, v[1]);
		ktz8866_i2c_write(KTZ8866_DISP_BB_MSB, v[0]);
	} else {
...
void ktz8866_power_on(void)
{
	...
	if(project == EFI_PROJECTINFO_TYPE_O81A) {
		ktz8866_i2c_write(KTZ8866_DISP_BC1, 0x93);/* Backlight OVP 23.9V, exponential mapping, enable PWM*/
