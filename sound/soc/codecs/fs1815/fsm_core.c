...
ssize_t fsm_get_reg(char *buf)
{
	...
	list_for_each_entry(fsm_dev, &fsm_dev_list, list) {
		...
		for (reg_addr = 0; reg_addr <= reg_end; reg_addr++) {
			if(fsm_dev->is1820){
				if(reg_addr == 0x1A) reg_addr = 0x1F;
				else if(reg_addr == 0x24) reg_addr = 0x29;
				else if(reg_addr == 0x31) reg_addr = 0x3E;
				else if(reg_addr == 0x40) reg_addr = 0x4C;
				else if(reg_addr == 0x4F) reg_addr = 0x58;
				else if(reg_addr == 0x5D) reg_addr = 0x80;
				else if(reg_addr == 0x9F) reg_addr = 0xA9;
				else if(reg_addr == 0xAE) reg_addr = 0xB8;
				else if(reg_addr == 0xBD) reg_addr = 0xC0;
			}
