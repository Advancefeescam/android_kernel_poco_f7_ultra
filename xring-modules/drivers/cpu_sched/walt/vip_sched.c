...
static u64 g_wait_time_thres = 50000000;
static u64 g_vip_wait_time_thres = 3000000;
...
int find_vip_preempt_cpu(const cpumask_t *mask)
{
	int cpu = -1;
	struct vip_rq *vrq;
	...
	for_each_cpu(cpu, mask) {
		...
		if (vrq->vip_preempt != VIP_PREEMPT_NONE)
			return cpu;
	}
...
#define VIP_GRACE_PERIOD	(1000000)
static u64 get_task_wait_time(struct rq *rq, struct task_struct *p, u64 now)
{
	u64 wait_time = 0, last_ts;
	struct walt_task_struct *wts = (struct walt_task_struct *)p->android_vendor_data1;
	struct vip_rq *vrq = &per_cpu(vip_rq, cpu_of(rq));

	/*
	 * Normal task whose wait_time exceed threshold would
	 * obtain a *1ms* grace period for avoid preempted by
	 * vip tasks
	 */
	if (p == rq->curr && now - vrq->last_grace_ts > VIP_GRACE_PERIOD)
		return 0;

	last_ts = max(wts->last_wake_ts, wts->last_preempt_ts);
	wait_time = now > last_ts ? now - last_ts : 0;
		...
		struct sched_entity **se, bool *repick, bool simple, struct task_struct *prev)
{
	struct vip_rq *vrq = &per_cpu(vip_rq, cpu_of(rq));
	struct vip_task *vip;
	struct task_struct *task, *task_orig;
	struct cfs_rq *cfs_rq;
	u64 wait_time = 0, vip_wait_time = 0, now;
	...
	if (task && task->nr_cpus_allowed == 1) {
		trace_vip_replace_next_task(task, 0, task->pid, 0);
		return;
	}
	...
	now = rq_clock(rq);
	if (task_orig != NULL)
		wait_time = get_task_wait_time(rq, task_orig, now);

	vip_wait_time = get_task_wait_time(rq, task, now);

	if ((wait_time > g_wait_time_thres) &&
			(vip_wait_time < g_vip_wait_time_thres)) {
		if (rq->curr != task_orig)
			vrq->last_grace_ts = now;

		trace_vip_replace_next_task(task_orig, 0, task_orig->pid, wait_time);
		return;
	}
	...
	*repick = true;
	...
	trace_vip_replace_next_task(task, vip->prio, task_orig ? task_orig->pid : -1,
			wait_time);
}
...
struct vip_wait_cfg {
	u64 wait_time_thres;
	u64 vip_wait_time_thres;
};

#define CMD_SET_VIP_PRIO		PERF_W_CMD(SET_VIP_PRIO, struct vip_io_cfg)
#define CMD_SET_VIP_WAIT_THRES	PERF_W_CMD(SET_VIP_WAIT_THRES, struct vip_wait_cfg)
...
DEFINE_MUTEX(vip_update_lock);
int perf_actuator_set_vip_wait_thres(void __user *uarg)
{
	struct vip_wait_cfg cfg;

	if (copy_from_user(&cfg, uarg, sizeof(struct vip_wait_cfg)))
		return -EFAULT;

	mutex_lock(&vip_update_lock);

	g_wait_time_thres = cfg.wait_time_thres;
	g_vip_wait_time_thres = cfg.vip_wait_time_thres;

	mutex_unlock(&vip_update_lock);
...
int vip_sched_init(void)
{
	...
	register_perf_actuator(CMD_SET_VIP_WAIT_THRES, perf_actuator_set_vip_wait_thres);
	return 0;
}
