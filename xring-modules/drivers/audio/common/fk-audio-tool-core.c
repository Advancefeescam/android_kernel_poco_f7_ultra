	...
	struct _pipeline_context_t pipeline_t, int cal_id)
{
	...
	AUD_LOG_DBG(AUD_COMM, "user data addr: 0x%llx, len: %d",
		(uint64_t)user_data_addr, len);
	if (len && len < CAL_SIZE_MAX) {
		if (!user_data_addr) {
		...
		if (m_diff > 100)
			AUD_LOG_INFO(AUD_COMM, "alloc cal timeout, m_diff:%lums.", m_diff);
		if (!cal_vaddr) {
			AUD_LOG_ERR(AUD_COMM, "alloc cal failure.");
	...
	struct cal_param_info *cal_info, int tool_type)
{
	...
	AUD_LOG_INFO(AUD_COMM, "sence:%d, port%d, size:%hu",
	...
	size_t data_size, void *data)
{
	...
	if ((dsp_log_info->param.log_level == pdata->log_param.param.log_level) &&
		(dsp_log_info->param.log_port == pdata->log_param.param.log_port)) {
		AUD_LOG_INFO(AUD_COMM, "same params.");
		return 0;
	}
	...
	AUD_LOG_INFO(AUD_COMM, "level%d, port%d, enable:%d",
		dsp_log_info->param.log_level, dsp_log_info->param.log_port,
		dsp_log_info->param.log_enable);
	...
	memcpy(&pdata->log_param.param, &dsp_log_info->param,
		sizeof(struct dsp_log_dump_param_user));
	...
	size_t data_size, void *data)
{
	...
	if ((pdata->pcm_param.param.point1 == dsp_pcm_info->param.point1) &&
		(pdata->pcm_param.param.point2 == dsp_pcm_info->param.point2) &&
		(pdata->pcm_param.param.point3 == dsp_pcm_info->param.point3)) {
		AUD_LOG_INFO(AUD_COMM, "same param.");
		return 0;
	}

	/* the open and close cmd of pcm dump must in pairs */
	if ((pdata->pcm_param.param.point1 || pdata->pcm_param.param.point2 ||
		pdata->pcm_param.param.point3)) {
		if ((dsp_pcm_info->param.point1 || dsp_pcm_info->param.point2 ||
		dsp_pcm_info->param.point3)) {
			AUD_LOG_ERR(AUD_COMM, "dsp pcm dump has opened, the new open cmd must be."
				"performed after a close cmd");
			return -EFAULT;
		}
	}
	...
	memcpy(&pdata->pcm_param, dsp_pcm_info, sizeof(struct dsp_pcm_dump_param_info));
	AUD_LOG_INFO(AUD_COMM, "point1:%x point2:%x point3:%x size:%d",
		dsp_pcm_info->param.point1, dsp_pcm_info->param.point2,
		dsp_pcm_info->param.point3, dsp_pcm_info->blk_size);
...
/************************************************************************
 * function: fk_audio_tool_reg_check
 * description: dsp reg check
 * parameter:
 *		  reg: reg
 * return:
 *		    0: valid
 *		other: invalid
 ************************************************************************/
static int fk_audio_tool_reg_check(uint32_t reg)
{
	if (reg >= ADSP_SYS_MEM_START_0 && reg <= ADSP_SYS_MEM_END_0)
		return 0;
	else if (reg >= ADSP_SYS_MEM_START_1 && reg <= ADSP_SYS_MEM_END_1)
		return 0;
	else if (reg >= ADSP_SYS_MEM_START_2 && reg <= ADSP_SYS_MEM_END_2)
		return 0;
	else if (reg >= ADSP_SYS_MEM_START_3 && reg <= ADSP_SYS_MEM_END_3)
		return 0;
	else
		return -EFAULT;
}
	...
	size_t data_size, void *data)
{
	...
	ret = fk_audio_tool_reg_check(read_reg_info.reg);
	if (ret) {
		AUD_LOG_ERR(AUD_COMM, "Invalid reg:0x%x.", read_reg_info.reg);
		return -EFAULT;
	}
	...
	AUD_LOG_INFO(AUD_COMM, "reg: %d, num: %d", dsp_reg_info->reg_addr,
		dsp_reg_info->num);
	...
	size_t data_size, void *data)
{
	...
	ret = fk_audio_tool_reg_check(write_reg_info.reg);
	if (ret) {
		AUD_LOG_ERR(AUD_COMM, "Invalid reg:0x%x.", write_reg_info.reg);
		return -EFAULT;
	}
	...
	AUD_LOG_INFO(AUD_COMM, "reg: %d, value: %d", dsp_reg_info->reg_addr,
		dsp_reg_info->value);
...
int fk_audio_tool_close_msg_send(void)
{
	...
	if (pdata->pcm_blk_malloc_flag) {
		...
		pdata->pcm_blk_malloc_flag = 0;
		/* init pcm params */
		memset(&pdata->pcm_param.param, 0, sizeof(struct dsp_pcm_dump_param_user));
	}
	...
	size_t data_size, void *data)
{
	...
	if (pdata->uncache_log_en_flag == en_flag) {
		AUD_LOG_INFO(AUD_COMM, "same flag:%d.", en_flag);
		return 0;
	}

	if (pdata->uncache_log_en_flag) {
		AUD_LOG_INFO(AUD_COMM, "already enable, flag:%d.", pdata->uncache_log_en_flag);
		return 0;
	}
	...
	(struct xring_aud_msg *tool_msg)
{
	...
	AUD_LOG_INFO(AUD_COMM, "type:%d, p_len:%d, c_len:%d",
		log_param->log_type, log_param->point_len, log_param->char_len);
...
/************************************************************************
 * function: fk_audio_pdata_init
 * description: log & pcm params of pdata init.
 * parameter:
 * return:
 ************************************************************************/
void fk_audio_pdata_init(void)
{
	struct audio_tool_pdata *pdata = fk_audio_tool_pdata_get();

	AUD_LOG_INFO(AUD_COMM, "log & pcm params of pdata init");

	/* init log params */
	memset(&pdata->log_param.param, 0, sizeof(struct dsp_log_dump_param_user));
	/* init pcm params */
	memset(&pdata->pcm_param.param, 0, sizeof(struct dsp_pcm_dump_param_user));
	/* init uncache log params */
