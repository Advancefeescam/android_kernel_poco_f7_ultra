...
void dsi_panel_i2c_power_on(struct dsi_panel *panel)
{
	...
	if (panel_info->is_fake_panel) {
		DSI_INFO("bypass the backlight on operaction for fake panel\n");
		return;
	}

	DSI_DEBUG("i2c power on\n");
	if (panel_info->bl_setting_type != DSI_PANEL_BL_UPDATE_METHOD_I2C)
		return;

#ifdef MI_DISPLAY_MODIFY
	if (mi_get_panel_id(mi_cfg->mi_panel_id) == O81A_PANEL_PA)
		ktz8866_power_on_except_vsp_vsn();
	else
		ktz8866_power_on();
#else
	ktz8866_power_on();
...
static void dsi_panel_i2c_power_on_work(struct work_struct *work)
{
	struct dsi_panel *panel;

	panel = container_of(to_delayed_work(work),
			struct dsi_panel, i2c_power_on_work);
	if (unlikely(!panel)) {
		DSI_ERROR("failed to get panel\n");
		return;
	}
	...
	if (!dsi_panel_initialized(panel)) {
		DSI_INFO("panel is uninitialized, skip i2c power on\n");
		dsi_panel_unlock(panel);
		return;
	}

	dsi_panel_i2c_power_on(panel);
	dsi_panel_unlock(panel);
}
		...
		struct dsi_panel_mode *new_mode)
{
	...
	dpu_idle_fps_update(new_mode->timing.framerate);
	DPU_DEBUG("update panel mode to %u fps\n", new_mode->timing.framerate);
}
...
#ifdef MI_DISPLAY_MODIFY
		if (mi_get_panel_id(mi_cfg->mi_panel_id) == O81A_PANEL_PA)
			ret = ktz8866_backlight_update_status_o81a(brightness);
		else
#endif
		...
		struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "dsi-panel-idle-enter-time-ms",
		&panel_info->idle_enter_time);
	if (ret) {
		panel_info->idle_enter_time = 0;
		DSI_DEBUG("set dsi panel idle enter time as default: 0 ms\n");
	} else {
		DSI_DEBUG("get idle_enter_time: 0x%x\n", panel_info->idle_enter_time);
	}
...
const char *cmd_set_update_names[DSI_CMD_SET_MAX] = {
	...
	"mi,mdss-dsi-cabcuion-command-update",
	"mi,mdss-dsi-cabcstillon-command-update",
	"mi,mdss-dsi-cabcmovieon-command-update",
	"mi,mdss-dsi-cabcoff-command-update",
...
#ifdef MI_DISPLAY_MODIFY
	if (!mi_cfg->is_tddi_flag || (mi_cfg->is_tddi_flag && (!mi_cfg->tddi_doubleclick_flag || mi_cfg->panel_dead_flag))) {
		if (mi_cfg->panel_dead_flag)
		...
		if (panel_info->bl_setting_type == DSI_PANEL_BL_UPDATE_METHOD_I2C) {
#ifdef MI_DISPLAY_MODIFY
			/* the o81a i2c backlight will enabled in later */
			if (mi_get_panel_id(mi_cfg->mi_panel_id) != O81A_PANEL_PA)
				ktz8866_power_on();
#else
			ktz8866_power_on();
...
#ifdef MI_DISPLAY_MODIFY
	} else if (mi_cfg->is_tddi_flag && mi_cfg->tddi_doubleclick_flag) {
		DSI_INFO("mi tddi doubleclick power on keep power on\n");
	}
#endif
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	/* cancel i2c power on work before power off */
	if (panel->panel_info.bl_setting_type == DSI_PANEL_BL_UPDATE_METHOD_I2C)
		cancel_delayed_work_sync(&panel->i2c_power_on_work);
	...
	if (!mi_cfg->is_tddi_flag || (mi_cfg->is_tddi_flag && (!mi_cfg->tddi_doubleclick_flag || mi_cfg->panel_dead_flag))) {
		...
		DSI_INFO("mi tddi doubleclick power off dont need reset_gpio and keep power on\n");
	}
#endif
	return ret;
}
		...
		const char *type)
{
	...
	INIT_DELAYED_WORK(&panel->i2c_power_on_work,
			dsi_panel_i2c_power_on_work);
	...
	DSI_INFO("panel create success, initialized: %d, bl: %d\n",
			panel->initialized,
			bl_config->current_brightness);
