...
#define ONETRACK_UNIFY_EVENT_ID 1000
#define MEM_BIG_DATA_APP_ID "31000402203"
...
static void do_report(struct work_struct *work)
{
	...
	event = cdev_tevent_alloc(ONETRACK_UNIFY_EVENT_ID);
	if (!event) {
		pr_err("misight event is error\n");
		return;
	}
	cdev_tevent_add_str(event, "appId", MEM_BIG_DATA_APP_ID);
	cdev_tevent_add_str(event, "eventName", "memory_event");
