...
#include "inc/sc96231_reg.h"
#include "inc/sc96231_mtp_program.h"
#include "inc/sc96231_pgm.h"

#include <mca/common/mca_log.h>
#include <mca/common/mca_event.h>

#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "sc96231_mtp_program"
#endif
...
/*********************pgm api**********************/
static uint16_t pgm_xor(uint8_t *buffer, uint16_t len) {
    uint16_t val = 0x00;
    uint16_t i;
...
static uint8_t pgm_state(struct sc96231 *sc) {
    uint16_t i;
    uint8_t state[4] = {0};
...
static int pgm_chip_erase(struct sc96231 *sc) {
    uint8_t state = 0;
    pgmPktType pgm = {0};
    uint16_t sram_addr_h = SRAM_BASE >> 16;
    uint16_t sram_addr_l = SRAM_BASE & 0xFFFF;
...
static int pgm_set_margin(struct sc96231 *sc, uint8_t buffer) {
    uint8_t state = 0;
    pgmPktType pgm = {0};
    uint16_t sram_addr_h = SRAM_BASE >> 16;
    uint16_t sram_addr_l = SRAM_BASE & 0xFFFF;
...
static int sc96231_get_pgm_info(struct sc96231 *sc, char * info) {
    uint8_t state = 0;
    uint8_t i;
    uint8_t index;
    pgmPktType pgm = {0};
...
static int pgm_read_crc(struct sc96231 *sc, uint8_t * crc, uint16_t crc_size, uint16_t addr, uint16_t len) {
...
static int pgm_write(struct sc96231 *sc, uint32_t addr, uint8_t *buffer, uint16_t len, uint8_t sel) {
    uint16_t i;
    uint8_t state = 0;
    pgmPktType pgm = {0};
    uint16_t offset;
...
static int wait_for_update(struct sc96231 *sc)
{
    ...
    if (ret < 0) {
        mca_log_err("%s: sc96231_write_register failed\n", __func__);
        return -EINVAL;
    }

    ret = sc96231_read_register(sc, CMD_ADDR_LOW, cmd, 4);
    if (ret < 0) {
        mca_log_err("%s: sc96231_read_register failed\n", __func__);
        return -EINVAL;
    }
    cmd[3] |= BIT(7);
...
static int sram_write(struct sc96231 *sc)
{
    ...
    uint32_t size = sizeof(SC96231_PGM_DATA);
    uint8_t addr_high[2] = {0};
    int pgm_pkg_count = 0;
    int pgm_pkg_num = 0;
    ...
    for (i = 0; i < size; i += PGM_PKG_SIZE) {
        if (pgm_pkg_count == pgm_pkg_num && pgm_pkg_remaind != 0) {
            sc96231_write_block(sc, addr_base_l + i, &SC96231_PGM_DATA[i], pgm_pkg_remaind);
            break;
        }
        ret = sc96231_write_block(sc, addr_base_l + i, &SC96231_PGM_DATA[i], PGM_PKG_SIZE);
        if (ret < 0) {
            mca_log_err("%s: sc96231_write_block failed\n", __func__);
            return -EINVAL;
        }
        pgm_pkg_count++;
    }
    ...
    for (x = 0; x < size; x += PGM_PKG_SIZE) {
        if (pgm_pkg_count == pgm_pkg_num && pgm_pkg_remaind != 0) {
            sc96231_read_block(sc, addr_base_l + x, &PGM_READ[x], pgm_pkg_remaind);
            break;
        }
    ...
    for(j = 0; j < size; j++){
        if(SC96231_PGM_DATA[j] != PGM_READ[j]){
            mca_log_err("pgm%d compare wrong!!!------SC96231_PGM_DATA = %02x, PGM_READ = %02x", j, SC96231_PGM_DATA[j], PGM_READ[j]);
            flag = 0;
        }
    }
    if(flag) mca_log_err("pgm compare right!");
//  print_memory_str(PGM_READ, size);
    ...
    if (ret < 0) {
        mca_log_err("%s: sys_reset_ctrl failed\n", __func__);
        return ret;
    }
    msleep(20);
    ...
    mca_log_err("%s: info = %s\n", __func__, info);
    if (info[0] != 'T') {
        mca_log_err("%s: pgm run failed\n", __func__);
        return -EINVAL;
    }
    mca_log_err("%s: pgm run sucessfully\n", __func__);
    return ret;
}
...
int sc96231_mtp_program(struct sc96231 *sc, uint8_t *image, uint32_t len)
{
    ...
    if (ret < 0) {
        mca_log_err("margin 3 check crc fail\n");
        goto program_fail;
    }
