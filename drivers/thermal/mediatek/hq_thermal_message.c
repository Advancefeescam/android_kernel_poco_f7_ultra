...
//#define DISP_DPMS_EVENT             0x01
/* A hardware display power mode state early change occurred */
//#define DISP_DPMS_EARLY_EVENT       0x02
...
/*
    ...
    if (evdata && evdata->data && s_m){
      blank = *(int *)(evdata->data);
		...
		if ((val == DISP_DPMS_EVENT) && (blank == DISP_DPMS_POWERDOWN 
                   || blank == DISP_DPMS_LP1 || blank == DISP_DPMS_LP2)) {
			    sm.screen_state = 0;
		} else if ((val == DISP_DPMS_EVENT) && (blank == DISP_DPMS_ON)) {
			    sm.screen_state = 1;
		}
		pr_info("%s OUT screen_state %d", __func__,sm.screen_state);
	}else {
		pr_info("DISP can not get screen_state");
		return -1;
	}*/
...
static void create_thermal_message_node()
{
	...
	if (!thermal_dev.class && cls) {
		...
		thermal_dev.dev = device_create(thermal_dev.class, NULL, 'H', NULL, "thermal_message");
		if (!thermal_dev.dev) {
			pr_err("%s create device dev err\n", __func__);
			return;
		}
		thermal_dev.attrs.attrs = thermal_dev_attr_group;
		ret = sysfs_create_group(&thermal_dev.dev->kobj, &thermal_dev.attrs);
		if (ret) {
...
static int __init thermal_message_init(void)
{
	int result;
        ...
        //result = blocking_notifier_chain_register(&disp_notifier_list, &sm.thermal_notifier);
	    //mi_disp_register_client(&sm.thermal_notifier);
    	if (result) {
...
static void __exit thermal_message_exit(void)
{
    ...
    //blocking_notifier_chain_unregister(&disp_notifier_list, &sm.thermal_notifier);
