...
/*BSP.TP - 2022.10.14 - Add for focal spi  - Start Modify*/
...
/*BSP.TP - 2022.10.19 - Add for tp_lockdown_info - Start Modify*/
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
	...
	if (enable) {
		if (ts_data->power_disabled) {
			...
			msleep(1);
			gpio_direction_output(ts_data->pdata->dvdd_gpio, 1);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	FTS_INFO("dvdd_gpio =%d", pdata->dvdd_gpio);
	if (pdata->dvdd_gpio < 0)
		FTS_ERROR("Unable to get dvdd_gpio");
...
/*#if defined(CONFIG_DRM)
	...
	/*BSP.TP - 2022.10.14 - Add for focal tp_info  - Start Modify*/
	...
	ret = mtk_disp_notifier_register("FT3680 Touch", &ts_data->fb_notif);
	if (ret) {
...
static int fts_ts_probe(struct spi_device *spi)
{
	...
	/*BSP.TP - 2022.10.14 - Add for focal spi  - Start Modify*/
	#if IS_ENABLED(CONFIG_SPI_MT65XX) //\D3Ż\AFspi\B2\A8\D0β\BB\B7\FB\BA\CFspec\CE\CA\CC\E2
