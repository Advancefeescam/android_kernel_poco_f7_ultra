...
/************************************************************************
 * function: fk_adsp_vote_info_get
 * description: get adsp power vote info.
 * parameter:
 * return:
 *		0: success
 *		other: failure
 ************************************************************************/
int fk_adsp_vote_info_get(void)
{
	int ret = 0;
	struct adsp_vote_info *voteInfo = NULL;
	struct acore_client *ac = fk_acore_get_acore_client();
	...
	list_for_each_entry(voteInfo, &ac->vote_list, vote_info_list) {
		if (voteInfo != NULL && (voteInfo->pu_vote_cnt > voteInfo->pd_vote_cnt)) {
			AUD_LOG_INFO(AUD_CORE, "audio_session(0x%x), pu: cnt(%d), pd: cnt(%d)",
				voteInfo->sess_id, voteInfo->pu_vote_cnt, voteInfo->pd_vote_cnt);
		}
	}
...
int fk_adsp_vote_unregister(uint16_t session_id)
{
	...
	if (voteInfo->pd_vote_cnt > voteInfo->pu_vote_cnt) {
		...
		voteInfo->pu_vote_cnt = 0;
		voteInfo->pd_vote_cnt = 0;
#if IS_ENABLED(CONFIG_MIEV)
		mievent  = cdev_tevent_alloc(906001501);
		cdev_tevent_add_str(mievent, "Keyword", "adsp vote unregister fail");
	...
	ret = fk_adsp_vote_info_get();
	if (ret < 0)
		AUD_LOG_ERR(AUD_CORE, "get adsp vote info failed");
