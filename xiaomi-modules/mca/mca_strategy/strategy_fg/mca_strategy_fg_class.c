...
{
	if (is_invalid_ops(strategy_fg_get_thermal_temp))
		return -1;

	return strategy_fg_class_ops_with_one_para(strategy_fg_get_thermal_temp, temp);
}
...
int strategy_class_fg_get_auth_event(bool *auth_event_flag)
{

	if (is_invalid_ops(strategy_fg_get_auth_event))
		return -1;

	return strategy_fg_class_ops_with_one_para(strategy_fg_get_auth_event, auth_event_flag);
}
EXPORT_SYMBOL(strategy_class_fg_get_auth_event);
...
int strategy_class_fg_get_batt_ctr_sts(int role, bool *batt_ctr_sts)
{
	if (is_invalid_ops(strategy_fg_get_batt_ctr_sts))
		return -1;

	return g_stg_fg_class_info.ops->strategy_fg_get_batt_ctr_sts(g_stg_fg_class_info.data,
		role, batt_ctr_sts);
}
EXPORT_SYMBOL(strategy_class_fg_get_batt_ctr_sts);

int strategy_class_fg_get_final_iterm(int *iterm)
{
	if (is_invalid_ops(strategy_fg_get_final_iterm))
		return -1;

	return strategy_fg_class_ops_with_one_para(strategy_fg_get_final_iterm, iterm);
}
EXPORT_SYMBOL(strategy_class_fg_get_final_iterm);
