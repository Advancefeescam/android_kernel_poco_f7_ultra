...
static int keyboard_check_panel(struct device_node *np)
{
    ...
    dbgprint(INFO_LEVEL, "panel count=%d\n", count);
    if (count <= 0)
        return -ENODEV;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (!IS_ERR(panel))
        {
            ...
            if (active_panel)
            {
                dbgprint(INFO_LEVEL, "check panel ok\n");
            }
            return 0;
        }
        else
		        ...
		        struct xring_panel_event_notification *notification, void *client_data)
{
    ...
    case DRM_PANEL_EVENT_UNBLANK:
        ...
        */
        dbgprint(INFO_LEVEL, "keyboard resume\n");
        queue_work(mclient_data->event_wq, &mclient_data->early_resume_work);
        queue_work(mclient_data->event_wq, &mclient_data->resume_work);
        break;
    case DRM_PANEL_EVENT_BLANK:
        ...
        */
        dbgprint(INFO_LEVEL, "keyboard suspend\n");
        queue_work(mclient_data->event_wq, &mclient_data->early_suspend_work);
        queue_work(mclient_data->event_wq, &mclient_data->suspend_work);
        break;
    default:
                                           ...
                                           struct panel_event_notification *notification, void *client_data)
{
    ...
    case DRM_PANEL_EVENT_UNBLANK:
        ...
        {
            queue_work(mclient_data->event_wq, &mclient_data->early_resume_work);
            dbgprint(INFO_LEVEL, "early_trigger keyboard resume\n");
        }
        else
        {
            queue_work(mclient_data->event_wq, &mclient_data->resume_work);
    ...
    case DRM_PANEL_EVENT_BLANK_LP:
        ...
        {
            queue_work(mclient_data->event_wq, &mclient_data->early_suspend_work);
            dbgprint(INFO_LEVEL, "early_trigger keyboard suspend\n");
        }
        else
        {
            queue_work(mclient_data->event_wq, &mclient_data->suspend_work);
...
static int nanosic_803_probe(struct i2c_client *client)
{
    ...
    dbgprint(INFO_LEVEL, "probe adapter nr %d, addr 0x%x\n", client->adapter->nr, client->addr);
    ...
    dbgprint(INFO_LEVEL, "probe nanosic driver\n");
...
static void nanosic_803_remove(struct i2c_client *client)
{
    ...
    dbgprint(INFO_LEVEL, "remove nanosic driver\n");
...
static __init int nanosic_driver_init(void)
{
    ...
    dbgprint(INFO_LEVEL, "nanosic_driver_init, ret = %d\n", ret);
...
static __exit void nanosic_driver_exit(void)
{
    ...
    dbgprint(INFO_LEVEL, "nanosic_driver_exit");
}
...
static int xiaomi_keyboard_init(struct nano_i2c_client *i2c_client)
{
    ...
    dbgprint(INFO_LEVEL, "irq:%d", mdata->irq);
    mdata->event_wq = create_singlethread_workqueue("kb-event-queue");
    if (!mdata->event_wq)
    {
        dbgprint(INFO_LEVEL, "Can not create work thread for suspend/resume!!");
        // ret = -ENOMEM;
    }
    ...
    if (active_panel)
    {
        ...
        {
            dbgprint(ERROR_LEVEL, "register drm_notifier failed. cookie=%s\n", (char *)mdata->notifier_cookie);
            goto err_register_drm_notif_failed;
        }
    }
    dbgprint(INFO_LEVEL, "success. \n");
    return ret;
...
static void keyboard_resume_work(struct work_struct *work)
{
    ...
    dbgprint(INFO_LEVEL, "send screen on:%d\n", ret);
...
static void keyboard_early_suspend_work(struct work_struct *work)
{
    ...
    dbgprint(INFO_LEVEL, "send screen off:%d\n", ret);
...
{
    int ret = 0;
    dbgprint(INFO_LEVEL, "enter, enable_irq_wake\n");
    ret = enable_irq_wake(mdata->irq); // i2c_client->irqno
    if (ret < 0)
        dbgprint(ERROR_LEVEL, "enter, enable_irq_wake irq failed\n");
    ret = enable_irq_wake(g_wakeup_irqno);
    if (ret < 0)
        dbgprint(ERROR_LEVEL, "enter, enable_irq_wake wakeup_irqno failed\n");
...
{
    int ret = 0;
    dbgprint(INFO_LEVEL, "disable_irq_wake\n");
    ret = disable_irq_wake(mdata->irq); // i2c_client->irqno
    if (ret < 0)
        dbgprint(ERROR_LEVEL, "disable_irq_wake irq failed\n");
    ret = disable_irq_wake(g_wakeup_irqno);
    if (ret < 0)
        dbgprint(ERROR_LEVEL, "disable_irq_wake wakeup_irqno failed\n");
