...
int charger_disable_dpdm_block(struct charger_dev *charger)
{
	if (!charger || !charger->ops)
		return -EINVAL;
	if (charger->ops->disable_dpdm_block == NULL)
		return -EOPNOTSUPP;
	return charger->ops->disable_dpdm_block(charger);
}
EXPORT_SYMBOL(charger_disable_dpdm_block);
...
int charger_get_rechg_volt(struct charger_dev *charger, int *mv)
{
	if (!charger || !charger->ops)
		return -EINVAL;
	if (charger->ops->get_rechg_vol == NULL)
		return -EOPNOTSUPP;
	return charger->ops->get_rechg_vol(charger, mv);
}
EXPORT_SYMBOL(charger_get_rechg_volt);
...
int charger_dump_registers(struct charger_dev *charger)
{
	if (!charger || !charger->ops)
		return -EINVAL;
	if (charger->ops->dump_registers == NULL)
		return -EOPNOTSUPP;
	return charger->ops->dump_registers(charger);
}
EXPORT_SYMBOL(charger_dump_registers);

int charger_en_cid(struct charger_dev *charger, bool en)
{
	if (!charger || !charger->ops)
		return -EINVAL;
	if (charger->ops->en_cid == NULL)
		return -EOPNOTSUPP;
	return charger->ops->en_cid(charger, en);
}
EXPORT_SYMBOL(charger_en_cid);

#if IS_ENABLED(CONFIG_RUST_DETECTION)
int charger_rust_detection_enable(struct charger_dev *charger, int en)
{
	if (!charger || !charger->ops)
		return -EINVAL;
	if (charger->ops->rust_detection_enable == NULL)
		return -EOPNOTSUPP;
	return charger->ops->rust_detection_enable(charger, en);
}
EXPORT_SYMBOL(charger_rust_detection_enable);

int charger_rust_detection_read_res(struct charger_dev *charger)
{
	if (!charger || !charger->ops)
		return -EINVAL;
	if (charger->ops->rust_detection_read_res == NULL)
		return -EOPNOTSUPP;
	return charger->ops->rust_detection_read_res(charger);
}
EXPORT_SYMBOL(charger_rust_detection_read_res);
#endif
...
{
	return atomic_notifier_chain_unregister(&charger_notifier, nb);
}
				...
				struct charger_ops *ops, void *private)
{
	...
	if (!parent)
		hq_err("Expected proper parent device\n");
...
static int __init charger_class_init(void)
{
	...
	hq_info("charger class initialize success\n");
