 ...
 * @last_irq_time: the time of irq trigger
 * @sink_id: sink id of irq trigger
 * @irq_downgrade_flag: if a irq triggered serval times, need to downgrade link_rate
 * @irq_reconn_flag: if a irq is triggered and required to re-training
 * @lock: to protect hardware config shared with hpd and frame commit
 * @irq_lock: to protect dpcd access shared with hpd irq and lp_exit
 */
struct dp_hpd {
	struct dp_display *display;
	...
	u8 sink_id[SINK_ID_SIZE];
	bool irq_downgrade_flag;
	bool irq_reconn_flag;
