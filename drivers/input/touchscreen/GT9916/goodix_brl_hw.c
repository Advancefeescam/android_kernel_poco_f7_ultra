...
#define GOODIX_EDGE_DATA_LEN		130
#define GOODIX_CFG_CMD_LEN		4
#define GOODIX_CMD_LEN			6
#define GOODIX_DIR_PORTRAIT		0
#define GOODIX_DIR_RIGHT		1
#define GOODIX_DIR_LEFT			3
#define CONFIG_CMD_EDGE_RETRY		12
...
/*P6 code for HQFEAT-118700 by p-zhaobeidou3 at 2025/6/27 start*/
		...
		xiaomi_touch_mievent_report_int(TOUCH_EVENT_TRANSFER_ERR, 0, "TpTransferErr", "goodix", retry);
#endif
...
/* P6 code for BUGP6-3531 by p-xiewei79 at 2025/8/19 start */
	enable_temperature_detection_func();
...
{
	if (!touch_data || !buf) {
		ts_err("touch_data or buf is NULL");
		return;
	}
...
{
	u8 cur_key_map = 0;
	int16_t x_angle = 0, y_angle = 0;
	int i = 0;
	if (!pen_data || !event_head || !buf) {
		ts_err("pen_data or event_head or buf is NULL");
		return;
	}
...
{
	struct goodix_touch_data *touch_data = NULL;
	struct goodix_pen_data *pen_data = NULL;
	int point_struct_len = 0;
	u8 touch_num = 0;
	int ret = 0;
	u8 *data = NULL;
	u8 point_type = 0;
	...
	int i = 0;
	int tid = 0;
	if (!cd || !ts_event || !buffer) {
		ts_err("cd or ts_event or buffer is NULL");
		return -EINVAL;
	}
	touch_data = &ts_event->touch_data;
	pen_data = &ts_event->pen_data;
	touch_num = buffer[2] & 0x0F;
	point_struct_len = cd->ic_info.misc.point_struct_len;
	if (point_struct_len == 0) {
		ts_debug("invalid point struct len");
		return -EINVAL;
	}
	...
	if (touch_num > 0) {
		...
		switch (point_type) {
			...
			case POINT_TYPE_FINGER:
				...
				break;
			default:
				ts_debug("not support point type:%d",
					point_type);
				break;
			}
		}
	...
	touch_data->fp_flag = (buffer[0] & GOODIX_FP_EVENT) ? 1 : 0;
	if (touch_data->fp_flag) {
		touch_data->overlay = buffer[IRQ_EVENT_HEAD_LEN + touch_num * point_struct_len + 2];
		if (buffer[IRQ_EVENT_HEAD_LEN + 1])
			touch_data->fod_id = (buffer[IRQ_EVENT_HEAD_LEN] >> 4) & 0x0F;
	}
	return 0;
}
			 ...
			 struct goodix_ts_event *ts_event)
{
	...
	struct goodix_ic_info_misc *misc = NULL;
	/*N6 code for HQ-305056 by zhangzhijian5 at 2023/10/26 start*/
	u8 pre_buf[IRQ_EVENT_MAX_SIZE] = {0};
	u8 event_status = 0;
	u8 large_touch_status = 0;
	int ret = 0;
	/*N6 code for HQ-305056 by zhangzhijian5 at 2023/10/26 end*/
	if (!cd || !ts_event) {
		ts_err("cd or ts_event is NULL");
		return -EINVAL;
	}
	hw_ops = cd->hw_ops;
...
/*N6 code for HQ-334519 by liaoxianguo at 2023/10/17 start*/
	...
	memcpy(ts_event->touch_data.tmp_data, pre_buf,
			sizeof(ts_event->touch_data.tmp_data));
...
{
	int ret = 0;
	int len = GOODIX_EDGE_DATA_LEN;
	u8 *tmp_buf = NULL;
	struct goodix_ts_cmd cfg_cmd = {0};
	struct goodix_ic_info_misc *misc;

	if (!cd || !cd->hw_ops || !cd->ic_info.misc.self_diffdata_addr) {
		ts_err("Edge data invalid pointer detected");
		return -EINVAL;
	}
	...
	ret = cd->hw_ops->write(cd, misc->self_diffdata_addr, cd->edge_data.buf, len);
	if (ret) {
	...
	ret = cd->hw_ops->read(cd, misc->self_diffdata_addr, tmp_buf, len);
	if (ret) {
	...
	memset(cfg_cmd.buf, 0, sizeof(cfg_cmd));
	cfg_cmd.len = GOODIX_CFG_CMD_LEN;
...
int brl_Edge_suppression(struct goodix_ts_core *cd)
{
	...
	if (!cd || !cd->hw_ops) {
		ts_err("cd or hw_ops is NULL");
		return -EINVAL;
	}

	mutex_lock(&cd->edge_data_mutex);
	cmd.len = GOODIX_CMD_LEN;
	cmd.cmd = GOODIX_NORMAL_CMD;
	cmd.data[1] = 0x80; /*按照供应商建议a0改成80*/
	/*02:Portrait;60:Horizontal_USB_right;0xa0:Horizontal_USB_left*/
	if (cd->gtp_direction_value == GOODIX_DIR_PORTRAIT ) {
		cmd.data[0] = 0x02;
	} else if (cd->gtp_direction_value == GOODIX_DIR_RIGHT ) {
		cmd.data[0] = 0x60;
	} else if (cd->gtp_direction_value == GOODIX_DIR_LEFT ) {
		cmd.data[0] = 0xa0;
	}
	ts_debug("nongamemode screen direction is %d", cd->gtp_direction_value);
	if (!(cd->hw_ops->send_cmd(cd, &cmd))) {
		ret = wait_cmd_status(cd, CONFIG_CMD_STATUS_PASS,CONFIG_CMD_EDGE_RETRY);
	} else {
...
int brl_game(struct goodix_ts_core *cd, u8 data0, u8 data1)
{
	...
	int ret = 0;
	int retry = 0;
	/*O6 code for HQ-410242 by liuyupei at 2024/08/12 start */
	if (!cd || !cd->hw_ops ) {
		ts_err("cd or hw_ops is NULL");
		return -EINVAL;
	}
	/*O6 code for HQ-410242 by liuyupei at 2024/08/12 end */
