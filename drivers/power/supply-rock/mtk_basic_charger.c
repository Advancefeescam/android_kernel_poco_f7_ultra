...
/*L19A HQ-216528 modify cycle count by tongjiacheng at 2022/05/17 start*/
	if (IS_ERR_OR_NULL(psy)) {
/*L19A HQ-216528 modify cycle count by tongjiacheng at 2022/05/17 end*/
...
/*L19A HQ-216528 add cycle count by tongjiacheng at 2022/04/29 start*/
	u32 ffc_constant_voltage = get_charge_cycle_count_level(info);

	if (info->enable_sw_ffc) {
		if (ffc_constant_voltage != 0) {
/*L19A HQ-216528 modify cycle count by gengyifei at 2022/05/18 start*/
...
/*L19A HQ-216528 modify cycle count by gengyifei at 2022/05/18 end*/
			pr_err("%s: ffc constant voltage = %d\n", __func__, ffc_constant_voltage);
...
/*L19A HQ-216528 add cycle count by tongjiacheng at 2022/04/29 end*/
...
static bool is_typec_adapter(struct mtk_charger *info)
{
	...
	if (info->pd_type == MTK_PD_CONNECT_TYPEC_ONLY_SNK &&
			...
			info->chr_type != POWER_SUPPLY_TYPE_USB_CDP && info->chr_type != POWER_SUPPLY_TYPE_USB_DCP &&info->enable_type_c == true)
		return true;
...
static void mtk_battery_psy_detect_work(struct work_struct *work)
{
	...
	device_create_file(&psy->dev,&dev_attr_mtbf_current);
...
/*L19A HQ-192247 modify atm current by miaozhichao at 2022/4/24 start*/
		pdata->input_current_limit = 500000; /* 500mA */
		pdata->charging_current_limit = 500000;
/*L19A HQ-192247 modify atm current by miaozhichao at 2022/4/24 end*/
		is_basic = true;
		goto done;
	}
...
/*L19AT HQ-289019 After the call, the current recovery on the charging line is delayed by miaozhichao at 2023/03/10 start*/
	if ((val.intval > 0) && (pdata->charging_current_limit > val.intval * 1000))
		pdata->charging_current_limit = val.intval * 1000;
/*L19AT HQ-289019 After the call, the current recovery on the charging line is delayed by miaozhichao at 2023/03/10 end*/
	/*L19A HQ-194117 add node to limit current by tongjiacheng at 2022/04/19 end*/
/*L19AT code for HQ-256463 by miaozhichao at 2022/11/28 start*/
	if ((info->chr_type == POWER_SUPPLY_TYPE_USB && info->usb_type == POWER_SUPPLY_USB_TYPE_SDP) || (info->chr_type == POWER_SUPPLY_TYPE_USB_CDP)) {
...
static int judge_recharge_status(struct mtk_charger *info)
{
	bool chg_done = false;
	unsigned int uisoc = 0;
	long int volt = 0;
	int health  = 0;
	int ret;
	struct power_supply *psy;
	...
	psy = power_supply_get_by_name("battery");
/*L19A HQ-194293 modify recharge ways by tongjiacheng at 2022/04/29 start*/
	if (IS_ERR_OR_NULL(psy)) {
/*L19A HQ-194293 modify recharge ways by tongjiacheng at 2022/04/29 end*/
		pr_err("%s: failed to find battery psy\n", __func__);
		return PTR_ERR(psy);
	}

	ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_HEALTH, &val);
	if (ret) {
		pr_err("%s: failed to get prop: %d\n", __func__, POWER_SUPPLY_PROP_HEALTH);
/*L19A HQ-194293 modify recharge ways by tongjiacheng at 2022/04/29 start*/
		return -EINVAL;
/*L19A HQ-194293 modify recharge ways by tongjiacheng at 2022/04/29 end*/
	}
	...
	if (ret) {
		pr_err("%s: failed to get prop: %d\n", __func__, POWER_SUPPLY_PROP_VOLTAGE_NOW);
/*L19A HQ-194293 modify recharge ways by tongjiacheng at 2022/04/29 start*/
		return -EINVAL;
/*L19A HQ-194293 modify recharge ways by tongjiacheng at 2022/04/29 end*/
	}
	...
	if (ret) {
		pr_err("%s: failed to get prop: %d\n", __func__, POWER_SUPPLY_PROP_CAPACITY);
/*L19A HQ-194293 modify recharge ways by tongjiacheng at 2022/04/29 start*/
		return -EINVAL;
/*L19A HQ-194293 modify recharge ways by tongjiacheng at 2022/04/29 end*/
	}
	...
	pr_err("%s: battery: health = %d, volt = %d,  uisoc = %d, recharger_soc_limt = %d\n",
					__func__, health, volt, uisoc, info->data.recharger_uisoc_limit);
	...
	if (health == POWER_SUPPLY_HEALTH_OVERHEAT) {
		...
		if (uisoc < info->data.recharger_uisoc_limit) {
...
/*L19A HQ-194293 add recharge ways by tongjiacheng at 2022/04/20  end*/
	...
	int ret;
	int val = 0;
/*HQ-226118 modify ui 100 to 98 change charging status miaozhichao at 2022/08/4 start*/
...
/*L19A HQ-194293 add recharge ways by tongjiacheng at 2022/04/20  start*/
			...
			flag = true;
			chr_err("%s battery not full\n", __func__);
		}
	}
	 if(recharge_flag && flag) {
		flag = false;
/*L19A HQ-194293 add recharge ways by tongjiacheng at 2022/04/20  end*/
	...
	chr_err("%s is_basic:%d,info->is_chg_done:%d\n", __func__, is_basic,info->is_chg_done);
	/*HQ-226118 modify ui 100 to 98 change charging status miaozhichao at 2022/08/4 end*/
	if (is_basic != true) {
		is_basic = true;
		for (i = 0; i < MAX_ALG_NO; i++) {
			alg = info->alg[i];
...
/*L19AT code for HQ-256463 by miaozhichao at 2022/11/28 start*/
	INIT_DELAYED_WORK(&info->battery_psy_detect_work,mtk_battery_psy_detect_work);
	schedule_delayed_work(&info->battery_psy_detect_work, msecs_to_jiffies(20000));
/*L19AT code for HQ-256463 by miaozhichao at 2022/11/28 end*/
	return 0;
}
