...
inline bool dsi_is_invalid_power_status(struct dsi_display *display)
{
	return (!is_dsi_powered_on() && dsi_display_initialized_get(display));
}

int dsi_display_backlight_power_on(struct dsi_display *display)
{
	...
	if (panel->panel_info.bl_setting_type != DSI_PANEL_BL_UPDATE_METHOD_I2C)
		return 0;

	display_frame_duration_get(display, &frame_duration_ms);
	frame_duration_ms += I2C_BACKLIGHT_ON_DELAY_MS;
#ifdef MI_DISPLAY_MODIFY
	if (mi_get_panel_id(mi_cfg->mi_panel_id) == O81A_PANEL_PA) {
		schedule_delayed_work(&display->panel->i2c_power_on_work,
				msecs_to_jiffies(frame_duration_ms));
		DSI_DEBUG("trigger bl power on after %d ms\n", frame_duration_ms);
	}
#endif
	return 0;
}
...
#ifdef MI_DISPLAY_MODIFY
		if (mi_cfg->is_tddi_flag && mi_cfg->tddi_doubleclick_flag) {
			ret = dsi_panel_reset(panel_info);
			if (ret)
				DSI_ERROR("failed to reset panel\n");
		}
...
{
	if (unlikely(dsi_is_invalid_power_status(display)))
		DPU_ERROR("invalid dsi power status, initialized %d, power %s\n",
				dsi_display_initialized_get(display),
				is_dsi_powered_on() ? "on" : "off");
		...
		struct dsi_display_info *info)
{
	...
	info->idle_enter_time = panel_info->idle_enter_time;
...
static int dsi_display_panel_lockdown_info_init(struct device_node *dev_node)
{
	u32 lockdown_array[8] = {0};
	u32 temp_count;
	int ret;

	temp_count = of_property_count_u32_elems(dev_node, "lockdown_info");
	if (temp_count != LOCKDOWN_INFO_LEN) {
		DSI_ERROR("set lockdown info length is error\n");
		return -EINVAL;
	}

	ret = of_property_read_u32_array(dev_node,
		"lockdown_info", lockdown_array, temp_count);
	if (ret)
		DISP_ERROR("error reading lockdown info values\n");

	dsi_panel_lockdown_info_set(lockdown_array);

	return ret;
}

#ifdef MI_DISPLAY_MODIFY
static int dsi_display_panel_wp_info_init(struct dsi_display *display, struct device_node *dev_node)
{
	struct dsi_panel *panel;
	...
	if (!display || !display->panel) {
		DSI_ERROR("Invalid display ptr\n");
		return -EINVAL;
	}

	panel = display->panel;
	config = &panel->mi_panel.wp_config;

	ret = of_property_read_string(dev_node, "panel_wp_info", &config->wp_info);
	if (ret) {
		config->wp_info = "00000000";
		DSI_ERROR("error reading wp info values!\n");
	}

	DSI_INFO("init panel wp info =%s\n", config->wp_info);
	return ret;
}

static int dsi_display_panel_cell_id_init(struct dsi_display *display, struct device_node *dev_node)
{
	struct dsi_panel *panel;
	...
	panel = display->panel;
	config = &panel->mi_panel.cell_id_config;

	ret = of_property_read_string(dev_node, "panel_cell_id", &config->cell_id);
	if (ret) {
		config->cell_id = "00000000";
		DSI_ERROR("error reading cell id values!\n");
	}

	DSI_INFO("init panel cell_id = %s\n", config->cell_id);
	return ret;
}
#endif
...
#ifdef MI_DISPLAY_MODIFY
	panel->mi_panel.mi_cfg.panel_dead_flag = true;
#endif
		...
		struct device *master, void *data)
{
	...
	DSI_INFO("set display status: %d, panel status: %d\n",
			dsi_display_initialized_get(display),
			display->panel->initialized);

	ret = dsi_display_panel_lockdown_info_init(dev_node);
	if (ret)
		DSI_ERROR("getlockdown info failed, ret =%d\n", ret);

#ifdef MI_DISPLAY_MODIFY
	ret = dsi_display_panel_wp_info_init(display, dev_node);
	if (ret)
		DSI_ERROR("get wp info failed, ret =%d\n", ret);

	ret = dsi_display_panel_cell_id_init(display, dev_node);
	if (ret)
		DSI_ERROR("get cell id failed, ret =%d\n", ret);
#endif
