...
struct mca_bmd_dev {
	...
	struct delayed_work report_bmd_sts_work;
...
static void mca_bmd_monitor_workfunc(struct work_struct *work)
{
	...
	if (strategy_class_fg_ops_is_init_ok() <= 0 && count < 3) {
		mca_log_info("fg is not ready, wait for it, retry_count:%d\n", count);
		count++;
		goto out;
	} else {
		count = 0;
	...
	if (info->batt_missing != batt_missing) {
		...
		if (batt_missing) {
			if (info->fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX) {
				report_data = !info->btb_online[MASTER_BTB] ? 0 : 1;
				mca_charge_mievent_report(CHARGE_DFX_DUAL_BATTERY_MISSING, &report_data, 1);
			} else {
				mca_charge_mievent_report(CHARGE_DFX_BATTERY_MISSING, NULL, 0);
			}
		} else {
			if (info->fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX)
				mca_charge_mievent_set_state(MIEVENT_STATE_END, CHARGE_DFX_DUAL_BATTERY_MISSING);
			else
...
static void mca_bmd_report_bmd_sts_work(struct work_struct *work)
{
	struct mca_bmd_dev *info = container_of(work, struct mca_bmd_dev,
		report_bmd_sts_work.work);
	bool batt_missing = false;
	int fake_batt;
	...
	mca_event_block_notify(MCA_EVENT_TYPE_HW_INFO, MCA_EVENT_BATT_BTB_CHANGE, &batt_missing);
	...
	if (info->fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX) {
		...
		mca_event_block_notify(MCA_EVENT_TYPE_BATTERY_INFO, MCA_EVENT_BATTERY_FAKE_POWER, &fake_batt);
	}
}
...
static bool mca_bmd_get_btb_status(struct mca_bmd_dev *info, int index)
{
	...
	mca_log_info("bmd_scheme = %d, index = %d, fg_type = %d\n", info->bmd_scheme[index].scheme, index, info->fg_type);
	...
	case IIC_SCHEME:
		if (info->fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX)
			data = strategy_class_fg_dual_is_chip_ok(index);
		else
...
static int mca_bmd_process_event(int event, int value, void *data)
{
	...
	case MCA_EVENT_BMD_STSTUS_CHANGE:
		cancel_delayed_work_sync(&info->report_bmd_sts_work);
		schedule_delayed_work(&info->report_bmd_sts_work, 0);
		break;
	default:
		break;
	}
...
static int mca_bmd_parse_dt(struct mca_bmd_dev *info)
{
	...
	mca_parse_dts_u32(node, "fg_type", &(info->fg_type), MCA_FG_TYPE_SINGLE);
...
static int mca_bmd_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&info->report_bmd_sts_work, mca_bmd_report_bmd_sts_work);
	if (online) {
		cancel_delayed_work_sync(&info->monitor_bmd_work);
		schedule_delayed_work(&info->monitor_bmd_work, msecs_to_jiffies(FAST_HEARTBEAT_TIMER_MS));
	}
