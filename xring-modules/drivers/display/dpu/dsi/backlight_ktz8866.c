		...
		unsigned int reg, unsigned int *data)
{
	...
	if (ktz) {
		...
		DSI_DEBUG("i2c read, reg:0x%x, val:0x%x", reg, *data);
		if (ret < 0)
...
static int ktz_update_status_o81a(struct ktz8866_led *ktz, unsigned int level)
{
	int exponential_bl = level;
	int brightness = 0;
	const int normal_max_dvb = 1433;
	unsigned int v[2];
	...
	if (exponential_bl >= 0 && exponential_bl < (ARRAY_SIZE(mi_bl_level_remap_o81a)))
		brightness = mi_bl_level_remap_o81a[exponential_bl];
	...
	DSI_DEBUG("ktz8866_power_status = %d\n", ktz8866_power_status);
	if (!ktz->ktz8866_status && brightness > 0) {
		if (ktz8866_power_status)
			ktz8866_write(ktz, KTZ8866_DISP_BL_ENABLE, 0x7f);
		ktz->ktz8866_status = 1;
		DSI_DEBUG("ktz8866 backlight enable,dimming close");
	} else if (brightness == 0) {
...
static void ktz_power_on(struct ktz8866_led *ktz)
{
	...
	if (ktz->ktz8866_status) {
		ktz8866_write(ktz, KTZ8866_DISP_BB_LSB, ktz->level & 0x7);
		ktz8866_write(ktz, KTZ8866_DISP_BB_MSB, (ktz->level >> 3) & 0xff);
	} else {
...
static void ktz_power_on_except_vsp_vsn(struct ktz8866_led *ktz)
{
	...
	DSI_DEBUG("%s!!\n", __func__);
	ktz8866_write(ktz, KTZ8866_DISP_BC1, 0x93);/* Backlight OVP 23.9V, exponential mapping, enable PWM*/
...
#ifdef MI_DISPLAY_MODIFY
	.update_status_o81a	= ktz_update_status_o81a,
#endif
...
#ifdef MI_DISPLAY_MODIFY
int ktz8866_backlight_update_status_o81a(unsigned int level)
{
	int ret = 0;
	static struct ktz8866_led *ktz;
	unsigned int read;
	...
	mutex_lock(&ktz8866_dev_list_mutex);
	list_for_each_entry(ktz, &ktz8866_dev_list, entry) {
		ret = ktz->ops->update_status_o81a(ktz, level);
		if (ret)
			DSI_ERROR("failed to update ktz8866 bl status\n");
		ret = ktz->ops->get_brightness(ktz, KTZ8866_DISP_BB_MSB, &read);
		if (ret)
