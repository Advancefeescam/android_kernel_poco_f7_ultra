...
#include "../diamond_xiaomi/xiaomi_touch.h"
#endif

#define GOODIX_DEFAULT_CFG_NAME		"goodix_cfg_group.cfg"
#define GOOIDX_INPUT_PHYS			"goodix_ts/input0"
#define DISP_ID_DET					(290+50)
...
/*BSP.TP - 2022.10.21 - Add for goodix charger  - Start Modify*/
/* charger show */
...
/*BSP.TP - 2022.11.2 - Add for xiaomi_touch - Start Modify*/
...
static DEVICE_ATTR(double_tap, 0664,
		goodix_ts_double_tap_show, goodix_ts_double_tap_store);
	...
	for (i = 0; i < GOODIX_MAX_TOUCH; i++) {
		if (touch_data->coords[i].status == TS_TOUCH) {
			...
			input_report_abs(dev, ABS_MT_POSITION_X,
					touch_data->coords[i].x); //*10 Touch Super Resolution
...
out:
	...
	/*BSP.TP - 2022.10.25 - Add for goodix palm  - Start Modify*/
	...
	unsigned long event, void *data)
{
	...
	if (data && core_data) {
		...
		ts_info("GT9916R touch IC can not suspend or resume");
		return -1;
	}
...
int goodix_ts_stage2_init(struct goodix_ts_core *cd)
{
	...
	ret = mtk_disp_notifier_register("GT9916R Touch", &cd->fb_notifier);
	if (ret) {
...
/*BSP.TP - 2022.10.13 - Add for ITO test and read fw version - Start Modify*/
...
/*BSP.TP - 2022.10.17 - Add for Goodix read tp lockdown_info - Start Modify*/
...
/*BSP.TP - 2022.10.14 - Add for Goodix hw_info - Start Modify*/
int goodix_ts_hw_info(struct goodix_ts_core *core_data)
{
	...
	if (!hw_ops)
		return 0;
	if (hw_ops->read_version && hw_ops->get_ic_info) {
		ret = hw_ops->read_version(goodix_core_data, &chip_ver);
		tmp = hw_ops->get_ic_info(goodix_core_data, &goodix_core_data->ic_info);
		if (!ret || !cnt) {
			cnt = snprintf(gt_hw_info, sizeof(gt_hw_info),
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	/*BSP.TP - 2022.10.14 - Add for Goodix hw_info - Start Modify*/
	ret = goodix_ts_hw_info(core_data);
	if (ret) {
		ts_err("failed add hw_info");
		//goto err_out;
	}
	/*End Modify*/
...
static int __init goodix_ts_core_init(void)
{
	...
	gpio_50 = gpio_get_value(DISP_ID_DET);
	...
	ts_info("TP is goodix. Core layer init:%s", GOODIX_DRIVER_VERSION);
