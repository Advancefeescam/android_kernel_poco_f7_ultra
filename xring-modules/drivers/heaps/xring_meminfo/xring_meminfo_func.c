...
static BLOCKING_NOTIFIER_HEAD(xr_lowmem_notify_list);
...
int register_xr_lowmem_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&xr_lowmem_notify_list, nb);
}
EXPORT_SYMBOL(register_xr_lowmem_notifier);

int unregister_xr_lowmem_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&xr_lowmem_notify_list, nb);
}
EXPORT_SYMBOL(unregister_xr_lowmem_notifier);
...
void show_extra(enum meminfo_source where)
{
	...
	blocking_notifier_call_chain(&xr_lowmem_notify_list, 0, NULL);
}
...
void show_tasks(bool verbose)
{
	...
	for_each_process(p) {
		...
		xrmeminfo_info("[%7d] %5d %5d %8lu %8luKB %8ld %8luKB %5d %c %c %s\n",
				task->pid, from_kuid(&init_user_ns, task_uid(task)),
				task->tgid, task->mm->total_vm, task_mm_rss * 4,
				mm_pgtables_bytes(task->mm), task_mm_swap * 4,
