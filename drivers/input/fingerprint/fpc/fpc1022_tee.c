...
/*L19A code for HQ-194827 by xiongwei at 22.5.11 start*/
#include <linux/fb.h>
#include <drm/drm_bridge.h>
#include "../../../gpu/drm/mediatek/mediatek_v2/mtk_notifier_odm.h"
/*L19A code for HQ-194827 by xiongwei at 22.5.11 end*/
#endif
#include <teei_fp.h>
...
struct fpc1022_data {
	...
	struct pinctrl_state *st_pwr_disable;
	struct pinctrl_state *st_pwr_enable;
			    ...
			    size_t count)
{
	...
	if (!strncmp(buf, "reset", strlen("reset"))) {
		...
		pinctrl_select_state(fpc1022->pinctrl, fpc1022->st_pwr_disable);
		msleep(10);
		pinctrl_select_state(fpc1022->pinctrl, fpc1022->st_pwr_enable);
...
static int fpc1022_platform_probe(struct platform_device *pldev)
{
	 ...
	 */
	fpc1022->st_pwr_disable = pinctrl_lookup_state(fpc1022->pinctrl, "avdd_en_low");
	if (IS_ERR(fpc1022->st_pwr_disable)) {
		ret = PTR_ERR(fpc1022->st_pwr_disable);
		dev_err(dev, "pinctrl pwr disable failed\n");
		goto err_lookup_state;
	}
	fpc1022->st_pwr_enable = pinctrl_lookup_state(fpc1022->pinctrl, "avdd_en_high");
	if (IS_ERR(fpc1022->st_pwr_enable)) {
		ret = PTR_ERR(fpc1022->st_pwr_enable);
		dev_err(dev, "pinctrl pwr enable failed\n");
		goto err_lookup_state;
	}
	...
	if (goodix_fp_exist) {
		pr_err
		    ("%s goodix sensor has been detected, so exit FPC sensor detect.\n",
		     __func__);
/*L19A code for HQ-198787 by xiongwei at 22.4.23 start*/
		return 0;
/*L19A code for HQ-198787 by xiongwei at 22.4.23 end*/
	}
