...
int mi_dsi_panel_set_disp_param(struct dsi_panel *panel, struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_CABC:
		...
		if (ctl->feature_val == LCD_CABC_OFF) {
			rc = dsi_display_cmd_set_send_locked(display, DSI_CMD_SET_CABC_OFF, NULL, USE_CPU);
			DISP_INFO("lcd cabc off\n");
		}
		if (ctl->feature_val == LCD_CABC_UI_ON) {
			rc = dsi_display_cmd_set_send_locked(display, DSI_CMD_SET_CABC_UI_ON, NULL, USE_CPU);
			DISP_INFO("lcd cabc on\n");
		}
		if (ctl->feature_val == LCD_CABC_MOVIE_ON) {
			rc = dsi_display_cmd_set_send_locked(display, DSI_CMD_SET_CABC_MOVIE_ON, NULL, USE_CPU);
			DISP_INFO("lcd cabc movie on\n");
		}
		if (ctl->feature_val == LCD_CABC_STILL_ON) {
			rc = dsi_display_cmd_set_send_locked(display, DSI_CMD_SET_CABC_STILL_ON, NULL, USE_CPU);
...
{
	g_panel->mi_panel.mi_cfg.tddi_doubleclick_flag = on;
}
