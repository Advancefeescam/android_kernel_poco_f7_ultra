 ...
 */
 ...
 #include "../../../power/supply/hq_chg/charger_class/include/hq_led_class.h"
...
/*P6 code for HQFEAT-109451 by changqi at 20250620 start*/
 #define mt6789_FLASH_CURRENT 1000   /* ma */
 #define mt6789_TORCH_CURRENT 110   /* ma */
 #define mt6789_LONGEXPOSURE_CURRENT 400   /* ma */
 #define mt6789_LONGEXPOSURE_DUTY 10
 #define mt6789_TORCH_DUTY 6
/*P6 code for HQFEAT-109451 by changqi at 20250620 end*/
 #if 0
  ...
  * mt6789 operations
  *****************************************************************************/
 /*P6 code for HQFEAT-109451 by changqi at 20250711 start*/
 static const int mt6789_current[mt6789_LEVEL_NUM] = {
         15,   30,  45, 60, 85, 100, 120,  160,  210,  260,
      300, 380, 420, 460, 520,  580,  650,  700, 740, 800,
 ...
 /*P6 code for HQFEAT-109451 by changqi at 20250711 end*/
 #if 0
 ...
 static DEVICE_ATTR(mt6789_torch, 0664, att_show_mt6789, att_store_mt6789);
 /*N6 code for HQ-307144 by xiexinli at 20230711 end*/
/*P6 code for HQFEAT-120416 by changqi at 20250615 start*/
 static ssize_t att_flash_store_mt6789(struct device *dev,
    struct device_attribute *attr,
    const char *buf, size_t count)
{
    sprintf(node_one_buf, "%s", buf);
    pr_info("echo mt6789_flash1 debug buf,   %s ", buf);
    if ((strcmp ("0", buf) == 0) || (strcmp ("0\x0a", buf) == 0)) {
        pr_info(" mt6789_flash1  0");
        mt6789_camera_set_led_flash_enable(SUBPMIC_LED1, false);
        mt6789_set_driver(0);
    } else {
        pr_info(" mt6789_flash1  1");
        mt6789_set_driver(1);
        mt6789_camera_set_led_flash_curr(SUBPMIC_LED1, mt6789_LONGEXPOSURE_CURRENT);
        mt6789_timeout_ms = 0;
        mt6789_camera_set_led_flash_enable(SUBPMIC_LED1, true);
    }
    mdelay(800);
...
static ssize_t att_flash_show_mt6789(struct device *dev, struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%s", node_one_buf);
}
static DEVICE_ATTR(mt6789_flash, 0664, att_flash_show_mt6789, att_flash_store_mt6789);
/*P6 code for HQFEAT-120416 by changqi at 20250615 end*/
 ...
 /*N6 code for HQ-307144 by xiexinli at 20230711 start*/
/*P6 code for HQFEAT-120416 by changqi at 20250615 start*/
     sysfs_create_file(&pdev->dev.kobj, &dev_attr_mt6789_torch.attr);
