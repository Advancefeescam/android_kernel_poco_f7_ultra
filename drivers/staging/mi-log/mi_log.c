					    ...
					    struct binder_transaction *t, bool pending_async)
{
	...
	q = get_task_node_by_pid(caller_task->pid);
	if (q == NULL) {
		q = task_node_alloc(GFP_ATOMIC);
		if (q == NULL) {
			raw_spin_unlock_irqrestore(&binder_log_lock, lock_flags);
			return;
		}
		q->pid = caller_task->pid;
		insert_task_node_by_pid(q);
	}

	q->fill_state = FILL_STATE_NONE;
	get_task_comm(q->delay_info.binder_target_comm, binder_proc_task);
	q->version = q->delay_info.version = 1;
	q->delay_info.pid = caller_task->pid;
	q->delay_info.binder_target_pid = binder_proc_task->tgid;

	if (binder_th_task) {
		fill_delay_info_by_task(binder_th_task, q);
		q->fill_state = FILL_STATE_ONGOING;
		q->delay_info.binder_target_thread_full = 0;
	} else {
		q->fill_state = FILL_STATE_NONE_THREAD;
		q->delay_info.binder_target_thread_full = 1;
	}
	raw_spin_unlock_irqrestore(&binder_log_lock, lock_flags);
}
...
MODULE_AUTHOR("wangzhaoliang<wangzhaoliang@xiaomi.com>");
