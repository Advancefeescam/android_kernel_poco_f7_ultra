					   ...
					   ktime_t start)
{
	...
	if (!queue_work(wq, &ipc_w->workq))
		XRISP_PR_FORCE("queue work failed, topic=0x%02x", topic);
}
...
#ifdef IPC_TRACE_FW2AP_OPEN
	u64 rpmsg_delta_time, rpmsg_start_time;

	data_offset = 9;
	memcpy(&rpmsg_start_time, data + 1, sizeof(u64));
#else
	data_offset = 1;
#endif

	msg_data = (void *)data + data_offset;
	msg_len = len - data_offset;
	topic = *(uint8_t *)data;
	topic_hdl = xrisp_ipcm_get_topic_handle(topic);
	if (unlikely(!topic_hdl)) {
		XRISP_PR_ERROR("topic 0x%02x handle not find, msg_id=%llu cnt[%llu]", topic,
...
#ifdef IPC_TRACE_FW2AP_OPEN
	rpmsg_delta_time = xr_timestamp_gettime() / 1000 - rpmsg_start_time;
	if (rpmsg_delta_time > IPC_RPMSG_MAX_TIMEOUT_US)
		XRISP_PR_WARN(
			"ipc trace: fw2ap rpmsg long time, delta_time=%lluus, hd_time=%lluus,topic=0x%02x msg_id=%llu cnt=%llu",
			rpmsg_delta_time, use_time, topic, xrisp_get_msg_id(msg_data), cnt);
#endif
	return 0;
}
...
int xrisp_msg_send_core(void *data, uint32_t len, uint8_t topic, bool data_from_user)
{
...
#ifdef IPC_TRACE_AP2FW_OPEN
	if (len >= MBOX_BUF_MAXLEN) {
		//rpmsg add timestamp
		u64 rpmsg_start_time = xr_timestamp_gettime() / 1000; //us

		memcpy(buffer + 1, &rpmsg_start_time, 8);
		data_offset = 9;
	}
#endif
	...
	if (data_from_user == true) {
		...
		if (copy_from_user((buffer + data_offset), data, len)) {
	...
	if (len < MBOX_BUF_MAXLEN) {
		...
		ipc_type = IPC_RPMSG;
		ret = xrisp_rpmsg_send(buffer, len + data_offset);
	}
