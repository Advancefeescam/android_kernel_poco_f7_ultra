...
static int Nanosic_chardev_fops_open(struct inode *inode, struct file *file)
{
    ...
    dbgprint(INFO_LEVEL, "Nanosic_chardev_fops_open\n");
...
static ssize_t Nanosic_chardev_fops_read(struct file *file, char __user *user, size_t size, loff_t *loff)
{
    ...
    {
        retval = -ERESTARTSYS;
        dbgprint(INFO_LEVEL, "wake up by restart signal %d\n", retval);
    ...
    if (client->exit == true)
    {
        dbgprint(INFO_LEVEL, "wake up by exit signal\n");
        retval = -ENODATA;
        goto _out;
    }
...
static int Nanosic_chardev_fops_release(struct inode *inode, struct file *file)
{
    ...
    dbgprint(INFO_LEVEL, "Nanosic_chardev_fops_release\n");
...
{
    dbgprint(INFO_LEVEL, "change devnode\n");

    /* Step 1: Make our node globally writable for testing */
    if (mode)
    {
        dbgprint(INFO_LEVEL, "change devnode to 666\n");
        *mode = 0666;
    }
...
int Nanosic_chardev_register(void)
{
    ...
    dbgprint(INFO_LEVEL, "chardev create ok!\n");
...
{
    dbgprint(INFO_LEVEL, "chardev release begin!\n");
    ...
    dbgprint(INFO_LEVEL, "chardev release ok!\n");
}
