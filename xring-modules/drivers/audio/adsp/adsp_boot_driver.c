...
static int check_audsys_power_off_flow_done(int timwout)
{
	...
	while (1) {
		if ((audsys_power_status != AUDSYS_ADSP_VOTE_SHUTDOWN) && (power_off_flow_done == 1)) {
			AUD_LOG_INFO(AUD_DSP, "adsp status is %d, vad poweron", audsys_power_status);
			g_adsp_boot_dev.power_status = true;
			ret = 1;
			break;
		}

		if ((audsys_power_status == AUDSYS_ADSP_VOTE_SHUTDOWN) && (power_off_flow_done == 0)) {
			AUD_LOG_DBG(AUD_DSP, "adsp already poweroff");
			break;
		}
...
static int adsp_poweron_flow(void)
{
	...
	if (audsys_power_status == AUDSYS_ADSP_VOTE_SHUTDOWN) {
		...
		if (ret < 0) {
			...
			AUD_LOG_INFO(AUD_DSP, "vad vote adsp poweron");
			return 0;
		}
	}
