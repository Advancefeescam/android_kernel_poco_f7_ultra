...
#define AF_DRVNAME "CHAROITE_GT9764AF_DRV"
...
u8 charoite_read_data(u8 addr)
{
	u8 get_byte = 0;
	i2c_read(addr, &get_byte);
	LOG_INF("[CHAROITE_GT9764AF]  get_byte %d\n", get_byte);
	return get_byte;
}
...
static int s4AF_ReadReg(unsigned short *a_pu2Result)
{
	...
	*a_pu2Result = (charoite_read_data(0x03) << 8) + (charoite_read_data(0x04) & 0xff);
	printk("[CHAROITE_GT9764AF]  s4_CHAROITE_GT9764AF_ReadReg %d\n", *a_pu2Result);
	return 0;
}
...
static int initAF(void)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		char puSendCmd3[2] = {0x07, 0x0C}; //15ms
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		/*P6 code for HQFEAT-139110 by zhangziran at 20250704 start*/
		if (a_u4Position > 750) {
			int CntStep = a_u4Position - 750;
			int Position = 750;
			int PerStep = 50;

			s4AF_WriteReg(1, 0x03, 750);
			spin_lock(g_pAF_SpinLock);
		...
		/*P6 code for HQFEAT-139110 by zhangziran at 20250704 end*/
		spin_lock(g_pAF_SpinLock);
		*g_pAF_Opened = 2;
		spin_unlock(g_pAF_SpinLock);
	}
...
/* ////////////////////////////////////////////////////////////// */
long CHAROITE_GT9764AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int CHAROITE_GT9764AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	int Ret = 0;
	...
	if (*g_pAF_Opened == 2) {
		...
		if (g_u4CurrPosition > endPos) {
			if (g_u4CurrPosition > 700) {
				...
				if (s4AF_WriteReg(1, 0x03,(unsigned short)nextPosition) == 0) {
...
int CHAROITE_GT9764AF_PowerDown(struct i2c_client *pstAF_I2Cclient,
			int *pAF_Opened)
{
...
int CHAROITE_GT9764AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int CHAROITE_GT9764AF_GetFileName(unsigned char *pFileName)
{
