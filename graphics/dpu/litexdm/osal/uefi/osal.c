...
void dpu_dts_update_u32_prop(void *fdt, int32_t offset, const char *prop_name, const uint32_t new_value)
{
	struct fdt_property *blob;
	int32_t prop_len;
	uint32_t value;
	...
	value = fdt32_to_cpu(new_value);
	blob = fdt_get_property_w(fdt, offset, prop_name, &prop_len);
	if (blob) {
		ret = fdt_setprop(fdt, offset, prop_name, (VOID *)&value, sizeof(UINT32));
		if (ret) {
			dpu_pr_err("ERROR:fdt_setprop(): %a\n", fdt_strerror(ret));
		} else {
			dpu_pr_info("update %a as %u, size:%u\n", prop_name, value, sizeof(value));
		}
	} else {
		dpu_pr_warn("get null blob for prop:%a, value:%u, new_value:%u, size:%u\n", prop_name, value, new_value);
	}
}

void dpu_dts_update_u32_array_prop(void *fdt, int32_t offset, const char *prop_name, const uint32_t *new_value, uint32_t len)
{
	struct fdt_property *blob;
	int32_t prop_len;
	uint32_t *u32_array;
	...
	u32_array = dpu_mem_alloc(len * sizeof(uint32_t));
	if (!u32_array)
		return;

	for(i=0; i<len; i++)
		*(u32_array+i) = fdt32_to_cpu(*(new_value+i));

	blob = fdt_get_property_w(fdt, offset, prop_name, &prop_len);
	if (blob) {
		ret = fdt_setprop(fdt, offset, prop_name, (VOID *)u32_array, sizeof(UINT32)*len);
		if (ret) {
			dpu_pr_err("ERROR:fdt_setprop(): %a\n", fdt_strerror(ret));
		} else {
			dpu_pr_info("update %a size:%u\n", prop_name, prop_len);
		}
	} else {
		dpu_pr_warn("get null blob for prop:%a, size:%u\n", prop_name);
	}

	dpu_mem_free((void *)u32_array);
}
