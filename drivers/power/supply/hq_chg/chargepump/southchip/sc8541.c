...
static const struct {
	...
	{"IBATOCP",   CHARGER_DEV_NOTIFY_IBATOCP},
	{"IBUSOCP",   CHARGER_DEV_NOTIFY_IBUSOCP},
	{"VBATOVP",   CHARGER_DEV_NOTIFY_VBAT_OVP},
	{"VBUSOVP",   CHARGER_DEV_NOTIFY_VBUS_OVP},
	{"VACOVP",    CHARGER_DEV_NOTIFY_VAC_OVP},
	{"TDIEFLT",   CHARGER_DEV_NOTIFY_TDIE_FLT},
};
...
struct hq_cp_sysfs_field_info {
	struct device_attribute attr;
	enum cp_property prop;
	int (*set)(struct sc8541_chip *sc,
		struct hq_cp_sysfs_field_info *attr, int val);
...
__maybe_unused static int sc8541_enable_reverse_charge(struct sc8541_chip *sc, bool en)
{
	int ret = 0;

	if (en) {
		dev_info(sc->dev, "%s:enable", __func__);
		ret = sc8541_field_write(sc, MODE, FALSE);	//workmode 2:1
		ret |= sc8541_field_write(sc, IBUS_UCP_DIS, TRUE);	//disable ibus_ucp
		ret |= sc8541_field_write(sc, VBUS_IN_RANGE_DIS, TRUE);	//disable vbus_in_range
		ret |= sc8541_field_write(sc, SS_TIMEOUT_DIS, TRUE);	//disable ss_timeout
		ret |= sc8541_field_write(sc, PMID2OUT_UVP_DIS, TRUE);	//disable pimd2out_uvp
		ret |= sc8541_enable_charge(sc, TRUE);	//enable cp
		//sc8541_dump_reg(sc);
	} else {
		dev_info(sc->dev, "%s:disable", __func__);
		ret |= sc8541_field_write(sc, IBUS_UCP_DIS, FALSE);	//enable ibus_ucp
		ret |= sc8541_field_write(sc, VBUS_IN_RANGE_DIS, FALSE);	//enable vbus_in_range
		ret |= sc8541_field_write(sc, SS_TIMEOUT_DIS, FALSE);	//enable ss_timeout
		ret |= sc8541_field_write(sc, PMID2OUT_UVP_DIS, FALSE);	//enable pimd2out_uvp
		ret |= sc8541_enable_charge(sc, FALSE);	//disable cp
	}
...
static int cp_ibus_ocp_get(struct sc8541_chip *sc,
...
static int cp_ibus_ocp_set(struct sc8541_chip *sc,
	struct hq_cp_sysfs_field_info *attr,
	int val)
{
	int ibus_ocp = val;
	...
	if (sc) {
		sc8541_set_busocp_th(sc, ibus_ocp);
	} else {
		return -ENODEV;
	}
...
static int cp_ibat_ocp_get(struct sc8541_chip *sc,
	struct hq_cp_sysfs_field_info *attr,
	int *val)
{
	return 0;
...
static int cp_ibat_ocp_set(struct sc8541_chip *sc,
	struct hq_cp_sysfs_field_info *attr,
	int val)
{
	int ibat_ocp = val;
	...
	if (sc) {
		sc8541_set_batocp_th(sc, ibat_ocp);
	} else {
		return -ENODEV;
	}
...
static int sc_sc8541_set_reverse_charge(struct chargerpump_dev *charger_pump, bool enable)
{
	int ret;
	struct sc8541_chip *sc = chargerpump_get_private(charger_pump);

	ret = sc8541_enable_reverse_charge(sc, enable);
	if (ret != 0)
		dev_info(sc->dev, "%s:error", __func__);
...
static int sc_sc8541_reset_register(struct chargerpump_dev *charger_pump)
{
	...
	if (ret < 0) {
		dev_err(sc->dev, "%s reset register failed(%d)\n", __func__, ret);
	}

	return ret;
}

static int sc_sc8541_get_chip_ok(struct chargerpump_dev *charger_pump, int *val)
{
	struct sc8541_chip *sc = chargerpump_get_private(charger_pump);
	int ret = 0;
...
static struct chargerpump_ops sc_sc8541_chargerpump_ops = {
	...
	.set_reverse_charge = sc_sc8541_set_reverse_charge,
	.dump_cp_register = sc_sc8541_dump_register,
	.reset_cp_register = sc_sc8541_reset_register,
	.get_cp_chip_ok = sc_sc8541_get_chip_ok,
};
...
static void sc8541_notify_fault_status(struct sc8541_chip *sc, int notify)
{
		...
		case SC8541_NOTIFY_VACOVP:
			...
			hq_chargerpump_notifier_call_chain(CHARGER_DEV_NOTIFY_VAC_OVP, NULL);
			break;

		case SC8541_NOTIFY_TDIEFLT:
			// charger_dev_notify(sc->chg_dev, CHARGER_DEV_NOTIFY_TDIE_FLT);
			hq_chargerpump_notifier_call_chain(CHARGER_DEV_NOTIFY_TDIE_FLT, NULL);
			break;
