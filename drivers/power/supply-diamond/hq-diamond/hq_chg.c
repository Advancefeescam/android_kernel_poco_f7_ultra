...
#include "hq_chg_iio.h"
/* m6 add charge driver start*/
#include "hq_power_supply.h"
/* m6 add charge driver end*/
...
static int thermal_mitigation[] = {
	12200000, 10000000, 9000000, 8000000, 7000000,
	6500000, 6000000, 5000000, 4000000, 3000000,
	2500000, 2300000, 2100000, 2000000, 1800000,
	1600000, 1400000, 800000, 500000, 500000,
	500000, 500000, 500000, 500000, 500000,
	100000,  500000,  500000,  500000,  500000,
};
...
int get_prop_batt_health(struct batt_chg *chg, union power_supply_propval *val)
{
	...
	else if (val->intval >= 580 && val->intval < 600)
		val->intval = POWER_SUPPLY_HEALTH_HOT;
	else if (val->intval >= 450 && val->intval < 580)
...
static int sw_battery_jeita(struct batt_chg *chg)
{
	...
	if(temp < chg->dt.jeita_temp_step0){
		...
		batt_get_iio_channel(chg, BMS, BATT_QG_VOLTAGE_NOW, &chg->batt_voltage_now);
		en = 1;
	...
	if (chg->real_type == POWER_SUPPLY_TYPE_USB_PD) {
		...
		if(chg->vbus_voltage < 5600 && type == POWER_SUPPLY_TYPE_USB_HVDCP){
			batt_set_iio_channel(chg, MAIN, MAIN_INPUT_VOLTAGE_SETTLED, 4500);
		}
	} else {
		if(type == POWER_SUPPLY_TYPE_USB) {
			if(get_boot_mode())
			...
			if(get_boot_mode())
				...
				type_cur = 3010;
			if((chg->vbus_voltage > 5600 && chg->vbus_voltage < 8450) || chg->input_limit_flag){
				input_cur = 1200;
...
static int get_real_type(struct batt_chg *chg)
{
	...
	if(chg->old_charge_type == POWER_SUPPLY_TYPE_USB_FLOAT && real_type == POWER_SUPPLY_TYPE_UNKNOWN && !chg->usb_float) {
		pr_err("get_real_type POWER_SUPPLY_TYPE_USB_FLOAT\n");
		chg->usb_float = 1;
		cancel_delayed_work(&chg->usb_type_work);
		schedule_delayed_work(&chg->usb_type_work, msecs_to_jiffies(1900));
	}

	if(type)
		chg->pd_type_cnt = 0;
	else
		chg->changed_usb_pd = 0;
	...
	if(chg->pd_active == 10 && chg->old_pd_active == 1)
		real_type = POWER_SUPPLY_TYPE_USB_PD;
	...
	/*if(get_boot_mode()) {
		static bool firstflg = true;
		if((chg->old_charge_type != real_type) && (real_type == 0)) {
			if (firstflg) {
				firstflg = false;
				msleep(3000);
				if(chg->vbus_voltage > 4000) {
					pr_err("vbus_voltage > 4000 POWER_SUPPLY_TYPE_USB_FLOAT\n");
					real_type = POWER_SUPPLY_TYPE_USB_FLOAT;
				}
			}
		}
		pr_err("real_type %d, old_charge_type %d, vbus_voltage %d\n",real_type, chg->old_charge_type, chg->vbus_voltage);
	}*/
	chg->real_type = real_type;
	if(chg->old_charge_type != real_type) {
		if (real_type == POWER_SUPPLY_TYPE_USB_HVDCP ||
			real_type == POWER_SUPPLY_TYPE_USB_DCP || real_type == POWER_SUPPLY_TYPE_USB_PD) {
			batt_set_iio_channel(chg, CP_MASTER, CHARGE_PUMP_SC_PRESENT, true);
			batt_set_iio_channel(chg, CP_SLAVE, CHARGE_PUMP_SC_PRESENT, true);
		}
		chg->update_cont = 15;
		chg->pd_type_cnt = 0;
	...
	pr_err("real_type=%d,old_charge_type=%d,pd_active=%d,vbus_cnt=%d,vbus_voltage=%d\n",
		chg->real_type, chg->old_charge_type, chg->pd_active, chg->vbus_cnt, chg->vbus_voltage);
	return real_type;
}
...
static int get_usb_real_type(struct batt_chg *chg)
{
	...
	if(chg->otg_enable || (chg->vbus_cnt > 3))
		real_type = 0;
	if(!real_type) {
		chg->pd_auth = 0;
		chg->therm_step = 0;
...
#if 0
	if (val->intval == 25)
		chg->is25thermmitigflg = true;
	else if (val->intval == 29)
		chg->is25thermmitigflg = false;

	if (chg->is25thermmitigflg)
		chg->system_temp_level = 25;
	else
 #endif
	chg->system_temp_level = val->intval;
	cur = thermal_mitigation[chg->system_temp_level] / 1000;
	if(chg->real_type)
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TYPE:
		real_type = get_real_type(chg);
		batt_get_iio_channel(chg, MAIN, MAIN_CHARGER_TYPE, &type);
		if (type == POWER_SUPPLY_TYPE_USB || type == POWER_SUPPLY_TYPE_USB_CDP) {
			if (real_type == POWER_SUPPLY_TYPE_USB_PD && chg->changed_usb_pd < 3) {
		...
		switch(val->intval)
			{
				...
				batt_set_iio_channel(chip, MAIN, MAIN_OTG_ENABLE, val->intval);
...
static void batt_chg_main(struct work_struct *work)
{
	...
	if(chg->mishow_flag && (chg->real_type == POWER_SUPPLY_TYPE_USB_DCP || chg->real_type == POWER_SUPPLY_TYPE_USB_FLOAT)) {
		val = 1;
					...
					struct class_attribute *attr, char *buf)
{
	...
	val += temp;
	batt_get_iio_channel(chg, MAIN, MAIN_CHAGER_CURRENT, &temp);
	val += temp;
...
static ssize_t enable_otg_show(struct class *c,
					struct class_attribute *attr, char *buf)
{
	struct batt_chg *chg = container_of(c, struct batt_chg,
						battery_class);
	...
	batt_get_iio_channel(chg, MAIN, MAIN_OTG_ENABLE, &val);

	return scnprintf(buf, PAGE_SIZE, "%d\n", val);
}
static CLASS_ATTR_RO(enable_otg);

static ssize_t main_ibus_show(struct class *c,
					struct class_attribute *attr, char *buf)
{
	struct batt_chg *chg = container_of(c, struct batt_chg,
						battery_class);
	...
	batt_get_iio_channel(chg, MAIN, MAIN_CHAGER_CURRENT, &val);

	return scnprintf(buf, PAGE_SIZE, "%d\n", val);
}
static CLASS_ATTR_RO(main_ibus);
		...
		struct device_attribute *attr, char *buf)
{
	...
	pr_err("chg->pd_active %d, chg->old_pd_active %d, type  %d\n", g_batt_chg->pd_active, g_batt_chg->old_pd_active, type);
	if(real_type == POWER_SUPPLY_TYPE_USB_PD && type){
		if(type == POWER_SUPPLY_TYPE_USB)
			real_type = POWER_SUPPLY_TYPE_USB;
		else if(g_batt_chg->pd_active == 1 && g_batt_chg->old_pd_active == 10) {
			real_type = POWER_SUPPLY_TYPE_USB;
			batt_set_iio_channel(g_batt_chg, MAIN, MAIN_OTG_ENABLE, 2);
		} else
			real_type = POWER_SUPPLY_TYPE_USB_PD;
	} else {
	...
	if(real_type == POWER_SUPPLY_TYPE_USB_PD && type == POWER_SUPPLY_TYPE_USB_FLOAT){
		pr_err("test for pd \n");
		batt_set_iio_channel(g_batt_chg, MAIN, MAIN_OTG_ENABLE, 2);
	}
...
/*M6 create symlink from /sys/class/mtk-battery to /sys/class/qcom-battery by xuyechen at 2022/10/31 start*/
#define QCOM_BATT_CLASS_NAME "qcom-battery"
...
/*M6 create symlink from /sys/class/mtk-battery to /sys/class/qcom-battery by xuyechen at 2022/10/31 end*/
static int wt_init_dev_class(struct batt_chg *chg)
{
	...
	chg->battery_class.name = "mtk-battery";
	...
	/*M6 create symlink from /sys/class/mtk-battery to /sys/class/qcom-battery by xuyechen at 2022/10/31 start*/
	rc = sysfs_create_link(&chg->battery_class.p->subsys.kobj.kset->kobj,&chg->battery_class.p->subsys.kobj, QCOM_BATT_CLASS_NAME);
	if (rc < 0) {
		pr_err("Failed to create symlink qcom-battery rc=%d\n", rc);
	}
	/*M6 create symlink from /sys/class/mtk-battery to /sys/class/qcom-battery by xuyechen at 2022/10/31 end*/
	return rc;
}
		...
		int val2, long mask)
{
	...
	case PSY_IIO_PD_ACTIVE:
		if(chg->pd_active != val1){
			chg->old_pd_active = chg->pd_active;
		}
		chg->pd_active = val1;
		chg->update_cont = 15;
		...
		power_supply_changed(chg->usb_psy);
		power_supply_changed(chg->batt_psy);
		pr_err("chg->pd_active %d, chg->old_pd_active %d\n", chg->pd_active, chg->old_pd_active);
		break;
	...
	case PSY_IIO_OTG_ENABLE:
		...
		if (get_boot_mode()) {
			chg->otg_enable = 0;
			pr_err("power off not enable otg %d\n", chg->otg_enable);
		} else {
...
static int batt_chg_probe(struct platform_device *pdev)
{
	...
	if (rc < 0) {
		pr_err("Failed to initialize batt_config rc=%d\n", rc);
		return rc;
	}

	rc = batt_node_map(batt_chg);
	if (rc < 0) {
		pr_err("Failed to initialize batt_node_map rc=%d\n", rc);
		return rc;
	}
