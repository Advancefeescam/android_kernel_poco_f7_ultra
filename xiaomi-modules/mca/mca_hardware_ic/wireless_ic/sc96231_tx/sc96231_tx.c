...
//--------------------------ops functions------------------------
static int sc96231_download_fw_from_bin(void *data)
{
    int ret = 0;
    ...
    ret = sc96231_get_image_fwver(sc->fw_bin, sc->fw_bin_length, &bin_image_ver);
    mca_log_info("sc96231 current firmware version=%x, new bin firmware version=%x\n", cur_ver,
        bin_image_ver);

    mca_log_info("sc96231 start download mtp from bin\n");
    //need check
    ...
    if (ret < 0) {
        ...
        mca_log_err("sc96231_mtp_program: from bin failed, update fw version: 0xfe.\n");
    } else {
        mca_log_info("sc96231 download mtp from bin successfully\n");
    }

    return ret;
}

static int sc96231_set_fw_bin(const char *buf, int count, void *data)
{
...
static int sc96231_set_external_boost_enable(bool enable, void *data)
{
	...
	if (enable) {
		if (gpio_is_valid(chip->reverse_txon_gpio)) {
			...
			mca_log_err("reverse txon gpio value is %d\n", gpio_enable_val);
...
static struct platform_class_wireless_ops sc96231_wls_ops = {
    ...
    .wls_set_fw_bin = sc96231_set_fw_bin,
    .wls_download_fw_from_bin = sc96231_download_fw_from_bin,
    ...
    .wls_get_pen_soc = sc96231_get_pen_soc,
...
static int sc96231_parse_fw_data(struct sc96231 *chip)
{
    ...
    case WLS_CHIP_VENDOR_SC96231:
        chip->fw_data_ptr = SC96231_FIRMWARE[chip->fw_version_index];
        chip->fw_data_size = sizeof(SC96231_FIRMWARE[chip->fw_version_index]);
        break;
    default:
        chip->fw_data_ptr = SC96231_FIRMWARE[chip->fw_version_index];
        chip->fw_data_size = sizeof(SC96231_FIRMWARE[chip->fw_version_index]);
...
#else
static int sc96231_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
#endif
{
    int ret = 0;
    struct sc96231 *chip;
    ...
    ret = platform_class_wireless_register_ops(chip->rx_role, chip, &sc96231_wls_ops);
    if (ret) {
...
MODULE_AUTHOR("zhaojian5@xiaomi.com");
