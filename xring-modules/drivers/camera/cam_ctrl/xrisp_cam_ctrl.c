...
static int xrisp_isp_power_up(struct xrisp_power_up *powerup_set)
{
	...
	if (ret) {
		edr_drv_submit_api(EDR_MCU, EDR_FW_BOOT_ERROR,
				   EDR_RAMLOG | EDR_LOGCAT | EDR_KERNEL_KMSG | EDR_OFFLINELOG, 1, 0);
		return ret;
	}
...
int xrisp_ipc_cpc_handler(void *data, unsigned int len)
{
	...
	case DRV_ISP_SERVICE_ONLINE: {
		...
		XRISP_PR_FORCE("isp service online done");
	} break;
