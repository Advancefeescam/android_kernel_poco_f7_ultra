...
{
	int ret = 0;
	static int pre_verify_state;
	//union power_supply_propval val = {0,};
	//struct power_supply *usb_psy = NULL;

	if (verify_in_process == 0 && pre_verify_state == 1) {
		dev->verify_done = true;
	}

	adapter_err("[%s] pd verify in process:%d, pre verify state:%d, pd verify done:%d\n",
		__func__, verify_in_process, pre_verify_state, dev->verify_done);

	pre_verify_state = verify_in_process;
/*
...
static int xm_pd_adapter_probe(struct platform_device *pdev)
{
	...
	if (info->pd_adapter) {
		...
		info->pd_adapter->verify_done = false;
	}
