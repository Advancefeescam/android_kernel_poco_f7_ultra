...
static void pdm_multi_mode_switch(struct chargerpump_policy *policy)
{
   ...
   if (policy == NULL)
	...
	hq_info("ibat_request = %d, 2_1 mode = %d, 1_1 mode = %d, soc = %d\n",
		...
		if (manager != NULL){
			...
			if(manager->pps_fast_mode){
				if (policy->switch2_1_enable && (!((policy->m_cp_mode == 0) && (policy->s_cp_mode == 0)))) {
					...
					hq_info("low_fast working, set switch2_1_enable\n");
				}
				hq_info("low_fast working, cp_mode not switch\n");
				return false;
			}
		}
		else
...
static int chargerpump_policy_update_chg_info(struct chargerpump_policy *policy)
{
	...
	master_cp_ibat = info->ibus * 2;
	if (master_cp_ibat > 12700) {
		hq_chargermanager_notifier_call_chain(CHG_FW_EVT_IBAT_OCP, NULL);
	}
...
static int chargerpump_policy_state_enable_chargerpump(struct chargerpump_policy *policy)
{
	...
	if (get_cp_en) {
		...
		hq_chargermanager_notifier_call_chain(CHG_FW_EVT_CP_EN, &get_cp_en);
...
static int __chargerpump_cc_cv_algo(struct chargerpump_policy *policy)
{
...
#if IS_ENABLED(CONFIG_XM_FG_I2C_ERR)
	if(fuel_gauge_check_fg_status(policy->fuel_gauge))
	...
	if (policy->lpd_detected) {
		return PM_ALGO_RET_RUST_DETECTION;
	}
#endif
	...
	if ((info->vbat > vbat_lmt) && (vbat_lmt < final_cv - 5)){
		if ((info->last_vbat > vbat_lmt) && (info->last_vbat >= info->vbat)) {
			step_vbat = 0;
			hq_info("To allow the current to decrease gradually, slowly adjust down the Vbus.\n");
		} else {
			step_vbat = -cfg->step_volt;
		}
	}

	info->last_vbat = info->vbat;
	...
	hq_info("cp algo [blo_200 blo_150] %d ,%d\n",
				blo_200, blo_150);
...
#if IS_ENABLED(CONFIG_RUST_DETECTION)
	if (ret == PM_ALGO_RET_RUST_DETECTION) {
		policy->sm = PM_STATE_CHARGERPUMP_EXIT;
		policy->recover = false;
		hq_info("lpd is detected, exit state machine.\n");
	}
#endif
...
static int chargepump_policy_parse_dt(struct chargerpump_policy *policy, struct platform_device *pdev)
{
	int ret = 0;
	struct device_node *np = pdev->dev.of_node;

	if (!policy) {
		hq_err("charger pump policy is null pointer\n");
		return -ENOMEM;
	}

	ret |= of_property_read_u32(np, "hq_cp_policy,cv_offset", &policy->cfg.cv_offset);
	ret |= of_property_read_u32(np, "hq_cp_policy,exit_cc", &policy->cfg.exit_cc);
	ret |= of_property_read_u32(np, "hq_cp_policy,step_volt", &policy->cfg.step_volt);
	ret |= of_property_read_u32(np, "hq_cp_policy,warm_temp", &policy->cfg.warm_temp);
	ret |= of_property_read_u32(np, "hq_cp_policy,max_request_volt", &policy->cfg.max_request_volt);
	ret |= of_property_read_u32(np, "hq_cp_policy,min_request_volt", &policy->cfg.min_request_volt);
	ret |= of_property_read_u32(np, "hq_cp_policy,max_request_curr", &policy->cfg.max_request_curr);
	ret |= of_property_read_u32(np, "hq_cp_policy,min_request_curr", &policy->cfg.min_request_curr);
	ret |= of_property_read_u32(np, "hq_cp_policy,max_ibat", &policy->cfg.max_ibat);
	ret |= of_property_read_u32(np, "hq_cp_policy,max_vbat", &policy->cfg.max_vbat);
	ret |= of_property_read_u32(np, "hq_cp_policy,min_vbat", &policy->cfg.min_vbat);
	if (ret) {
		hq_err("failed to parse policy configurations from device tree\n");
		return ret;
	}

	policy->cfg.s_cp_enable = of_property_read_bool(np, "hq_cp_policy,s_cp_enable");

	hq_info("policy cfg - cv_offset:%u, exit_cc:%u, step_volt:%u, warm_temp:%u\n",
			policy->cfg.cv_offset, policy->cfg.exit_cc, policy->cfg.step_volt, policy->cfg.warm_temp);
	hq_info("policy cfg - max_request_volt:%u, min_request_volt:%u, max_request_curr:%u, min_request_curr:%u\n",
			policy->cfg.max_request_volt, policy->cfg.min_request_volt,
			policy->cfg.max_request_curr, policy->cfg.min_request_curr);
	hq_info("policy cfg - max_ibat:%u, max_vbat:%u, min_vbat:%u, s_cp_enable:%d\n",
...
static int chargerpump_policy_probe(struct platform_device *pdev)
{
	...
	// Set policy configurations to defalut configuration
	// TODO: remove as device tree configurations will overlay these
	memcpy(&g_policy->cfg, &default_cfg, sizeof(default_cfg));

	chargepump_policy_parse_dt(g_policy, pdev);
