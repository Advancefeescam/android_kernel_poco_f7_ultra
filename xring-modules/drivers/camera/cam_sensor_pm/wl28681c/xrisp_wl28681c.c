...
static int xrisp_wl28681c_is_enabled(struct regulator_dev *rdev)
{
	if (!rdev) {
		pr_err("rdev NULL ERR!");
		return -EINVAL;
	...
	if ((!strcmp(rdev->desc->name, "WL0_LDO2") && (rdev->use_count == 0)) ||
		(!strcmp(rdev->desc->name, "WL0_LDO3") && (rdev->use_count == 0)) ||
		(!strcmp(rdev->desc->name, "WL0_LDO4") && (rdev->use_count == 0))) {
		pr_info("%s is not used.", rdev->desc->name);
		return 0;
	}
...
static const struct regulator_ops wl28681c_reg_control_ops = {
	...
	.is_enabled	= xrisp_wl28681c_is_enabled,
