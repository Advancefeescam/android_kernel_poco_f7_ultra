...
{
	if (page0_data[0] == SWD_SN)		//SWD--first_supplier
		*bat_id = BATTERY_VENDOR_FIRST;
	else if (page0_data[0] == GY_SN1 || page0_data[0] == GY_SN2)  //GY--second_supplier
...
static int st_set_ui_soh(struct auth_device *auth_dev, u8 *ui_soh_data, int len)
{
	int i = 0;
	u8 ui_soh[32] = {0};
	stick_ReturnCode_t ret;
...
static int st_get_raw_soh(struct auth_device *auth_dev, int *raw_soh)
{
	u8 page_data[32] = {0};
	u8 retry_cnt = 10;
	u8 exit_retry_cnt = 10;
	...
	ret = stickapi_read_page(page_data, PAGE_INDEX8, 16);
	if (ret != 0) {
		pr_err("%s read fail\n", __func__);
		return ret;
	}

	ret = stickapi_read_page(&page_data[16], PAGE_INDEX9, 16);
	if (ret != 0) {
		pr_err("%s read fail\n", __func__);
		return ret;
	}
...
static int st_set_raw_soh(struct auth_device *auth_dev, int raw_soh)
{
	u8 page_data[32] = {0};
	stick_ReturnCode_t ret;
	u8 retry_cnt = 10;
	...
	page_data[15] = raw_soh;
	ret = stickapi_write_page(page_data, PAGE_INDEX8, 16);
	if (ret != 0) {
		pr_err("%s write fail\n", __func__);
		return ret;
	}

	ret = stickapi_write_page(&page_data[16], PAGE_INDEX9, 16);
	if (ret != 0) {
		pr_err("%s write fail\n", __func__);
		return ret;
	}
...
static struct auth_ops st_auth_ops = {
	...
	.get_raw_soh = st_get_raw_soh,
	.set_raw_soh = st_set_raw_soh,
};
