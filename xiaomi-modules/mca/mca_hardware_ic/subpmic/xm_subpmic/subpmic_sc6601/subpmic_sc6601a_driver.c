...
static int set_typec_orientation_ready(struct subpmic_chg_device *sc, bool ready)
{
	sc->typec_orientation_ready = ready;
...
static void usb_queue_device_on(struct subpmic_chg_device *sc)
{
	...
	if (pd_active || sc->vbus_type == XM_CHARGER_TYPE_SDP || sc->vbus_type == XM_CHARGER_TYPE_CDP) {
		mca_log_info("wait sc6601_type_c_ready\n");
		while(!sc->typec_orientation_ready && (wait_counter < 10)) {
			msleep(300);
			wait_counter++;
		};
		mca_log_info("sc6601_type_c_ready ok\n");
...
static int subpmic_chg_get_online(struct subpmic_chg_device *sc, int *en)
{
	bool usb_present;
	int en_temp = 0;
	int wls_online = 0, wls_otg_boost_en = 0, pogo_online = 0;
	int pd_active = 0;
	...
	platform_class_cp_get_pogo_online(CP_ROLE_MASTER, &pogo_online);
	if (wls_otg_boost_en)
		return 0;

	en_temp = sc->state.online | usb_present | pogo_online;
	...
	*en = en_temp;
	if (!(sc->state.online | usb_present | wls_online | pogo_online))
		sc6601_ctrl_vbus_path(sc, RESUME_BUCK_VBUS_GPIO_VAL);
	mca_log_err("buck vbus online = %d, cp vbus online = %d, vbus online = %d, wls online = %d, pd_active = %d, pogo_online = %d\n",
				sc->state.online, usb_present, *en, wls_online, pd_active, pogo_online);
	return 0;
}
...
static int sc6601_buckchg_set_typec_orientation_ready(void *data, bool ready)
{
	struct subpmic_chg_device *sc = (struct subpmic_chg_device *)data;

	set_typec_orientation_ready(sc, ready);
...
{
	struct subpmic_chg_device *sc = (struct subpmic_chg_device *)data;
	int keyboard_plugin = 0;

	if (en) {
		if (sc->otg_boost_src == 1) {    // otg boost src is gpio
			if (sc->pogo_support) {
				platform_class_cp_get_keyboard_plugin(CP_ROLE_MASTER, &keyboard_plugin);
				if (!keyboard_plugin) {
					platform_class_cp_enable_wpcgate(CP_ROLE_MASTER, true);
				}
				platform_class_cp_enable_ovpgate(CP_ROLE_MASTER, false);
				platform_class_cp_enable_acdrv_manual(CP_ROLE_MASTER, true);
				platform_class_cp_set_mode(CP_ROLE_MASTER, CP_MODE_FORWARD_4_1);
			} else {
		...
		if (sc->otg_boost_src == 1) {    // otg boost src is gpio
			...
			if (sc->pogo_support) {
				...
				if (keyboard_plugin) {
					...
					platform_class_cp_enable_acdrv_manual(CP_ROLE_MASTER, false);
				}
			} else {
				platform_class_cp_enable_ovpgate(CP_ROLE_MASTER, true);
			}
...
//	.set_otg = sc6601_pd_request_otg,
	...
	.set_typec_orientation_ready = sc6601_buckchg_set_typec_orientation_ready,
...
static int subpmic_sc6601a_drv_probe(struct platform_device *pdev)
{
	...
	ret = mca_parse_dts_u32(sc->dev->of_node, "pogo_support", &sc->pogo_support, 0);
	if (ret)
	{
		sc->pogo_support = 0;
		mca_log_err("%s failed to get pogo_support %d\n", __func__, sc->pogo_support);
	}
