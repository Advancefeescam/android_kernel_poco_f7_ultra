...
// chargerpump notifer intertface to other module
static BLOCKING_NOTIFIER_HEAD(chargerpump_notifier);

int hq_chargerpump_notifier_register(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&chargerpump_notifier, nb);
}
EXPORT_SYMBOL_GPL(hq_chargerpump_notifier_register);

int hq_chargerpump_notifier_unregister(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&chargerpump_notifier, nb);
}
EXPORT_SYMBOL_GPL(hq_chargerpump_notifier_unregister);

int hq_chargerpump_notifier_call_chain(unsigned long val, void *v)
{
	return blocking_notifier_call_chain(&chargerpump_notifier, val, v);
}
EXPORT_SYMBOL_GPL(hq_chargerpump_notifier_call_chain);
