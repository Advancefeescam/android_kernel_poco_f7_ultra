...
int edr_dump_log_complete(void)
{
	int timeout = 2000;

	while (timeout > 0 && (atomic_read(&sync_done) != 1)) {
		timeout -= 1;
		usleep_range(1000, 1001);
	}

	if (atomic_read(&sync_done) == 0) {
		edr_err("wait ramlog catch timeout");
		return -ETIMEDOUT;
	}

	atomic_set(&sync_done, 0);
	edr_info("wait ramlog catch complete");
	return 0;
}
