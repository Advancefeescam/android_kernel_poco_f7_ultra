...
# function description:
#      copy platform/<common/o1> to xring-modules/include
link_header_file() {
    ...
    ln -s -f ${LOCAL_PATH}/../platform/O1 ${XRING_MODULES_PATH}/include/dt-bindings/xring/platform-specific || exit 1
    ln -s -f ${LOCAL_PATH}/../platform/O1/common ${XRING_MODULES_PATH}/include/dt-bindings/xring  || exit 1
...
create_pack_dtb_img(){
    echo "Building  dtbo ..."
    cp ${XRING_CONFIG_PATH}/${SOC_NAME}/dtboimg.cfg ${KERNEL_DIST_PATH}/dtboimg.cfg
...
build_kernel_6_6() {
    SOC_NAME=$(echo $TARGET_PLATFORM | awk -F"_" '{print $NF}')
    ...
    # step 1: copy platform/<common/O1> to xring-modules/include
    link_header_file
    ...
    if [[ $TARGET_PRODUCT == *"emu" || $TARGET_PRODUCT == *"fpga" || $TARGET_PRODUCT == *"asic" ]]; then
        ...
        FSTAB_COPY_PATH=${XRING_CONFIG_PATH}/xiaomi/${PRODUCT_TYPE}
    ...
    if [ "${SOC_NAME}" = "o1" ]; then
        if [ -z ${is_product} ]; then
            PRODUCT_FLAGS=""
            if [[ $IMG_TYPE == "f3_uft" || $IMG_TYPE == "broadway" ]]; then
                build_target="${TARGET_PLATFORM}_${IMG_TYPE}"
            else
