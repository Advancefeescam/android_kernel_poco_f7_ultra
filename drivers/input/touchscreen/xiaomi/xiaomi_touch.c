...
int get_bms_temp_common(void)
{
	...
	if (!battery) {
		...
		if (!battery) {
			pr_err("%s can't find bms and battery", __func__);
			return -EINVAL;
		}
	}
	...
	if (ret) {
		pr_err("%s can't read battery temp", __func__);
		return -EINVAL;
	}
...
static ssize_t set_thermal_temp_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return 0;
}
static ssize_t set_thermal_temp_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	int cur_temp = 0, cur_temp0 = 0;
	...
	if (pdata->touch_data[0]->set_thermal_temp) {
		pdata->touch_data[0]->set_thermal_temp(cur_temp, true);
		pr_info("%s cur_temp:%d\n", __func__, cur_temp);
	} else {
		pr_info("%s has not set_thermal_temp\n", __func__);
	}
...
int xiaomitouch_register_modedata(int touchId, struct xiaomi_touch_interface *data)
{
	...
	/* P6 code for HQFEAT-118700 by p-zhaobeidou3 at 2025/6/27 end */
	/*P6 code for BUGP6-605 by p-zhaobeidou3 at 2025/7/21 start*/
	if (data->set_thermal_temp)
		touch_data->set_thermal_temp = data->set_thermal_temp;
	/*P6 code for BUGP6-605 by p-zhaobeidou3 at 2025/7/21 end*/
...
/*P6 code for BUGP6-605 by p-zhaobeidou3 at 2025/7/21 start*/
void enable_temperature_detection_func(void)
{
	int cur_temp = 0, cur_temp0 = 0;
	...
	if (!touch_pdata || !touch_pdata->touch_data[0]) {
		pr_err("%s touch_pdata is null!\n", __func__);
		return;
	}

	if (touch_pdata->touch_data[0]->set_thermal_temp) {
		touch_pdata->touch_data[0]->set_thermal_temp(cur_temp, true);
		pr_info("%s cur_temp:%d\n", __func__, cur_temp);
	} else {
		pr_info("%s has not set_thermal_temp\n", __func__);
	}
...
/*P6 code for BUGP6-605 by p-zhaobeidou3 at 2025/7/21 start*/
static DEVICE_ATTR(set_thermal_temp, (0664), set_thermal_temp_show, set_thermal_temp_store);
/*P6 code for BUGP6-605 by p-zhaobeidou3 at 2025/7/21 end*/
...
static struct attribute *touch_attr_group[] = {
	...
	/*P6 code for HQFEAT-118700 by p-zhaobeidou3 at 2025/6/27 end*/
	/*P6 code for BUGP6-605 by p-zhaobeidou3 at 2025/7/21 start*/
	&dev_attr_set_thermal_temp.attr,
	/*P6 code for BUGP6-605 by p-zhaobeidou3 at 2025/7/21 end*/
	NULL,
};
