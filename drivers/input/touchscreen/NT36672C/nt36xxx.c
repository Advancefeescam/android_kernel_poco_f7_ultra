...
EXPORT_SYMBOL(tp_charger_status);
...
#endif
/*L19A code for HQ-198798 by gaoxue at 2022/4/21 start*/
...
#endif
/*L19A code for HQ-198798 by gaoxue at 2022/4/21 end*/
...
EXPORT_SYMBOL(nvt_read_fw_history);
...
{
	return ts->lockdowninfo[0];
}

static u8 nvt_panel_display_read(void)
{
	return ts->lockdowninfo[1];
}

static u8 nvt_panel_color_read(void)
{
	return ts->lockdowninfo[2];
}
...
static void nvt_charger_notify_work(struct work_struct *work)
{
	...
	if (USB_DETECT_IN == ts->usb_plug_status) {
		NVT_LOG("USB plug in");
		nvt_set_charger_switch(USB_DETECT_IN);
	} else if (USB_DETECT_OUT == ts->usb_plug_status) {
		NVT_LOG("USB plug out");
		nvt_set_charger_switch(USB_DETECT_OUT);
	}else{
...
/*L19A code for HQ-235102 by lizongrui at 2022/12/12 start*/
		...
		if (size != 8 ) {
			NVT_ERR("wrong lockdowninfo length:%lu\n", size);
		} else {
			memcpy(ts->lockdowninfo, tp_lockdown_info, 8);
			NVT_LOG("%s tp_lockdown_info : %hhx,%hhx,%hhx,%hhx,%hhx,%hhx,%hhx,%hhx\n", __func__,
				ts->lockdowninfo[0],
				ts->lockdowninfo[1],
				ts->lockdowninfo[2],
				ts->lockdowninfo[3],
...
/*BSP.Tp - 2020.11.05 -add NVT_LOCKDOWN - end*/
	...
	ret = mtk_disp_notifier_register("NVT Touch", &ts->fb_notif);
	if (ret) {
...
return:
	Executive outcomes. 0---succeed.
*******************************************************/
__attribute__((unused)) static int32_t nvt_ts_suspend(struct device *dev)
{
        /*L19A code for HQ-198762 by zhangkexin at 2022/04/25 start*/
	uint8_t buf[4] = {0};
#if MT_PROTOCOL_B
...
return:
	Executive outcomes. 0---succeed.
*******************************************************/
__attribute__((unused)) static int32_t nvt_ts_resume(struct device *dev)
{
	if (bTouchIsAwake) {
...
static int32_t __init nvt_driver_init(void)
{
	...
	NVT_LOG("lcmname %50.50s,length:%lu", lcmname, size);
	if (!(strcmp(lcmname, "dsi_panel_l19a_31_03_0b_dsc_vdo_lcm_drv"))) {
		NVT_LOG("Func:%s lcmname:%d", __func__, lcmname);
		return 0;
	}
	//---add spi driver---
