		...
		struct panel_drv_private *priv)
{
	...
	for (i = 0; i < DSI_CMD_SET_MAX; i++) {
		...
		if (!cmd_name || !cmd_state_name) {
			dpu_pr_debug("%d-th cmd/cmd state name is null\n", i);
			continue;
		}
...
static void dsi_panel_parse_lockdwon_config(void *fdt, int32_t offset,
		struct panel_drv_private *priv)
{
	struct panel_lockdown_config *lockdown_config;

	const char *cmd_name = "dsi-panel-lockdown-info-read-command";
	const char *cmd_tx_name = "dsi-panel-lockdown-info-read-pre-tx-command";
	int ret = 0;

	lockdown_config = &priv->lockdown_config;
	lockdown_config->lockdown_cmds_rlen = 0;
	if (priv->is_fake_panel)
		return;

	ret = dsi_panel_parse_cmd_set(fdt, offset, &lockdown_config->id_cmd, cmd_name,
			"dsi-panel-lockdown-info-read-command-state");
	if (ret) {
		dpu_pr_debug("empty cmd set:%a\n", cmd_name);
		lockdown_config->id_cmd.num_cmds = 0;
		lockdown_config->id_cmd.cmds = NULL;
		return;
	}

	ret = dpu_dts_parse_u32(fdt, offset, "dsi-panel-lockdown-info-read-length",
			&lockdown_config->lockdown_cmds_rlen);
	if (ret) {
		dpu_pr_err("fail lockdown to get lockdown length,return\n");
		return;
	}

	ret = dsi_panel_parse_cmd_set(fdt, offset, &lockdown_config->lockdown_tx_cmd, cmd_tx_name,
			"dsi-panel-lockdown-info-read-pre-tx-command");
	if (ret) {
		dpu_pr_err("fail lockdown to get lockdown length\n");
	}
}

static void dsi_panel_parse_wp_info_config(void *fdt, int32_t offset,
		struct panel_drv_private *priv)
{
	struct panel_wp_info_config *wp_info_config;

	const char *cmd_name = "mi,dsi-panel-wp-read-command";
	const char *cmd_tx_name = "mi,dsi-panel-wp-read-pre-tx-command";
	int ret = 0;

	wp_info_config = &priv->wp_info_config;
	wp_info_config->wp_info_cmds_rlen = 0;
	if (priv->is_fake_panel)
		return;

	ret = dsi_panel_parse_cmd_set(fdt, offset, &wp_info_config->id_cmd, cmd_name,
			"mi,dsi-panel-wp-read-command-state");
	if (ret) {
		dpu_pr_info("empty cmd set:%a\n", cmd_name);
		wp_info_config->id_cmd.num_cmds = 0;
		wp_info_config->id_cmd.cmds = NULL;
		return;
	}

	ret = dpu_dts_parse_u32(fdt, offset, "mi,dsi-panel-wp-read-length",
			&wp_info_config->wp_info_cmds_rlen);
	if (ret) {
		dpu_pr_err("fail wp_info to get wp_info length,return\n");
		return;
	}

	ret = dsi_panel_parse_cmd_set(fdt, offset, &wp_info_config->wp_info_tx_cmd, cmd_tx_name,
			"mi,dsi-panel-wp-read-pre-tx-command");
	if (ret) {
		dpu_pr_err("fail wp_info to get wp_info length\n");
	}

	wp_info_config->return_buf = dpu_mem_alloc(wp_info_config->wp_info_cmds_rlen);
	if (!wp_info_config->return_buf) {
		dpu_pr_err("fail wp_info to get wp_info length\n");
		return;
	}
}
		...
		const char *panel_node_name)
{
	...
	dsi_panel_parse_lockdwon_config(fdt, offset, priv);

	dsi_panel_parse_wp_info_config(fdt, offset, priv);
...
static void dsi_panel_lockdown_info_config_release(struct panel_drv_private *priv)
{
	dpu_pr_debug("+\n");
	if (priv->is_fake_panel)
		return;

	dsi_panel_cmd_set_release(&priv->lockdown_config.id_cmd);
	dsi_panel_cmd_set_release(&priv->lockdown_config.lockdown_tx_cmd);
	dpu_pr_debug("-\n");
}

static void dsi_panel_wp_info_config_release(struct panel_drv_private *priv)
{
	dpu_pr_debug("+\n");
	if (priv->is_fake_panel)
		return;

	dsi_panel_cmd_set_release(&priv->wp_info_config.id_cmd);
	dsi_panel_cmd_set_release(&priv->wp_info_config.wp_info_tx_cmd);
	dpu_mem_free(priv->wp_info_config.return_buf);
	dpu_pr_debug("-\n");
}
...
{
	dpu_pr_debug("+\n");
	dsi_panel_lockdown_info_config_release(priv);
	dsi_panel_wp_info_config_release(priv);
