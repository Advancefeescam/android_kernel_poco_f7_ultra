...
int chargerpump_get_chip_ok(struct chargerpump_dev *chargerpump, int *value)
{
	if (!chargerpump || !chargerpump->ops)
		return -EINVAL;
	if (chargerpump->ops->get_cp_chip_ok == NULL)
		return -EOPNOTSUPP;
	return chargerpump->ops->get_cp_chip_ok(chargerpump, value);
}
EXPORT_SYMBOL(chargerpump_get_chip_ok);
...
int chargerpump_set_reverse_charge(struct chargerpump_dev *chargerpump, bool enable)
{
	if (!chargerpump || !chargerpump->ops)
		return -EINVAL;
	if (chargerpump->ops->set_reverse_charge == NULL)
		return -EOPNOTSUPP;
	return chargerpump->ops->set_reverse_charge(chargerpump, enable);
}
EXPORT_SYMBOL_GPL(chargerpump_set_reverse_charge);
