...
void driver_update_touch_mode(s8 touch_id, int _touch_mode[Touch_Mode_NUM], long update_mode_mask)
{
	int i = 0;
...
{
	if (touch_id < 0 || touch_id >= MAX_TOUCH_PANEL_COUNT || mode < 0 || mode >= Touch_Mode_NUM)
		return -1;
...
void xiaomi_notify_sensorhub_enable(s8 touch_id, bool status, bool force)
{
	...
	if (status) {
		...
		if ((xiaomi_get_gesture_type(touch_id) & GESTURE_LONGPRESS_EVENT) != 0) {
			...
			add_common_data_to_buf(0, SET_CUR_VALUE, Touch_Suspend, 1, &ssh_value);
...
static void xiaomi_touch_grid_update_work(struct work_struct *work)
{
	...
	if (touch_mode[touch_id][Touch_Game_Mode][GET_CUR_VALUE]) {
		...
		corner_filter_value = game_mode_bdata[touch_id].
				cornerfilter_area_step[touch_mode[touch_id][Touch_Edge_Filter][GET_CUR_VALUE]];
		for (i = 0; i < MI_GRIP_PARAMETERS_SIZE; i++) {
			if (touch_mode[touch_id][Touch_Panel_Orientation][GET_CUR_VALUE]) {
				...
				if (touch_mode[touch_id][Touch_Panel_Orientation][GET_CUR_VALUE] == 1) {
				...
				} else if (touch_mode[touch_id][Touch_Panel_Orientation][GET_CUR_VALUE] == 3) {
					grid_updata_value[MI_GRIP_PARAMETERS_SIZE * 2 + i] =
...
static void xiaomi_touch_cmd_update_work(struct work_struct *work)
{
	...
	if (mode_update_flag & ((1 << Touch_Game_Mode) | (1 << Touch_Panel_Orientation) | (1 << Touch_Edge_Filter))) {
...
int xiaomi_touch_init_touch_mode(s8 touch_id, struct device *dev)
{
	...
	touch_mode[touch_id][Touch_Game_Mode][GET_MAX_VALUE] = game_mode_bdata[touch_id].game_mode[MI_DTS_GET_MAX_INDEX];
	touch_mode[touch_id][Touch_Game_Mode][GET_MIN_VALUE] = game_mode_bdata[touch_id].game_mode[MI_DTS_GET_MIN_INDEX];
	touch_mode[touch_id][Touch_Game_Mode][GET_DEF_VALUE] = game_mode_bdata[touch_id].game_mode[MI_DTS_GET_DEF_INDEX];
	touch_mode[touch_id][Touch_Game_Mode][SET_CUR_VALUE] = game_mode_bdata[touch_id].game_mode[MI_DTS_SET_CUR_INDEX];
	touch_mode[touch_id][Touch_Game_Mode][GET_CUR_VALUE] = game_mode_bdata[touch_id].game_mode[MI_DTS_GET_CUR_INDEX];

	/* Active Mode */
	touch_mode[touch_id][Touch_Active_MODE][GET_MAX_VALUE] = game_mode_bdata[touch_id].active_mode[MI_DTS_GET_MAX_INDEX];
	touch_mode[touch_id][Touch_Active_MODE][GET_MIN_VALUE] = game_mode_bdata[touch_id].active_mode[MI_DTS_GET_MIN_INDEX];
	touch_mode[touch_id][Touch_Active_MODE][GET_DEF_VALUE] = game_mode_bdata[touch_id].active_mode[MI_DTS_GET_DEF_INDEX];
	touch_mode[touch_id][Touch_Active_MODE][SET_CUR_VALUE] = game_mode_bdata[touch_id].active_mode[MI_DTS_SET_CUR_INDEX];
	touch_mode[touch_id][Touch_Active_MODE][GET_CUR_VALUE] = game_mode_bdata[touch_id].active_mode[MI_DTS_GET_CUR_INDEX];

	/* Finger Hysteresis */
	touch_mode[touch_id][Touch_UP_THRESHOLD][GET_MAX_VALUE] = game_mode_bdata[touch_id].up_threshold[MI_DTS_GET_MAX_INDEX];
	touch_mode[touch_id][Touch_UP_THRESHOLD][GET_MIN_VALUE] = game_mode_bdata[touch_id].up_threshold[MI_DTS_GET_MIN_INDEX];
	touch_mode[touch_id][Touch_UP_THRESHOLD][GET_DEF_VALUE] = game_mode_bdata[touch_id].up_threshold[MI_DTS_GET_DEF_INDEX];
	touch_mode[touch_id][Touch_UP_THRESHOLD][SET_CUR_VALUE] = game_mode_bdata[touch_id].up_threshold[MI_DTS_SET_CUR_INDEX];
	touch_mode[touch_id][Touch_UP_THRESHOLD][GET_CUR_VALUE] = game_mode_bdata[touch_id].up_threshold[MI_DTS_GET_CUR_INDEX];

	/* Tolerance */
	touch_mode[touch_id][Touch_Tolerance][GET_MAX_VALUE] = game_mode_bdata[touch_id].tolerance[MI_DTS_GET_MAX_INDEX];
	touch_mode[touch_id][Touch_Tolerance][GET_MIN_VALUE] = game_mode_bdata[touch_id].tolerance[MI_DTS_GET_MIN_INDEX];
	touch_mode[touch_id][Touch_Tolerance][GET_DEF_VALUE] = game_mode_bdata[touch_id].tolerance[MI_DTS_GET_DEF_INDEX];
	touch_mode[touch_id][Touch_Tolerance][SET_CUR_VALUE] = game_mode_bdata[touch_id].tolerance[MI_DTS_SET_CUR_INDEX];
	touch_mode[touch_id][Touch_Tolerance][GET_CUR_VALUE] = game_mode_bdata[touch_id].tolerance[MI_DTS_GET_CUR_INDEX];

	/*aim sensitivity*/
	touch_mode[touch_id][Touch_Aim_Sensitivity][GET_MAX_VALUE] = game_mode_bdata[touch_id].aim_sensitivity[MI_DTS_GET_MAX_INDEX];
	touch_mode[touch_id][Touch_Aim_Sensitivity][GET_MIN_VALUE] = game_mode_bdata[touch_id].aim_sensitivity[MI_DTS_GET_MIN_INDEX];
	touch_mode[touch_id][Touch_Aim_Sensitivity][GET_DEF_VALUE] = game_mode_bdata[touch_id].aim_sensitivity[MI_DTS_GET_DEF_INDEX];
	touch_mode[touch_id][Touch_Aim_Sensitivity][SET_CUR_VALUE] = game_mode_bdata[touch_id].aim_sensitivity[MI_DTS_SET_CUR_INDEX];
	touch_mode[touch_id][Touch_Aim_Sensitivity][GET_CUR_VALUE] = game_mode_bdata[touch_id].aim_sensitivity[MI_DTS_GET_CUR_INDEX];

	/*tap stability*/
	touch_mode[touch_id][Touch_Tap_Stability][GET_MAX_VALUE] = game_mode_bdata[touch_id].tap_stability[MI_DTS_GET_MAX_INDEX];
	touch_mode[touch_id][Touch_Tap_Stability][GET_MIN_VALUE] = game_mode_bdata[touch_id].tap_stability[MI_DTS_GET_MIN_INDEX];
	touch_mode[touch_id][Touch_Tap_Stability][GET_DEF_VALUE] = game_mode_bdata[touch_id].tap_stability[MI_DTS_GET_DEF_INDEX];
	touch_mode[touch_id][Touch_Tap_Stability][SET_CUR_VALUE] = game_mode_bdata[touch_id].tap_stability[MI_DTS_SET_CUR_INDEX];
	touch_mode[touch_id][Touch_Tap_Stability][GET_CUR_VALUE] = game_mode_bdata[touch_id].tap_stability[MI_DTS_GET_CUR_INDEX];

	/* Edge Filter */
	touch_mode[touch_id][Touch_Edge_Filter][GET_MAX_VALUE] = game_mode_bdata[touch_id].edge_filter[MI_DTS_GET_MAX_INDEX];
	touch_mode[touch_id][Touch_Edge_Filter][GET_MIN_VALUE] = game_mode_bdata[touch_id].edge_filter[MI_DTS_GET_MIN_INDEX];
	touch_mode[touch_id][Touch_Edge_Filter][GET_DEF_VALUE] = game_mode_bdata[touch_id].edge_filter[MI_DTS_GET_DEF_INDEX];
	touch_mode[touch_id][Touch_Edge_Filter][SET_CUR_VALUE] = game_mode_bdata[touch_id].edge_filter[MI_DTS_SET_CUR_INDEX];
	touch_mode[touch_id][Touch_Edge_Filter][GET_CUR_VALUE] = game_mode_bdata[touch_id].edge_filter[MI_DTS_GET_CUR_INDEX];

	/* Orientation */
	touch_mode[touch_id][Touch_Panel_Orientation][GET_MAX_VALUE] = game_mode_bdata[touch_id].panel_orien[MI_DTS_GET_MAX_INDEX];
	touch_mode[touch_id][Touch_Panel_Orientation][GET_MIN_VALUE] = game_mode_bdata[touch_id].panel_orien[MI_DTS_GET_MIN_INDEX];
	touch_mode[touch_id][Touch_Panel_Orientation][GET_DEF_VALUE] = game_mode_bdata[touch_id].panel_orien[MI_DTS_GET_DEF_INDEX];
	touch_mode[touch_id][Touch_Panel_Orientation][SET_CUR_VALUE] = game_mode_bdata[touch_id].panel_orien[MI_DTS_SET_CUR_INDEX];
	touch_mode[touch_id][Touch_Panel_Orientation][GET_CUR_VALUE] = game_mode_bdata[touch_id].panel_orien[MI_DTS_GET_CUR_INDEX];

	/* Report Rate */
	touch_mode[touch_id][Touch_Report_Rate][GET_MAX_VALUE] = game_mode_bdata[touch_id].report_rate[MI_DTS_GET_MAX_INDEX];
	touch_mode[touch_id][Touch_Report_Rate][GET_MIN_VALUE] = game_mode_bdata[touch_id].report_rate[MI_DTS_GET_MIN_INDEX];
	touch_mode[touch_id][Touch_Report_Rate][GET_DEF_VALUE] = game_mode_bdata[touch_id].report_rate[MI_DTS_GET_DEF_INDEX];
	touch_mode[touch_id][Touch_Report_Rate][SET_CUR_VALUE] = game_mode_bdata[touch_id].report_rate[MI_DTS_SET_CUR_INDEX];
	touch_mode[touch_id][Touch_Report_Rate][GET_CUR_VALUE] = game_mode_bdata[touch_id].report_rate[MI_DTS_GET_CUR_INDEX];
...
static void xiaomi_touch_set_mode_long_value(common_data_t *common_data)
{
	...
	if (mode == Touch_Grip_Mode) {
		if (touch_mode[common_data->touch_id][Touch_Game_Mode][GET_CUR_VALUE]) {
...
static void xiaomi_touch_set_mode_value(common_data_t *common_data)
{
	...
	case Touch_Power_Status:
		...
		if (val) {
			...
			if (resume_skip) {
				LOG_INFO("last status is screen off, skip resume work");
				return;
			}
#endif
	...
	case TOUCH_SENSOR_ENABLE:
		...
		add_common_data_to_buf(touch_id, SET_CUR_VALUE, Touch_Suspend, 1, &value);
		break;
...
int xiaomi_touch_mode(private_data_t *client_private_data, u32 user_size, unsigned long arg)
{
	if (user_size != sizeof(common_data_t)) {
		...
		if (common_data.cmd == SET_CUR_VALUE && common_data.mode != THP_BASE_STATUS) {
			/* print the other log*/
			LOG_INFO("cmd: %d, mode: %d, data_buf[0]: %d", common_data.cmd, common_data.mode, common_data.data_buf[0]);
		}

		if (common_data.cmd == SET_LONG_VALUE && common_data.mode == Touch_Thp_Gesture_Type) {
		...
		case SET_CMD_FOR_THP:
			...
			return 0;
		case SET_CUR_VALUE:
			if (common_data.mode == Touch_GAMETURBOTOOL_FOLLOWUP && common_data.data_buf[0] > 100 && common_data.data_buf[0] < 127) {
		...
		if ((common_data.mode > Touch_Power_Status && common_data.mode < TOUCH_MOTION_BASE) || common_data.mode == THP_HAL_INIT_READY) {
