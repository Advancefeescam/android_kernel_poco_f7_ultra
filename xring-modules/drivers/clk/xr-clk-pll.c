...
static int clk_pll_enable(struct clk_hw *hw)
{
	...
	if (pll->flags & PLL_FLAGS_DOZE_VOTE) {
		ret = sys_state_doz2nor_vote(pll->vote_id);
		if (ret) {
...
struct clk_hw *devm_xr_clk_hw_pll(struct device *dev, const char *name,
				...
				unsigned int num_entry, unsigned char safe_id,
				unsigned int vote_id, unsigned char pll_flags)
{
