...
/* P6 code for BUGP6-1976 by p-chenchen79 at 2025/8/19 start */
#define AOD_FPS 30
/* P6 code for BUGP6-1976 by p-chenchen79 at 2025/8/19 end */
...
/* P6 code for BUGP6-1976 by p-chenchen79 at 2025/8/19 start */
	if(comp && comp->mtk_crtc){
		cur_mode_index = to_mtk_crtc_state(comp->mtk_crtc->base.state)->prop_val[CRTC_PROP_DISP_MODE_IDX];
		cur_mode = &(comp->mtk_crtc->avail_modes[cur_mode_index]);
		cur_fps = drm_mode_vrefresh(cur_mode);
		pr_info(" %s cur_fps = %d\n", __func__, cur_fps);
		if (cur_fps == AOD_FPS) {
			pr_info(" %s aod_switch_fps set true\n", __func__);
			aod_switch_fps = true;
/* P6 code for HQFEAT-178208 by p-chenchen79 at 2025/8/27 start */
			reinit_completion(&dsi->aod_delay_wait_completion);
...
/* P6 code for BUGP6-1976 by p-chenchen79 at 2025/8/19 start */
		if(doze_brightness == DOZE_TO_NORMAL && aod_switch_fps && dsi->ext && dsi->ext->params && dsi->ext->params->aod_delay_cmd_enable ){
/* P6 code for BUGP6-1976 by p-chenchen79 at 2025/8/19 end */
/* P6 code for BUGP6-3171 by p-chenchen79 at 2025/8/11 start */
/* P6 code for HQFEAT-178208 by p-chenchen79 at 2025/8/27 start */
			ret = wait_for_completion_timeout(
					&dsi->aod_delay_wait_completion,
					msecs_to_jiffies(200));
				pr_info("%s aod_delay_wait_completion return %d\n",
					__func__ , ret);
/* P6 code for HQFEAT-178208 by p-chenchen79 at 2025/8/27 end */
			mdelay(20);
/* P6 code for BUGP6-3171 by p-chenchen79 at 2025/8/11 end */
			if(panel_ext && panel_ext->funcs && panel_ext->funcs->send_delay_cmd){
				panel_ext->funcs->send_delay_cmd();
			}
		}
/* P6 code for BUGP6-719 by p-liaoxianguo at 2025/07/14 start*/
...
#endif
/* P6 code for HQFEAT-178208 by p-chenchen79 at 2025/8/27 start */
	init_completion(&dsi->aod_delay_wait_completion);
/* P6 code for HQFEAT-178208 by p-chenchen79 at 2025/8/27 end */
