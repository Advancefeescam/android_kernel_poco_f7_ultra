...
static int of_xring_regulator_phy_addr(struct platform_device *pdev,
				struct xring_regulator_ip *sreg)
{
	struct device *dev = &pdev->dev;
	struct platform_device *pdev_parent = to_platform_device(pdev->dev.parent);
	struct resource *res;

	res = platform_get_resource_byname(pdev_parent, IORESOURCE_MEM, "acpu-gpu-lpctrl");
	if (!res) {
		dev_err(dev, "Failed to get 'acpu-gpu-lpctrl' resource\n");
		return -ENODEV;
	}
	sreg->phy_addr.base_gpu_lpctrl = res->start;
	res = platform_get_resource_byname(pdev_parent, IORESOURCE_MEM, "acpu-peri-crg");
	if (!res) {
		dev_err(dev, "Failed to get 'acpu-peri-crg' resource\n");
		return -ENODEV;
	}
	sreg->phy_addr.base_peri_crg = res->start;

	res = platform_get_resource_byname(pdev_parent, IORESOURCE_MEM, "acpu-gpu-ctrl");
	if (!res) {
		dev_err(dev, "Failed to get 'acpu-gpu-ctrl' resource\n");
		return -ENODEV;
	}
	sreg->phy_addr.base_gpu_ctrl = res->start;

	res = platform_get_resource_byname(pdev_parent, IORESOURCE_MEM, "acpu-media1-crg");
	if (!res) {
		dev_err(dev, "Failed to get 'acpu-media1-crg' resource\n");
		return -ENODEV;
	}
	sreg->phy_addr.base_media1_crg = res->start;

	res = platform_get_resource_byname(pdev_parent, IORESOURCE_MEM, "acpu-media1-lpctrl-ft");
	if (!res) {
		dev_err(dev, "Failed to get 'acpu-media1-lpctrl-ft' resource\n");
		return -ENODEV;
	}
	sreg->phy_addr.base_media1_lpctrl_ft = res->start;

	res = platform_get_resource_byname(pdev_parent, IORESOURCE_MEM, "acpu-dpu-glb");
	if (!res) {
		dev_err(dev, "Failed to get 'acpu-dpu-glb' resource\n");
		return -ENODEV;
	}
	sreg->phy_addr.base_dpu_glb = res->start;

	res = platform_get_resource_byname(pdev_parent, IORESOURCE_MEM, "acpu-gpu-crg");
	if (!res) {
		dev_err(dev, "Failed to get 'acpu-gpu-crg' resource\n");
		return -ENODEV;
	}
	sreg->phy_addr.base_acpu_gpu_crg = res->start;
...
static int xring_regulator_probe(struct platform_device *pdev)
{
	...
	ret = of_xring_regulator_phy_addr(pdev, sreg);
