...
static int goodix_init_testlimits(struct goodix_ts_test *ts_test)
{
	...
	if (tp_compatible_flag == TOUCH_SELECT_VISIONOX) {
		...
		ts_info("VISIONOX limit_file %s", limit_file);
	} else if (tp_compatible_flag == TOUCH_SELECT_HUAXING) {
		sprintf(limit_file, "%s_%d_csot.csv", GOODIX_TEST_FILE_NAME,
		ts_core->fw_version.sensor_id);
		ts_info("huaxing limit_file %s", limit_file);
	} else {
		ts_err("goodix_test_limits IS NOT CORRECT!");
	}
...
/*P6 code for HQFEAT-118700 by p-zhaobeidou3 at 2025/6/27 start*/
		...
		xiaomi_touch_mievent_report_int(TOUCH_EVENT_SHORTTEST_FAIL, 0, "TpShortTestFail", "goodix", ret);
#endif
...
/*P6 code for HQFEAT-118700 by p-zhaobeidou3 at 2025/6/27 start*/
#if IS_ENABLED(CONFIG_MIEV)
			xiaomi_touch_mievent_report_int(TOUCH_EVENT_OPENTEST_FAIL, 0, "TpOpenTestFail", "goodix",ret);
#endif
/*P6 code for HQFEAT-118700 by p-zhaobeidou3 at 2025/6/27 end*/
		} else if (ret == 0) {
...
/*O6 code for HQ-391820 by liaoxianguo at 2024/07/24 start*/
	seq_printf(m, data);
/*O6 code for HQ-391820 by liaoxianguo at 2024/07/24 end*/
	ts_info("data = %s", data);
