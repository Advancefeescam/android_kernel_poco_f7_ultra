...
/*L19 code for HQ-173140 by gaoxue at 2021/12/23 start*/
	fts_read_reg(FTS_REG_EARPHONE_MODE_EN, &val);
	count += snprintf(buf + count, PAGE_SIZE, "earphone stat:0x%02x\n", val);
/*L19 code for HQ-173140 by gaoxue at 2021/12/23 end*/
...
/*L19A code for HQ-235102 by lizongrui at 2022/12/12 start*/
	seq_printf(m, "0x%.2hhx,0x%.2hhx,0x%.2hhx,0x%.2hhx,0x%.2hhx,0x%.2hhx,0x%.2hhx,0x%.2hhx\n",
                        fts_data->fts_lockdowninfo[0],fts_data->fts_lockdowninfo[1],
                        fts_data->fts_lockdowninfo[2],fts_data->fts_lockdowninfo[3],
                        fts_data->fts_lockdowninfo[4],fts_data->fts_lockdowninfo[5],
                        fts_data->fts_lockdowninfo[6],fts_data->fts_lockdowninfo[7]);
