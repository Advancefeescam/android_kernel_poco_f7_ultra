...
static u32 base_reg_arr[BASE_REG_NUM] = {};
...
static void xring_flowctrl_get_dts_info(struct device_node *np)
{
	int ret = 0;

	ret = of_property_read_u32_index(np, "acpu_pctrl", 0, &base_reg_arr[ACPU_PCTRL_ID]);
	ret += of_property_read_u32_index(np, "acpu_dpu_top", 0, &base_reg_arr[ACPU_DPU_TOP_ID]);
	ret += of_property_read_u32_index(np, "acpu_m1_tpc", 0, &base_reg_arr[ACPU_MEDIA1_TPC_ID]);
	ret += of_property_read_u32_index(np, "acpu_gpu_ctrl", 0, &base_reg_arr[ACPU_GPU_CTRL_ID]);
	ret += of_property_read_u32_index(np, "acpu_m2_sc_tpc", 0, &base_reg_arr[ACPU_M2_SC_TPC_ID]);
	ret += of_property_read_u32_index(np, "acpu_m2_crg", 0, &base_reg_arr[ACPU_M2_CRG_ID]);
	ret += of_property_read_u32_index(np, "acpu_usb_subsys_xctrl", 0, &base_reg_arr[ACPU_USB_SUBSYS_XCTRL_ID]);
	if (ret)
...
static int xring_flowctrl_pdev_probe(struct platform_device *pdev)
{
	...
	xring_flowctrl_get_dts_info(np);

	for (i = ACPU_PCTRL_ID; i < BASE_REG_NUM; i++) {
		g_pa_va_remap[i].pa = base_reg_arr[i];
		g_pa_va_remap[i].va = ioremap_wc(base_reg_arr[i], 0xFF0000);
	}
