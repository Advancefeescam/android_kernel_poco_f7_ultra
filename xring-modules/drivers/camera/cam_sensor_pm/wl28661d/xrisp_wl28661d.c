...
static int xrisp_wl28661d_is_enabled(struct regulator_dev *rdev)
{
	if (!rdev) {
		pr_err("rdev NULL ERR!");
		return -EINVAL;
	...
	if ((!strcmp(rdev->desc->name, "WL2_LDO1") && (rdev->use_count == 0)) ||
		(!strcmp(rdev->desc->name, "WL2_LDO3") && (rdev->use_count == 0))) {
...
static const struct regulator_ops wl28661d_reg_control_ops = {
	...
	.is_enabled	= xrisp_wl28661d_is_enabled,
