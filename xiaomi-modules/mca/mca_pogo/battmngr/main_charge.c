...
static int charger_enable_keyboard_ovp(struct mainchg *bq, int enable)
{
	...
	if (gpio_is_valid(bq->kb_ovp_en_gpio)) {
		...
		main_err("unable to set kb_ovp_en_gpio\n");
		return -EINVAL;
	}
...
int charger_process_event_irq(struct battmngr_notify *noti_data)
{
	...
	case DCIN_IRQ:
		...
		if (g_bq) {
			schedule_delayed_work(&g_bq->handler_adsp_work, msecs_to_jiffies(10000));
...
int xm_set_input_volt_limit(int volt)
{
	...
	if (object == 0x38) {
		val = volt / 100;
		ret = Nanosic_ADSP_Set_Charge_Status_Single(0x0C, 1, (uint8_t*)&val, sizeof(val), I2C_ADDR_POGO_PIN);
	} else {
		val = (volt / 1000) * 1024 + (((volt % 1000) * 1024) / 1000 + 1);
		ret = Nanosic_ADSP_Set_Charge_Status_Single(0x21, 2, (uint8_t*)&val, sizeof(val), I2C_ADDR_POGO_PIN);
	}

	if (ret < 0) {
		main_err("%s: set input volt is fail\n", __func__);
	}
	main_info("%s: set input volt: %d, ret:%d, val:%d\n", __func__, volt, ret, val);

	return ret;
}
EXPORT_SYMBOL(xm_set_input_volt_limit);
...
int xm_set_term_current(struct mainchg *bq, int curr)
{
	...
	if (check_qti_ops(&bq->battmg_dev)) {
		//ret = battmngr_qtiops_set_term_cur(bq->battmg_dev, curr);//zj_1118
		if (ret < 0)
...
int xm_enable_charger(void)
{
    ...
    if (check_g_bcdev_ops()) {
        //ret = pogo_enale_charge(true);//zj_1118
...
int xm_disable_charger(void)
{
    ...
    if (check_g_bcdev_ops()) {
        //ret = pogo_enale_charge(false);//zj_1118
...
static void xm_monitor_workfunc(struct work_struct *work)
{
	...
	if (object == 0x38)
		...
		main_info("%s:check mps D1 reg! \n", __func__);
		Nanosic_ADSP_Read_Charge_Status_Single(0xD1,1, I2C_ADDR_POGO_PIN);
		msleep(5);
		Nanosic_ADSP_Read_Charge_Status_Single(0x21,2, I2C_ADDR_POGO_PIN);
		msleep(5);
		Nanosic_ADSP_Read_Charge_Status_Single(0x01,1, I2C_ADDR_POGO_PIN);
		msleep(5);
		Nanosic_ADSP_Read_Charge_Status_Single(0x79,1, I2C_ADDR_POGO_PIN);

	#if 0
		main_info("%s:check volt and curr! \n", __func__);
		Nanosic_ADSP_Read_Charge_Status_Single(0x13,1);
		msleep(5);
		Nanosic_ADSP_Read_Charge_Status_Single(0x14,1);
		msleep(5);

		Nanosic_ADSP_Read_Charge_Status_Single(0x1e,1);
		msleep(5);
		Nanosic_ADSP_Read_Charge_Status_Single(0x1f,1);
		msleep(5);

		Nanosic_ADSP_Read_Charge_Status_Single(0x01,1);
		msleep(5);

		Nanosic_ADSP_Read_Charge_Status_Single(0x12,1);
		msleep(5);
	#endif
	}
	...
	if (dc_in) {
		if (!kb_in) {
			...
			if (object == 0x38) {
				...
				//rc = Nanosic_ADSP_Read_Charge_Status_Single(0x15,1);//zj_1118
				msleep(5);
				//rc = Nanosic_ADSP_Read_Charge_Status_Single(0x16,1);//zj_1118
				msleep(5);
				if (rc < 0) {
					main_err("%s: read holder charge data is fail.\n", __func__);
				}
			}
			Nanosic_ADSP_Read_803_Version();
			...
			if (currState == PAD_KEY) {
				...
				main_info("%s: detect PAD_KEY_TYPEC scene\n", __func__);
...
static void xm_first_irq_workfunc(struct work_struct *work)
{
	...
	//dc_in = qti_get_DCIN_STATE();//zj_1118
	platform_class_cp_get_pogo_online(CP_ROLE_MASTER, &dc_in);
	g_battmngr_noti->irq_msg.value = dc_in;
	main_err("%s: start first_irq_work, dcin state:%d\n", __func__, dc_in);
	if (bq && dc_in) {
		if (g_mcu_data) {
			//qti_deal_report();//zj_1118
