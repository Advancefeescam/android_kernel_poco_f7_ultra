...
/*L19A code for HQ-194810  by gaoxue at 2022/5/10 start*/
...
#endif
/*L19A code for HQ-194810  by gaoxue at 2022/5/10 end*/
/*L19A-T code for HQ-236699 by zenghui at 2022/11/21 start */
#include <linux/power_supply.h>//charge flag
/*L19A-T code for HQ-236699 by zenghui at 2022/11/21 end */
/*****************************************************************************
...
#endif
#if FOCAL_LOCKDOWN
/*L19A code for HQ-235102 by lizongrui at 2022/12/12 start*/
char fts_lockdown[8] = {0};
/*L19A code for HQ-235102 by lizongrui at 2022/12/12 end*/
#endif
/*****************************************************************************
* Global variable or extern global variabls/functions
*****************************************************************************/
/*L19A code for HQ-198798  by gaoxue at 2022/4/27 start*/
...
#endif
/*L19A code for HQ-198798  by gaoxue at 2022/4/27 end*/
...
/*L19 code for HQ-159440 by sunfeiting at 2021/12/8 start*/
...
/*L19 code for HQ-159440 by sunfeiting at 2021/12/8 end*/
...
*****************************************************************************/
/*L19 code for L19-68 by gaoxue at 2021/12/23 start*/
bool tp_ready = false;
...
/*L19 code for L19-68 by gaoxue at 2021/12/23 end*/
/*****************************************************************************
*  Reprot related
*****************************************************************************/
...
/*L19A code for HQ-194810  by gaoxue at 2022/4/25 start*/
#if	FTS_PALM_EN
...
#endif
/*L19A code for HQ-194810  by gaoxue at 2022/4/25 end*/
								...
								unsigned long event, void *data)
{
	...
	/*L19A code for HQ-221066 by zhangkexin at 2022/07/27 start*/
	#if (defined CONFIG_MI_ESD_SUPPORT) && (defined PROJECT_ROCK)
	if (!(event == MTK_DISP_EARLY_EVENT_BLANK || event == MTK_DISP_EVENT_BLANK || event == MTK_DISP_ESD_EVENT_SUSPEND || event == MTK_DISP_ESD_EVENT_RESUME)) {
	#else
	...
	case MTK_DISP_BLANK_UNBLANK:
		...
		/*L19A code for HQ-221066 by zhangkexin at 2022/07/27 start*/
		#if (defined CONFIG_MI_ESD_SUPPORT) && (defined PROJECT_ROCK)
		} else if (MTK_DISP_ESD_EVENT_RESUME == event) {
			FTS_DEBUG("esd resume: event = %lu \n", event);
			fts_ts_tp_resume();
		#endif
		}
		break;
	case MTK_DISP_BLANK_POWERDOWN:
		...
		#if (defined CONFIG_MI_ESD_SUPPORT) && (defined PROJECT_ROCK)
		} else if (MTK_DISP_ESD_EVENT_SUSPEND == event) {
			FTS_DEBUG("esd suspend: event = %lu \n", event);
			fts_ts_tp_suspend();
		#endif
		}
		/*L19A code for HQ-221066 by zhangkexin at 2022/07/27 end*/
...
/*L19A code for HQ-194846  by gaoxue at 2022/4/29 start*/
...
/*L19A code for HQ-235102 by lizongrui at 2022/12/12 start*/
	unsigned char *tp_lockdown_info = NULL;
/*L19A code for HQ-235102 by lizongrui at 2022/12/12 end*/
	...
	if (chosen) {
/*L19A code for HQ-235102 by lizongrui at 2022/12/12 start*/
		tp_lockdown_info = (unsigned char *)of_get_property(chosen, "tp_lockdown_info", (int *)&size);
		if (size != 8) {
			FTS_ERROR("wrong lockdowninfo length:%lu", size);
		} else {
			memcpy(ts_data->fts_lockdowninfo, tp_lockdown_info, 8);
			FTS_DEBUG("lockdowninfo=%hhx,%hhx,%hhx,%hhx,%hhx,%hhx,%hhx,%hhx",
					ts_data->fts_lockdowninfo[0],
					ts_data->fts_lockdowninfo[1],
					ts_data->fts_lockdowninfo[2],
					ts_data->fts_lockdowninfo[3],
					ts_data->fts_lockdowninfo[4],
					ts_data->fts_lockdowninfo[5],
					ts_data->fts_lockdowninfo[6],
					ts_data->fts_lockdowninfo[7]);
		}
/*L19A code for HQ-235102 by lizongrui at 2022/12/12 end*/
	}
#endif
/*L19A code for HQ-194846  by gaoxue at 2022/4/29 end*/
...
#endif
/*L19 code for HQ-173453 by caogaojie at 2021/12/21 start*/
#if (defined CONFIG_MI_ESD_SUPPORT)&&(defined RPOJECT_ROCK)
...
#if defined(CONFIG_DRM_PANEL)
	if (active_panel)
		mtk_disp_notifier_unregister(active_panel, &ts_data->fb_notif);
#else
...
{
	return fts_data->fts_lockdowninfo[0];
}

static u8 fts_panel_display_read(void)
{
	return fts_data->fts_lockdowninfo[1];
}
static u8 fts_panel_color_read(void)
{
	return fts_data->fts_lockdowninfo[2];
}
...
/*L19A code for HQ-235102 by lizongrui at 2022/12/12 start*/
	...
	/*L19A code for HQ-264934 by lizongrui at 2022/11/21 start*/
	...
	/*L19A code for HQ-264934 by lizongrui at 2022/11/21 end*/
...
/*L19A code for HQ-216583  by gaoxue at 2022/6/9 start*/
#if FOCAL_LOCKDOWN
...
#endif
/*L19A code for HQ-214210  by gaoxue at 2022/6/8 start*/
...
/*L19A code for HQ-235102 by lizongrui at 2022/12/12 start*/
	FTS_DEBUG("lcmname %50.50s,length:%lu", lcmname, size);
/*L19A code for HQ-235102 by lizongrui at 2022/12/12 end*/
	if ((strcmp(lcmname, "dsi_panel_l19a_31_03_0b_dsc_vdo_lcm_drv"))) {
		FTS_INFO("Func:%s lcmname:%d", __func__, lcmname);
		return 0;
	}
...
/*L19A code for HQ-214210  by gaoxue at 2022/6/8 end*/
/*L19A code for HQ-216583  by gaoxue at 2022/6/9 end*/
