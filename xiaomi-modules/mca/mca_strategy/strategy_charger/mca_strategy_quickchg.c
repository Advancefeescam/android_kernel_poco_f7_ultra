...
static void mca_quick_charge_stop_charging(struct mca_quick_charge_info *info)
{
	...
	if (info->proc_data.cur_work_cp == MCA_QUICK_CHG_CP_DUAL) {
		...
		ret |= platform_class_cp_check_iic_check(CP_ROLE_SLAVE);
	} else {
...
static int mca_quick_charge_judge_thermal_vote(struct mca_quick_charge_info *info, bool is_prechk)
{
	...
	else if (info->cp_type == MCA_BATTERY_TYPE_PARALLEL)
		thermal_curr = info->proc_data.thermal_cur[MCA_QUICK_CHG_CH_MULTI];
...
static int mca_pogo_quick_charge_open_path(struct mca_quick_charge_info *info)
{
	int ret, i;
	int vbat = 0;
	bool chg_enable = false;
	...
	ret = platform_class_cp_enable_adc(CP_ROLE_MASTER, true);
	ret |= platform_class_cp_set_mode(CP_ROLE_MASTER, CP_MODE_FORWARD_4_1);
	ret |= platform_class_cp_set_mode(CP_ROLE_SLAVE, CP_MODE_FORWARD_4_1);
	...
	info->proc_data.cur_adp_volt = vbat * 4 + 450;
	info->proc_data.cur_adp_cur = MCA_QUICK_CHG_OPEN_PATH_CURR;
...
static void mca_quick_charge_start_charging(struct mca_quick_charge_info *info)
{
	...
	if (info->support_base_flip) {
		ret = mca_quick_get_parallel_volt_para(info, FG_IC_BASE);
		ret |= mca_quick_get_parallel_volt_para(info, FG_IC_FLIP);
		if(ret) {
	...
	if (info->is_pogo_online) {
		ret = mca_pogo_quick_charge_open_path(info);
	} else if (!info->is_platform_qc)
...
static void mca_quick_charge_pre_charge_check(struct mca_quick_charge_info *info)
{
	...
	if (info->proc_data.total_err > MCA_QUICK_CHG_MAX_ERR_COUNT) {
		...
		mca_log_info("total_err : %d\n", info->proc_data.total_err);
		return;
	}
	if (info->force_stop || !info->cur_support_mode ||
		!info->sysfs_data.chg_enable || info->taper_done_no_retry ||
		!info->batt_auth) {
		 mca_log_info("force_stop : %d, cur_support_mode : %d, chg_enable : %d, "
                      "taper_done_no_retry : %d, batt_auth : %d\n", info->force_stop,
                       info->cur_support_mode, info->sysfs_data.chg_enable,
                       info->taper_done_no_retry, info->batt_auth);
		return;
	}
	...
	case XM_CHARGER_TYPE_HVDCP3P5:
		...
		break;
	case XM_CHARGER_TYPE_POGO:
		info->proc_data.work_mode = MCA_QUICK_CHG_MODE_DIV_4;
		info->proc_data.min_adp_volt = MCA_QUICK_CHG_ADP_DEFAULT_VOLT;
		info->proc_data.max_adp_volt = 20000;
		info->proc_data.max_adp_curr = MCA_QUICK_CHG_ADP_QC_MAX_CURR;
		//info->pogo_ibus = 2100;
		break;
	default:
...
#define QUICK_CHARGE_EXIT_RAWSOC 9600
static void mca_quick_charge_check_if_switch_buck(struct mca_quick_charge_info *info, int chg_done, int ffc_flag, int iterm)
{
	int rawsoc = 0;
	...
	if(!chg_done && high_rawsoc_exit) {
		mca_log_err("high soc swith buck\n");
		/* high ibat may causes vbat too high if switch to pmic and normal charging */
		if (info->batt_type > MCA_BATTERY_TYPE_SINGLE_NUM_MAX) {
			mca_log_info("cur_max: %d, ffc_flag: %d, iterm: %d\n", info->proc_data.cur_max_final, ffc_flag, iterm);
			if (!ffc_flag ||
				(ffc_flag && (rawsoc >= QUICK_CHARGE_EXIT_RAWSOC) && (info->proc_data.cur_max_final < iterm))) {
				info->proc_data.charge_flag = MCA_QUICK_CHG_STS_CHARGE_DONE;
				mca_log_err("normal charging or fcc too low need to switch buckchg\n");
			}
		} else {
...
static void mca_quick_charge_check_charge_done(struct mca_quick_charge_info *info)
{
	...
	int iterm_temp = 0;
	static int count;
	int charging_done = 0, ffc_flag = 0;
	bool fg1_done = false, fg2_done = false;
	int last_volt_stage_para = info->proc_data.cur_volt_paraller[FG_IC_BASE]->volt_para_size - 1;

	if (info->support_base_flip) {
		(void)strategy_class_fg_get_batt_ctr_sts(FG_IC_MASTER, &fg1_done);
		(void)strategy_class_fg_get_batt_ctr_sts(FG_IC_SLAVE, &fg2_done);
		if (fg1_done && !fg2_done) {
			iterm = info->proc_data.cur_volt_paraller[FG_IC_FLIP]->volt_para[last_volt_stage_para].current_min;
			mca_log_info("master batt is full, use slave batt iterm: %d\n", iterm);
		} else if (!fg1_done && fg2_done) {
			iterm = info->proc_data.cur_volt_paraller[FG_IC_BASE]->volt_para[last_volt_stage_para].current_min;
			mca_log_info("slave batt is full, use master batt iterm: %d\n", iterm);
		}
	}
	...
	ffc_flag = strategy_class_fg_get_fastcharge();
	mca_quick_charge_check_if_switch_buck(info, charging_done, ffc_flag, iterm_temp);
...
static int mca_quick_charge_get_vstep(int cur_cap, int vbat_th, struct mca_quick_charge_info *info)
{
	int i;
	int vstep = 0;
	bool fast_vstep = false;
	...
	if (cur_cap <= 0) {
		...
		if (vbat <= vbat_th - MCA_QUICK_CHG_VOL_HYS_MV)
			fast_vstep = true;
	}
	mca_log_info("check if need fast_vstep:%d\n", fast_vstep);

	/* cv can not add step, unless thermal curr in effect */
	if (info->proc_data.cur_stage[0] % 2 && !fast_vstep)
...
static int mca_pogo_get_vbus_change_trend(struct mca_quick_charge_info *info)
{
	static int ibatt_count;
	static int vbatt_count;
	static int ibatt_change = 1;
	...
	if (info->proc_data.adp_type == XM_CHARGER_TYPE_POGO) {
		//ibat_qc_max_ma = 8000;
		//ibus_qc_max_ma = 2100;
		ibat_qc_max_ma = info->pogo_ibus * 4 - 400;
		ibus_qc_max_ma = info->pogo_ibus;
	} else if (info->proc_data.adp_type == XM_CHARGER_TYPE_HVDCP3_B) {
		ibat_qc_max_ma = info->qc3_ibat_max_limit_ma;
		ibus_qc_max_ma = info->qc3_max_ibus_limit_ma;
	} else {
		ibat_qc_max_ma = info->qc3p5_max_ibat_limit_ma;
	...
	if (info->thermal_cur[CHG_MODE_DIV4][MCA_QUICK_CHG_CH_SINGLE] >= 1500) {
		target_limit_fcc_ma = min(info->thermal_cur[CHG_MODE_DIV4][MCA_QUICK_CHG_CH_SINGLE], ibat_qc_max_ma);
		target_limit_ibus_ma = min((info->thermal_cur[CHG_MODE_DIV4][MCA_QUICK_CHG_CH_SINGLE] / 2 + 100),
			ibus_qc_max_ma);
		mca_log_info("target_limit_fcc_ma: %d, target_limit_ibus_ma:%d\n",
			target_limit_fcc_ma, target_limit_ibus_ma);
	} else if (!info->thermal_cur[CHG_MODE_DIV4][MCA_QUICK_CHG_CH_SINGLE]) {
		target_limit_fcc_ma = ibat_qc_max_ma;
		target_limit_ibus_ma = ibus_qc_max_ma;
	} else {
		return DIR_HOLD;
	...
	case XM_CHARGER_TYPE_HVDCP3_B:
		...
		break;
	case XM_CHARGER_TYPE_POGO:
		if (vbus <= 20000 && ibus < target_limit_ibus_ma - 100 &&
			(vbatt_change && vbat_now_mv < info->qc_normal_charge_fv_max_mv - 20 - delta_fv) &&
			(ibatt_change && ibat_now_ma < target_limit_fcc_ma - 50)) {
			dir = DIR_UP;
			mca_log_info("pogo plus pluse sent\n");
		}
		if ((vbatt_change && vbat_now_mv > info->qc_normal_charge_fv_max_mv - 10 - delta_fv)
			|| (ibatt_change && ibat_now_ma > target_limit_fcc_ma + 50)
			|| (ibus > target_limit_ibus_ma + 50)) {
			dir = DIR_DOWN;
			mca_log_info("pogo mius pluse sent\n");
		}
		break;
	case XM_CHARGER_TYPE_HVDCP3P5:
		if (vbus <= info->qc3p5_max_vbus_limit_mv && ibus < target_limit_ibus_ma - 100 &&
...
static int mca_pogo_quick_charge_regulation(struct mca_quick_charge_info *info)
{
	int qc_pluse_direction = DIR_HOLD;
	int ret = 0, rawsoc = 0;

	strategy_class_fg_ops_get_rawsoc(&rawsoc);
	if(rawsoc >= 9700) {
		mca_log_err("switch buck: %d\n", rawsoc);
		mca_vote(info->buck_charge_curr_voter, "qc_done", true, QUICK_RAWSOC_HIGH_DONE_LIMIT);
		info->proc_data.charge_flag = MCA_QUICK_CHG_STS_CHARGE_DONE;
	}

	qc_pluse_direction = mca_pogo_get_vbus_change_trend(info);

	if (qc_pluse_direction == DIR_UP)
		info->proc_data.cur_adp_volt += MCA_QUICK_CHG_DEFAULT_VSTEP * 5;
	else if (qc_pluse_direction == DIR_DOWN)
		info->proc_data.cur_adp_volt -= MCA_QUICK_CHG_DEFAULT_VSTEP * 5;
	else
		return 0;
...
static int mca_quick_charge_regulation(struct mca_quick_charge_info *info)
{
	...
	info->proc_data.cur_max_final = cur_max;
	...
	vstep = mca_quick_charge_get_vstep(cur_max - ibat, vbat_th, info);
...
{
	if (info->cp_type != MCA_CP_TYPE_PARALLEL || info->is_platform_qc || info->is_pogo_online)
		return;
...
static void mca_quick_charge_monitor_work(struct work_struct *work)
{
	...
	if (info->is_pogo_online) {
		ret = mca_pogo_quick_charge_regulation(info);
	} else if (info->is_platform_qc)
...
static int mca_quick_charge_process_event(int event, int value, void *data)
{
	...
	case MCA_EVENT_CHARGE_TYPE_CHANGE:
		...
		if (value == XM_CHARGER_TYPE_PD_VERIFY || value == XM_CHARGER_TYPE_PPS) {
			...
			info->is_pogo_online = false;
		} else if (value == XM_CHARGER_TYPE_POGO) {
			info->is_pogo_online = true;
		} else {
			info->is_pogo_online = false;
			return 0;
		}
	...
	case MCA_EVENT_IBAT_OCP_CHANGE:
		...
		break;
	case MCA_EVENT_POGO_IBUS_CHANGE:
		info->pogo_ibus = value;
		mca_log_info("pogo_ibus= %d\n", info->pogo_ibus);
		if (info->pogo_ibus == 375) {
			info->proc_data.stop_charging = true;
		}
		break;
...
static int mca_quick_charge_parse_dt(struct mca_quick_charge_info *info)
{
	...
	if (info->support_base_flip) {
		...
		ret = mca_quick_charge_parse_batt_info(info, info->batt_type - 1);
	}
...
static int mca_quick_charge_probe(struct platform_device *pdev)
{
	...
	strategy_class_fg_get_auth_event(&batt_auth_pass);
	if (batt_auth_pass) {
		mca_log_err("avoid missing batt auth event in probe\n");
