...
/*BSP.TP - Add MAX_POWER - 2020.12.08*/
/*L19 code for L19-63 by gaoxue at 2021/12/22 start*/
#define NVT_TP_INFO "tp_info"
/*L19 code for L19-63 by gaoxue at 2021/12/22 end*/
...
static ssize_t nvt_max_power_switch_proc_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	int32_t tmp;
	uint8_t max_power_switch;
/*L19 code for HQ-177953 by gaoxue at 2022/1/13 start*/
	char *tmp_buf = NULL;
/*L19 code for HQ-177953 by gaoxue at 2022/1/13 end*/
	NVT_LOG("++\n");
	if (count == 0 || count > 2) {
...
static ssize_t nvt_pf_switch_proc_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	int32_t tmp;
	uint8_t pf_switch;
/*L19 code for HQ-178000 by gaoxue at 2022/1/13 start*/
	char *tmp_buf = NULL;
/*L19 code for HQ-178000 by gaoxue at 2022/1/13 end*/
	NVT_LOG("++\n");
	if (count == 0 || count > 2) {
		NVT_ERR("Invalid value! count = %zu\n", count);
		ret = -EINVAL;
...
static ssize_t nvt_sensitivity_switch_proc_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	int32_t tmp;
	uint8_t sensitivity_switch;
/*L19 code for HQ-178001 by gaoxue at 2022/1/13 start*/
	char *tmp_buf = NULL;
/*L19 code for HQ-178001 by gaoxue at 2022/1/13 end*/
	NVT_LOG("++\n");
	if (count == 0 || count > 2) {
		NVT_ERR("Invalid value! count = %zu\n", count);
		ret = -EINVAL;
...
static const struct proc_ops nvt_dump_fops = {
	//.owner = THIS_MODULE,
	.proc_open = nvt_dump_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = seq_release,
};
...
static const struct proc_ops nvt_gesture_mode_fops = {
	...
	.proc_read = seq_read,
	.proc_write = nvt_gesture_mode_write,
};
#ifdef NVT_IRQ_SWITCH
/*BSP.TP add nvt_irq - 2020.11.24 - Start*/
...
static const struct proc_ops nvt_irq_control_fops = {
	//.owner = THIS_MODULE,
	.proc_open = nvt_irq_control_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = seq_release,
	.proc_write = nvt_irq_control_write,
};
#endif
...
static ssize_t nvt_charger_switch_proc_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	int32_t tmp;
	uint8_t charger_switch;
/*L19 code for HQ-177999 by gaoxue at 2022/1/13 start*/
	char *tmp_buf = NULL;
/*L19 code for HQ-177999 by gaoxue at 2022/1/13 end*/
	NVT_LOG("++\n");
	if (count == 0 || count > 2) {
		NVT_ERR("Invalid value!, count = %zu\n", count);
		ret = -EINVAL;
...
static ssize_t nvt_pocket_palm_switch_proc_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	int32_t tmp;
	uint8_t pocket_palm_switch;
/*L19 code for HQ-178002 by gaoxue at 2022/1/13 start*/
	char *tmp_buf = NULL;
/*L19 code for HQ-178002 by gaoxue at 2022/1/13 end*/
	NVT_LOG("++\n");
	if (count == 0 || count > 2) {
		NVT_ERR("Invalid value!, count = %zu\n", count);
		ret = -EINVAL;
