...
static u64 read_sysreg64(struct device *dev, enum sysreg_id id)
{
	struct arm_smccc_res res;

	arm_smccc_smc(FID_BL31_CPU_REG_READ_SYSREG, id, 0, 0, 0, 0, 0, 0, &res);
	if (res.a0 != 0) {
		dev_err(dev, "Failed to read from sysreg id %u.\n", id);
		return 0;
	}
	return (u64)res.a1;
}

static void write_sysreg64(struct device *dev, enum sysreg_id id, u64 val)
{
	struct arm_smccc_res res;

	arm_smccc_smc(FID_BL31_CPU_REG_WRITE_SYSREG, id, val, 0, 0, 0, 0, 0, &res);
	if (res.a0 != 0)
		dev_err(dev, "Failed to write to sysreg id %u, val 0x%llx\n", id, val);
}
...
static void sysreg_show(void *info)
{
	...
	value = read_sysreg64(args->dev, args->id);
...
static void sysreg_store(void *info)
{
	...
	if (ret != 0) {
		dev_err(args->dev, "input must be u64.\n");
		return;
	}

	write_sysreg64(args->dev, args->id, input);
}
...
static ssize_t reg_name##_c##cpu_id##_show( \
	struct device *dev, struct device_attribute __maybe_unused *attr, char *buf) \
{ \
	struct node_args_t args = {dev, buf, NULL, SYSREG_ID(reg_name)}; \
\
...
static ssize_t reg_name##_c##cpu_id##_store( \
	...
	const char *buf, size_t count) \
{ \
	struct node_args_t args = {dev, NULL, buf, SYSREG_ID(reg_name)}; \
\
...
MAKE_SYSREG_RW(cpu_id, CPUMPMMCR); \
MAKE_SYSREG_RW(cpu_id, CPUMPMMTUNE);  /* maybe exist */ \
...
MAKE_SYSREG_RW(cpu_id, CPUMPMMCR); \
MAKE_SYSREG_RW(cpu_id, CPUPPMPDPCR); \
MAKE_SYSREG_RW(cpu_id, CPUMPMMTUNE); \
