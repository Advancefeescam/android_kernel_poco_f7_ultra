...
/*P6 code for HQFEAT-109133 by qingweijie at 2025/06/10 start*/
#define FTS_IRQ                                 "fts_irq"
struct proc_dir_entry *fts_proc_irq_entry = NULL;
/*P6 code for HQFEAT-109133 by qingweijie at 2024/06/10 end*/
...
/*P6 code for HQFEAT-109133 by qingweijie at 2025/06/10 start*/
static ssize_t fts_irq_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(temp_buff, "%d", &pdata)) {
        if (pdata == 1) {
            fts_irq_enable();
            FTS_INFO("/proc/tp_irq enable\n");
        } else if (pdata == 0) {
            fts_irq_disable();
            FTS_INFO("/proc/tp_irq disable\n");
        }
    }
...
static const struct proc_ops fts_irq_fops = {
    .proc_write = fts_irq_write,
};
/*P6 code for HQFEAT-109133 by qingweijie at 2025/06/10 end*/
...
/*P6 code for HQFEAT-109133 by qingweijie at 2025/06/10 start*/
    fts_proc_irq_entry = proc_create(FTS_IRQ, (S_IWUSR | S_IRUGO), NULL, &fts_irq_fops);
    if (fts_proc_irq_entry == NULL) {
        FTS_ERROR("%s:fts_proc_irq_entry create failed!\n", __func__);
        return -ENOMEM;
    } else {
        FTS_INFO("%s:proc_create fts_proc_irq_entry success\n", __func__);
    }
/*P6 code for HQFEAT-109133 by qingweijie at 2025/06/10 end*/
...
/*P6 code for HQFEAT-109133 by qingweijie at 2025/06/10 start*/
    if (fts_proc_irq_entry != NULL) {
        remove_proc_entry(FTS_IRQ, NULL);
        fts_proc_irq_entry = NULL;
