...
/* L19A code for HQ-194052 by xukai at 2022/04/25 start */
/**
get ymtc hr
 */
int scsi_ymtc_hr(struct scsi_device *sdev, char *hr, int len)
{
	...
	buff[0] = 1;
	/* L19A code for HQ-218180 by xukai at 2022/06/22 start */
	result = scsi_execute(sdev, write_buffer_1, DMA_TO_DEVICE, NULL,
				  0, NULL, &sshdr, 30 * HZ, 3, 0, RQF_PM, NULL);
	if (result) {
	...
	result = scsi_execute(sdev, write_buffer_2, DMA_TO_DEVICE, buff,
				  len_buff, NULL, &sshdr, 30 * HZ, 3, 0, RQF_PM, NULL);
	if (result) {
	...
	result = scsi_execute(sdev, read_buffer, DMA_FROM_DEVICE, buff,
				  len_buff, NULL, &sshdrr, 30 * HZ, 3, 0, RQF_PM, NULL);
	/* L19A code for HQ-218180 by xukai at 2022/06/22 end */
	if (result) {
		pr_err("ufs: hr read buffer error 0x%x\n", result);
...
/* L19A code for HQ-194052 by xukai at 2022/04/25 end */
