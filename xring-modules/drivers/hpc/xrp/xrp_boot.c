				...
				Elf32_Phdr *phdr)
{
	...
	if (!xrp_sram_sys_dma_buf) {
		xrp_print(LOG_LEVEL_ERROR, "SRAM mapping failed: NULL pointer\n");
		ret = -ENOMEM;
		goto sram_error_exit;
	}
	...
	if (!xrp_srom_sys_dma_buf) {
		xrp_print(LOG_LEVEL_ERROR, "SROM mapping failed: NULL pointer\n");
		ret = -ENOMEM;
		goto srom_error_exit;
	}
	...
	if (ret != 0) {
		...
		goto loadimage_error_exit;
	}
	return ret;

loadimage_error_exit:
	sys_nocache_unmap(xrp_task_dev, &srom_data);
srom_error_exit:
	sys_nocache_unmap(xrp_task_dev, &sram_data);
sram_error_exit:
