...
static u32 g_lockdown_info[LOCKDOWN_INFO_LEN] = {0};
...
static char *event_client_str_table[] = {
	...
	"XRING_PANEL_EVENT_CLIENT_SENSORHUB_NOTIFIER",
...
ssize_t dsi_panel_lockdown_info_read(unsigned char *plockdowninfo)
{
	...
	for (i = 0; i < LOCKDOWN_INFO_LEN; i++)
		plockdowninfo[i] = (char)g_lockdown_info[i];

	return LOCKDOWN_INFO_LEN;
}
EXPORT_SYMBOL(dsi_panel_lockdown_info_read);

void dsi_panel_lockdown_info_set(u32 *lockdown_info)
{
	int i;

	for (i = 0; i < LOCKDOWN_INFO_LEN; i++) {
		g_lockdown_info[i] = lockdown_info[i];
		DSI_INFO("lockdown_info params[%d] = %x\n", i, g_lockdown_info[i]);
	}
}
