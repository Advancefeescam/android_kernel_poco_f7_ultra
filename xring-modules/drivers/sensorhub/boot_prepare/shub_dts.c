...
static struct device_node *g_dsi_panel_node;
...
static int shub_parse_dsi_panel_dts(struct device_node *parent, struct sh_bsp_dts *dts_mem_block)
{
	int count = 0;
	...
	count = of_count_phandle_with_args(parent, property_name, NULL);

	for (int iter = 0; iter < count; iter++) {
		node = of_parse_phandle(parent, property_name, iter);
		panel = of_drm_find_panel(node);
		if (!IS_ERR(panel)) {
			pr_info("matched to find matched panel node");
			g_dsi_panel_node = node;
			break;
		}
	...
	if (!panel) {
		pr_err("Failed to find matched panel node");
		ret = -ENODEV;
	}
...
struct device_node *get_dsi_panel_node(void)
{
	return g_dsi_panel_node;
}
EXPORT_SYMBOL(get_dsi_panel_node);
...
struct sh_dts_cb dts_cb_array[] = {
	{shub_parse_i3c_dts}, {shub_parse_conn_dts}, {shub_parse_dsi_panel_dts}, {shub_parse_aoc_dts},
};
