...
static void o1_host_set_force_gen1(struct xring_usb_o1 *usb_o1)
{
	unsigned int reg;

	reg = readl(usb_o1->usbsctrl + USB_FORCE_GEN1_SPEED);
	reg &= ~USB_GEN1_SPEED_HOST_FORCE_GEN1_SPEED_MASK;
	reg |= BIT(USB_GEN1_SPEED_HOST_FORCE_GEN1_SPEED_SHIFT);
	writel(reg, usb_o1->usbsctrl + USB_FORCE_GEN1_SPEED);
}
...
static int plat_sys_init(struct xring_usb *xr_usb)
{
	...
	if (xr_usb->host_force_gen1) {
		dev_info(xr_usb->dev, "plat force gen1\n");
		o1_host_set_force_gen1(usb_o1);
	}
