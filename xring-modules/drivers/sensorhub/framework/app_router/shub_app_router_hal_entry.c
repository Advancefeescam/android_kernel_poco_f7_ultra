	...
	for (clip = 0; clip < 2; clip++) {
		for (i = 0; i < len;) {
			...
			if (packet->type == DISCARD_PACKET)
				break;
			if (packet->wakeup) {
				mutex_lock(&app_router->service->lock);
				atomic_dec(warpper->wakeup_ref);
				pr_debug("hal client wakeup cnt %d!\n", atomic_read(warpper->wakeup_ref));
				if (atomic_read(warpper->wakeup_ref) == 0)
					pr_err("hal client dec to 0!");
				mutex_unlock(&app_router->service->lock);
			}
			       ...
			       loff_t *pos)
{
	...
	if (buffer_empty(app_router->service->buffers[index]))
		return 0;
	warpper.wakeup_ref = &app_router->service->wakeup[index];
...
static int app_router_release(struct inode *inode, struct file *fp)
{
	...
	if (atomic_read(&app_router->service->wakeup[index]) > 0) {
		__pm_relax(app_router->service->event_ws[index]);
		atomic_set(&app_router->service->wakeup[index], 0);
		pr_debug("hal client %u relese awake cnt %lu relax cnt! %lu\n",
			index,
			app_router->service->event_ws[index]->active_count,
			app_router->service->event_ws[index]->relax_count);
	}
	kfree(fp->private_data);
	fp->private_data = NULL;
...
static unsigned int app_router_poll(struct file *fp, poll_table *wait)
{
	...
	if (unlikely(!private_data))
		return -EFAULT;
	mutex_lock(&app_router->service->lock);
	if (atomic_read(&app_router->service->wakeup[index]) == 1) {
		__pm_relax(app_router->service->event_ws[index]);
		atomic_dec(&app_router->service->wakeup[index]);
		pr_debug("hal client %u wakeup cnt %d relese awake cnt %lu relax cnt %lu!\n",
			index,
			atomic_read(&app_router->service->wakeup[index]),
			app_router->service->event_ws[index]->active_count,
			app_router->service->event_ws[index]->relax_count);
	}
	mutex_unlock(&app_router->service->lock);
