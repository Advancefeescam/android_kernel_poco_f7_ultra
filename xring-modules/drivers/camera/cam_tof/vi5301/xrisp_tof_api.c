				...
				uint32_t noise, uint32_t integral_times)
{
	...
	const int32_t ylower1[] = {4, 10, 17, 30, 42, 49, 54, 66, 136, 211, 279, 566, 600, 1025,
			1221, 1682, 2086, 2559, 3336, 3779, 4338, 4796, 6806};
	const int32_t yupper1[] = {7, 13, 20, 35, 48, 60, 66, 80, 162, 243, 321, 630, 666, 1138,
...
int xrisp_tof_get_measure_data(struct xrisp_tof_data *dev)
{
	...
	if (peak1 > noise / 8 * dev->ma_sum)
		peakr = (peak1 - noise / 8 * dev->ma_sum) * 100 / integral_times;
	if ((tof1_bin <= xtalk_bin + 2) && (peakr <= 195))
		confidence = 0;
