...
static void qcm_update_charge_status(struct qcm_chip *chip)
{
	...
	charger_get_adc(chip->charger, CHG_ADC_VBUS, &chip->adapter_vbus);
	charger_get_adc(chip->charger, CHG_ADC_IBUS, &chip->sw_ibus);
	chargerpump_get_adc_value(chip->master_cp_chg, CP_ADC_IBUS, &chip->master_cp_ibus);
	chargerpump_get_adc_value(chip->slave_cp_chg, CP_ADC_IBUS, &chip->slave_cp_ibus);
