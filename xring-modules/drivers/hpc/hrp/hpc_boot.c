...
#include <dt-bindings/xring/platform-specific/npu_usage_mutex.h>
...
int get_hardware_spinlock(struct hwspinlock *hwlock)
{
	int ret = 0;
	...
	if (!hwlock) {
		hpcerr("Invalid hardware spinlock\n");
		return -ENODEV;
	}
	...
	do {
		...
		if (time_after(time, timeout)) {
			...
			hpcerr("Get hwspinlock[%d] timeout\n", lock_id);
			return -EBUSY;
		}
		/* sleep 100~200 us */
...
int get_npu_status_before_powerup(struct hpc_device *hdev)
{
	unsigned long timeout;
	uint32_t npu_status;

	if (!hdev->lpis_actrl_va) {
		hpcerr("Invalid lpis_actrl_va\n");
		return -ENOMEM;
	}

	timeout = jiffies + msecs_to_jiffies(GET_NPU_STATUS_TIMEOUT);

	while (!time_after(jiffies, timeout)) {
		npu_status = readl(hdev->lpis_actrl_va);
		npu_status = (npu_status >> NPU_USAGE_STATUS_START_BIT) & NPU_USAGE_STATUS_MASK;
		hpcdbg("npu_status: %x", npu_status);
		if (npu_status == NPU_USAGE_IDLE || npu_status == NPU_USAGE_ACPU)
			return 0;
	...
	hpcerr("wait npu idle timeout, npu_status:0x%x\n", npu_status);
	return -EBUSY;
}

int get_npu_status_before_powerdown(struct hpc_device *hdev)
{
	uint32_t npu_status;
	...
	npu_status = readl(hdev->lpis_actrl_va);
	npu_status = (npu_status >> NPU_USAGE_STATUS_START_BIT) & NPU_USAGE_STATUS_MASK;
	hpcdbg("npu_status: 0x%x", npu_status);
	if (npu_status == NPU_USAGE_ACPU)
		return 0;

	hpcerr("npu status[0x%x] is not expected!!", npu_status);

	return -ENODEV;
}

int set_npu_usage_status(struct hpc_device *hdev, enum npu_usage_status st)
{
	int ret = 0;
	uint32_t npu_status;
	...
	npu_status = (NPU_USAGE_STATUS_MASK << (NPU_MCU_ACTRL_BMRW_SHIFT + NPU_USAGE_STATUS_START_BIT)) |
			(st << NPU_USAGE_STATUS_START_BIT);
	writel(npu_status, hdev->lpis_actrl_va);
	hpcinfo("write val[0x%x] to sysreg", npu_status);
...
int hdev_boot(struct hpc_device *hdev)
{
	...
	ret = get_hardware_spinlock(hdev->hwspin_lock);
	if (ret != 0) {
		hpcerr("hw spinlock is not available!!");
		atomic_dec(&hdev->power);
		goto err_unlock_mutex;
	}

	ret = get_npu_status_before_powerup(hdev);
	if (ret != 0) {
		hpcerr("npu is not available!!");
		hwspin_unlock_raw(hdev->hwspin_lock);
		atomic_dec(&hdev->power);
		goto err_unlock_mutex;
	}

	ret = set_npu_usage_status(hdev, NPU_USAGE_ACPU);
	if (ret != 0) {
		hpcerr("failed to set npu status to NPU_USAGE_ACPU!!");
		hwspin_unlock_raw(hdev->hwspin_lock);
		atomic_dec(&hdev->power);
		goto err_unlock_mutex;
	}
...
err_dec_atomic:
	(void)set_npu_usage_status(hdev, NPU_USAGE_IDLE);
	atomic_dec(&hdev->power);
...
int hdev_shutdown(struct hpc_device *hdev)
{
	...
	ret = get_npu_status_before_powerdown(hdev);
	if (ret != 0) {
		hpcerr("npu status is not in NPU_USAGE_ACPU!!");
		goto out;
	}
	...
	ret = set_npu_usage_status(hdev, NPU_USAGE_IDLE);
	if (ret != 0) {
		hpcerr("failed to set npu status to NPU_USAGE_IDLE!!");
		goto out;
	}
		...
		struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(np, "hw-spinlock", &hw_spinlock_id);
	if (ret < 0)
		return ret;
	hpcinfo("hwspin_lock request id: %u\n", hw_spinlock_id);
	hdev->hwspin_lock = hwspin_lock_request_specific(hw_spinlock_id);
	if (!hdev->hwspin_lock) {
		hpcerr("hwspin_lock request failed\n");
		return -EBUSY;
	}
	...
	if (reg == NULL)
		return -EINVAL;
	hpcdbg("reg start: 0x%llx end: 0x%llx flag: %lu\n", reg->start, reg->end, reg->flags);
	hdev->lpis_actrl_va = devm_ioremap_resource(dev, reg);
	hpcdbg("lpis_actrl_va: 0x%llx\n", (uint64_t)hdev->lpis_actrl_va);
...
static int hpc_remove(struct platform_device *pdev)
{
	...
	if (hdev->hwspin_lock)
		hwspin_lock_free(hdev->hwspin_lock);
	free_page((unsigned long)hdev);
	hpcinfo("hpc end!\n");
