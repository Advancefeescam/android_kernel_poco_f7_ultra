...
int32_t nvt_update_firmware(char *firmware_name)
{
	...
	if(0 == strcmp(firmware_name, BOOT_UPDATE_FIRMWARE_NAME)) {
		if(ts->lcd_id_value) {
			...
			if (ts->lockdown_info[6] == NVT_DP_PAND && ts->lockdown_info[7] == NVT_TP_THICK){
				NVT_LOG("Update firmware 00");
				firmware_name = BOOT_UPDATE_FIRMWARE_NAME_TM00;
			} else if (ts->lockdown_info[6] == NVT_DP_PAND && ts->lockdown_info[7] == NVT_TP_THIN) {
				NVT_LOG("Update firmware 01");
				firmware_name = BOOT_UPDATE_FIRMWARE_NAME_TM01;
			} else if (ts->lockdown_info[6] == NVT_DP_LANS && ts->lockdown_info[7] == NVT_TP_THICK) {
				NVT_LOG("Update firmware 02");
				firmware_name = BOOT_UPDATE_FIRMWARE_NAME_TM02;
			}
		}
		...
		if(ts->lcd_id_value) {
			...
			if (ts->lockdown_info[6] == NVT_DP_PAND && ts->lockdown_info[7] == NVT_TP_THICK){
				firmware_name = MP_UPDATE_FIRMWARE_NAME_TM00;
			} else if (ts->lockdown_info[6] == NVT_DP_PAND && ts->lockdown_info[7] == NVT_TP_THIN) {
				firmware_name = MP_UPDATE_FIRMWARE_NAME_TM01;
			} else if (ts->lockdown_info[6] == NVT_DP_LANS && ts->lockdown_info[7] == NVT_TP_THICK) {
				firmware_name = MP_UPDATE_FIRMWARE_NAME_TM02;
			}
		}
	}
