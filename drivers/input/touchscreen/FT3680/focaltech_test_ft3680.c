...
/*BSP.TP - 2022.10.14 - Add for focal ITO test  - Start Modify*/
/*
...
static int start_test_ft5662(void)
{
	...
	ret = enter_factory_mode();
	if (ret < 0)
		fts_ftest->test_item[FTS_ENTER_FACTORY_MODE].testresult = RESULT_NULL;
	else {
	...
	if (true == test_item->rawdata_test) {
		...
		/*BSP.TP - 2022.10.14 - Add for focal ITO test  - Start Modify*/
			fts_ftest->test_item[FTS_RAWDATA_TEST].testresult = RESULT_NULL;
		} else
	...
	if (true == test_item->scap_cb_test) {
		...
		/*BSP.TP - 2022.10.14 - Add for focal ITO test  - Start Modify*/
			fts_ftest->test_item[FTS_SCAP_CB_TEST].testresult = RESULT_NULL;
		} else
	...
	if (true == test_item->scap_rawdata_test) {
		...
		/*BSP.TP - 2022.10.14 - Add for focal ITO test  - Start Modify*/
			...
			fts_ftest->test_item[FTS_SCAP_RAWDATA_TEST].testresult = RESULT_NULL ;
		} else
	...
	if (true == test_item->short_test) {
		...
		/*BSP.TP - 2022.10.14 - Add for focal ITO test  - Start Modify*/
			...
			fts_ftest->test_item[FTS_SHORT_TEST].testresult = RESULT_NULL;
		} else
	...
	if (true == test_item->panel_differ_test) {
		...
		/*BSP.TP - 2022.10.14 - Add for focal ITO test  - Start Modify*/
			...
			fts_ftest->test_item[FTS_PANEL_DIFFER_TEST].testresult = RESULT_NULL;
		} else
	...
	if (true == test_item->noise_test) {
		...
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_ftest->test_item[FTS_NOISE_TEST].testresult = RESULT_NULL;
		} else
