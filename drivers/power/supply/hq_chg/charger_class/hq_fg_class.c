...
int fuel_gauge_set_fake_soh(struct fuel_gauge_dev *fuel_gauge, int value)
{
	if (!fuel_gauge || !fuel_gauge->ops)
		return -EINVAL;
	if (fuel_gauge->ops->set_fake_soh == NULL)
		return -EOPNOTSUPP;
	return fuel_gauge->ops->set_fake_soh(fuel_gauge, value);
}
EXPORT_SYMBOL(fuel_gauge_set_fake_soh);

int fuel_gauge_get_manufacturing_date(struct fuel_gauge_dev *fuel_gauge, char *buf)
{
	if (!fuel_gauge || !fuel_gauge->ops)
		return -EINVAL;
	if (fuel_gauge->ops->get_manufacturing_date == NULL)
		return -EOPNOTSUPP;
	return fuel_gauge->ops->get_manufacturing_date(fuel_gauge, buf);
}
EXPORT_SYMBOL(fuel_gauge_get_manufacturing_date);

int fuel_gauge_get_soh_sn(struct fuel_gauge_dev *fuel_gauge, char *buf)
{
	if (!fuel_gauge || !fuel_gauge->ops)
		return -EINVAL;
	if (fuel_gauge->ops->get_soh_sn == NULL)
		return -EOPNOTSUPP;
	return fuel_gauge->ops->get_soh_sn(fuel_gauge, buf);
}
EXPORT_SYMBOL(fuel_gauge_get_soh_sn);

int fuel_gauge_get_first_usage_date(struct fuel_gauge_dev *fuel_gauge, char *buf)
{
	if (!fuel_gauge || !fuel_gauge->ops)
		return -EINVAL;
	if (fuel_gauge->ops->get_first_usage_date == NULL)
		return -EOPNOTSUPP;
	return fuel_gauge->ops->get_first_usage_date(fuel_gauge, buf);
}
EXPORT_SYMBOL(fuel_gauge_get_first_usage_date);

int fuel_gauge_set_first_usage_date(struct fuel_gauge_dev *fuel_gauge, const char *buf, size_t count)
{
	if (!fuel_gauge || !fuel_gauge->ops)
		return -EINVAL;
	if (fuel_gauge->ops->set_first_usage_date == NULL)
		return -EOPNOTSUPP;
	return fuel_gauge->ops->set_first_usage_date(fuel_gauge, buf, count);
}
EXPORT_SYMBOL(fuel_gauge_set_first_usage_date);

int fuel_gauge_set_fg_rsoc100(struct fuel_gauge_dev *fuel_gauge)
{
	if (!fuel_gauge || !fuel_gauge->ops)
		return -EINVAL;
	if (fuel_gauge->ops->set_rsoc_report_100 == NULL)
		return -EOPNOTSUPP;
	return fuel_gauge->ops->set_rsoc_report_100(fuel_gauge);
}
EXPORT_SYMBOL(fuel_gauge_set_fg_rsoc100);
...
int fuel_gauge_get_dod_count(struct fuel_gauge_dev *fuel_gauge)
{
	if (!fuel_gauge || !fuel_gauge->ops)
		return -EINVAL;
	if (fuel_gauge->ops->get_dod_count == NULL)
		return -EOPNOTSUPP;
	return fuel_gauge->ops->get_dod_count(fuel_gauge);
}
EXPORT_SYMBOL(fuel_gauge_get_dod_count);

int fuel_gauge_set_term_voltage(struct fuel_gauge_dev *fuel_gauge, int value)
{
	if (!fuel_gauge || !fuel_gauge->ops)
		return -EINVAL;
	if (fuel_gauge->ops->set_term_voltage == NULL)
		return -EOPNOTSUPP;
	return fuel_gauge->ops->set_term_voltage(fuel_gauge, value);
}
EXPORT_SYMBOL(fuel_gauge_set_term_voltage);
...
int fuel_gauge_get_isc_status(struct fuel_gauge_dev *fuel_gauge)
{
	if (!fuel_gauge || !fuel_gauge->ops)
		return -EINVAL;
	if (fuel_gauge->ops->get_isc_status == NULL)
		return -EOPNOTSUPP;
	return fuel_gauge->ops->get_isc_status(fuel_gauge);
}
EXPORT_SYMBOL(fuel_gauge_get_isc_status);

unsigned long fuel_gauge_get_calc_rvalue(struct fuel_gauge_dev *fuel_gauge)
{
	if (!fuel_gauge || !fuel_gauge->ops)
		return -EINVAL;
	if (fuel_gauge->ops->get_calc_rvalue == NULL)
		return -EOPNOTSUPP;
	return fuel_gauge->ops->get_calc_rvalue(fuel_gauge);
}
EXPORT_SYMBOL(fuel_gauge_get_calc_rvalue);
