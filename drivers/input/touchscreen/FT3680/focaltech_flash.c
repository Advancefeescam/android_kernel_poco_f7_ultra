...
static void fts_fwupg_work(struct work_struct *work)
{
	...
	/*BSP.TP - 2022.10.14 - Add for fts hw_info - Start Modify*/
...
int fts_hw_info_add(struct fts_ts_data *ts_data)
{
	...
	if(ret < 0){
  	...
  	sprintf(tp_version_info, "[Vendor]:Huaxing [TP-IC]:FT3680 [FW]:0x%x\n", fw_version);
...
static int32_t fts_tp_info_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "[Vendor]:Huaxing [TP-IC]:FT3680 [FW]:0x%x\n", fw_version);
	return 0;
}
