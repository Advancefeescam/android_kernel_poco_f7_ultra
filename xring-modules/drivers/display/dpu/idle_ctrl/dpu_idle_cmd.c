...
#define BASIC_POLICY IDLE_ISR_OFF

static u32 g_avail_idle_policy[] = {
	/* 0x2 */
	BASIC_POLICY,
	/* 0x6 */
	BASIC_POLICY | IDLE_MIPI_ULPS,
	/* 0x382 */
	BASIC_POLICY | IDLE_P1_POWER_OFF | IDLE_P2_POWER_OFF | IDLE_P0_POWER_OFF,
	/* 0x386 */
	BASIC_POLICY | IDLE_MIPI_ULPS | IDLE_P1_POWER_OFF | IDLE_P2_POWER_OFF | IDLE_P0_POWER_OFF,
};
...
int dpu_cmd_idle_prepare(struct dpu_idle_ctx *idle_ctx)
{
	...
	if (!idle_ctx->need_restore_clk) {
		...
		dpu_hw_dvfs_disable(&idle_ctx->dpu_kms->core_perf->hw_dvfs_mgr);
...
void dpu_cmd_idle_enter(struct dpu_idle_ctx *idle_ctx)
{
	...
	res_info = &idle_ctx->res_info;
	res_info->value = 0;
	res_info->bits.idle_type = CMD_IDLE;
	res_info->bits.need_dvfs = 1;
	IDLE_DEBUG("notify hwc. info:%#x\n", res_info->value);
