...
#define BASE_BATTERY_CAPACITY 5305
#define FLIP_BATTERY_CAPACITY 5305
...
static struct mca_sysfs_attr_info strategy_fg_auth_sysfs_field_tbl[] = {
	...
	mca_sysfs_attr_ro(strategy_fg_auth_sysfs, 0440, FG_PROP_SOH, soh),
...
static void strateg_fg_ops_get_parallel_soh_new(struct strategy_fg *fg, int *soh_new)
{
	int master_ui_soh, slave_ui_soh, master_cap, slave_cap;
	int ret = 0;

	platform_fg_ops_get_ui_soh(FG_IC_MASTER, &master_ui_soh);
	platform_fg_ops_get_ui_soh(FG_IC_SLAVE, &slave_ui_soh);
	ret = platform_fg_ops_get_full_design(FG_IC_MASTER, &master_cap);
	ret |= platform_fg_ops_get_full_design(FG_IC_SLAVE, &slave_cap);
	master_cap = master_cap / 1000;
	slave_cap = slave_cap / 1000;
	if ((master_cap <= 0) || (master_cap  > 10000))
		master_cap = 0;
	if ((slave_cap <= 0) || (slave_cap  > 10000))
		slave_cap = 0;
	if ((master_cap == 0) && (slave_cap == 0)) {
		master_cap = BASE_BATTERY_CAPACITY;
		slave_cap = FLIP_BATTERY_CAPACITY;
	}
	*soh_new = (master_cap * master_ui_soh + slave_cap * slave_ui_soh) / (master_cap + slave_cap);
	mca_log_info("master_cap=%d, slave_cap=%d, master_ui_soh=%d, slave_ui_soh=%d, soh_new=%d\n",
		master_cap, slave_cap, master_ui_soh, slave_ui_soh, *soh_new);
}
...
static int strategy_fg_get_parallel_original_temp(struct strategy_fg *fg)
{
	...
	ret = platform_fg_ops_get_original_temp(FG_IC_SLAVE, &(fg->slave_batt_info.original_temp));
	ret |= platform_fg_ops_get_original_temp(FG_IC_MASTER, &(fg->master_batt_info.original_temp));
	pr_info("%s master %d slave %d\n", __func__, fg->master_batt_info.original_temp, fg->slave_batt_info.original_temp);
	if (ret)
		return -1;

	if (fg->master_batt_info.original_temp <= JEITA_COOL_THR_DEGREE ||
	    fg->slave_batt_info.original_temp <= JEITA_COOL_THR_DEGREE)
		fg->batt_info.original_temp = min(fg->master_batt_info.original_temp, fg->slave_batt_info.original_temp);
	else if (min(fg->master_batt_info.original_temp, fg->slave_batt_info.original_temp) > EXTREME_HIGH_DEGREE)
		fg->batt_info.original_temp = min(fg->master_batt_info.original_temp, fg->slave_batt_info.original_temp);
	else
		fg->batt_info.original_temp = max(fg->master_batt_info.original_temp, fg->slave_batt_info.original_temp);
...
static int strategy_fg_get_parallel_error_state(struct strategy_fg *fg)
{
	...
	fg->fg_master_error = fg_error_master;
	fg->fg_slave_error = fg_error_slave;
...
static int strategy_fg_get_parallel_batt_info(struct strategy_fg *fg)
{
	...
	for (i = 0; i <= 1; i++) {
		...
		mca_log_err("fg_index[%d], CURRENT:%d RSOC:%d VOLTAGE:%d TEMP:%d ORIGINAL_TEMP:%d\n",
			i, batt_info.curr, batt_info.rsoc, batt_info.volt, batt_info.temp, batt_info.original_temp);
		mca_log_info("fg_index[%d], CYCLECOUNT:%d S:%d RM:%d F:%d\n",
...
static int mca_strategy_check_sigle_termination(struct strategy_fg *fg)
{
	...
	if (!fg->fast_charge && fg->cfg.fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX)
		soft_vterm = vterm - 7;
...
static int mca_strategy_parallel_first_termination(struct strategy_fg *fg)
{
	...
	if (!fg->fg1_batt_ctr_enabled && !fg->fg2_batt_ctr_enabled && fg->power_present) {
...
static int get_batt_temp_index(int temp, struct term_curr_para term_curr_data)
{
    for (int i = ITERM_PARA_MAX_GROUP; i >= 0; i--) {
        if (temp >= term_curr_data.iterm_data[i].batt_temp) {
            return i;
        }
    }
    return 0;
...
static int strategy_fg_get_term_current(struct strategy_fg *fg, int fg_index)
{
	...
	static int last_temp_index[FG_IC_MAX] = {-1,-1};
	struct term_curr_para term_curr_data = fg->cfg.term_curr_data[fg_index];
	int final_iterm = 300;
	int temp_index, last_index;
	last_index = last_temp_index[fg_index];

	temp = temp / 10;
	temp_index = get_batt_temp_index(temp, term_curr_data);
	mca_log_info("fg[%d] temp_index %d, last_index %d", fg_index, temp_index, last_index);

	if (last_index != -1 && temp_index < last_index) {
		if (temp >= term_curr_data.iterm_data[last_index].batt_temp - term_curr_data.iterm_data[last_index].batt_temp_offset) {
			temp_index = last_index;
		}
	}
	last_temp_index[fg_index] = temp_index;
	if (fg->fast_charge)
		final_iterm = term_curr_data.iterm_data[temp_index].ffc_iterm;
	else
		final_iterm = term_curr_data.iterm_data[temp_index].normal_iterm;

	if (fg->real_type == XM_CHARGER_TYPE_SDP) {
		final_iterm -= fg->cfg.sdp_decrease_iterm[fg_index];
		mca_log_info("fg[%d] sdp decrease iterm %d", fg_index, fg->cfg.sdp_decrease_iterm[fg_index]);
	}
	return final_iterm;
}
...
static int strategy_fg_check_parallel_termination(struct strategy_fg *fg)
{
	...
	//int vterm_target = 0;
	int ret;
	int vterm = mca_get_effective_result(fg->vterm_voter);
	int final_term_curr = 0;
	static int last_final_term_curr;
	int last_term[2] = {250, 250};
	//bool fc_master;
	int force_curr = 0;

	if (!fg->power_present || fg->charging_done || vterm < STRATEGY_FG_DEFAULT_VTERM)
		return 0;
	...
	/*if (fg->cfg.support_base_flip) {
		ret |= platform_fg_ops_get_fc(FG_IC_MASTER, &fc_master);
		if (fg->fast_charge && fg->batt_temperature > 400 && fg->fg2_batt_ctr_enabled && !fc_master) {
			final_term_curr = min(final_term_curr, 300);
		}
	}*/

	mca_log_info("final_term_curr:%d, first_termination: %d, fast_charge: %d\n",
		final_term_curr, fg->first_termination, fg->fast_charge);
	...
	if (fg->first_termination && !fg->fast_charge) {
		force_curr = final_term_curr * 130 / 100;
		if (-fg->batt_current >= force_curr)
			mca_vote(fg->charge_limit_voter, "drop_fcc", true, force_curr);
		mca_log_info("batt_curr: %d, force_curr: %d\n", -fg->batt_current, force_curr);
	}
...
static void mca_strategy_single_force_fw_report_full(struct strategy_fg *fg)
{
	...
	if (!fg->power_present || fg->batt_rsoc < 90 || (!fg->fast_charge && !fg->support_normal_force_full)) {
		force_full_count = 0;
	...
	if (!fg->charging_done) {
		if (-fg->batt_current / 1000 < force_iterm && fg->batt_voltage > force_vterm && !fg->start_force_full) {
			...
			if (force_full_count >= 6) {
				...
				schedule_delayed_work(&fg->master_force_report_full_work, msecs_to_jiffies(8000));
			}
		} else {
			force_full_count = 0;
		}
...
static void mca_strategy_parallel_force_fw_report_full(struct strategy_fg *fg, int fg_index)
{
	...
	if (!fg->power_present || batt_info.rsoc < 90 || (!fg->fast_charge && !fg->support_normal_force_full)) {
	...
	if (fg->fg1_batt_ctr_enabled || fg->fg2_batt_ctr_enabled) {
		...
		if (fg->fast_charge)
			force_iterm = iterm * 120 / 100;
		else
			force_iterm = iterm * 130 / 100;
	} else {
		force_vterm = vterm - 25;
		if (fg->fast_charge)
			force_iterm = iterm * 115 / 100;
		else
			force_iterm = iterm * 135 / 100;
	}
	...
	if ((batt_info.temp / 10 < 47) && !fg->charging_done) {
		if (-batt_info.curr / 1000 < force_iterm && batt_info.volt > force_vterm && !fg->dual_force_full[fg_index]) {
			...
			if (force_full_count[fg_index] >= count) {
				...
				if (fg_index == FG_IC_MASTER)
					schedule_delayed_work(&fg->master_force_report_full_work, msecs_to_jiffies(8000));
				else if (fg_index == FG_IC_SLAVE)
					schedule_delayed_work(&fg->slave_force_report_full_work, msecs_to_jiffies(8000));
...
static int mca_strategy_update_fg_info(struct strategy_fg *fg)
{
	...
	fg->batt_original_temperature = fg->batt_info.original_temp;
	...
	if (!checked) {
		...
		if (time_now > DFS_CHECK_BATT_AUTH_TIME_SEC) {
			...
			if (fg->cfg.fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX) {
				if (!fg->batt_auth) {
					role = 0;
					mca_charge_mievent_report(CHARGE_DFX_DUAL_BATTERY_AUTH_FAIL, &role, 1);
				}
...
static int fg_update_status(struct strategy_fg *fg, bool prohibit_jump)
{
	...
	static bool last_fg_error = false;
	...
	if (fg->cfg.fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX) {
		if (fg->fg_error != last_fg_error) {
			...
			mca_strategy_func_process(STRATEGY_FUNC_TYPE_BMD, MCA_EVENT_BMD_STSTUS_CHANGE, last_fg_error);
		}
	}
...
static void strategy_fg_monitor_workfunc(struct work_struct *work)
{
	...
	if (!fg->fg_init_flag) {
		...
		fg->fg_master_init_flag = master_ok;
		if (fg->cfg.fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX) {
			flag |= platform_fg_ops_probe_ok(FG_IC_SLAVE, &slave_ok);
			fg->fg_slave_init_flag = slave_ok;
		}
		if (flag) {
			mca_queue_delayed_work(&fg->monitor_work, msecs_to_jiffies(200));
			return;
		}
		prohibit_jump = false;
...
static void strategy_master_force_report_full_work(struct work_struct *work)
{
	struct strategy_fg *fg = container_of(work, struct strategy_fg, master_force_report_full_work.work);
	static int count = 0;

	if (!fg->power_present || fg->charging_done || (!fg->fast_charge && !fg->support_normal_force_full)) {
		count = 0;
		return;
	}

	if (fg->cfg.fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX) {
		if (fg->master_batt_info.rsoc != 100) {
			platform_fg_ops_set_force_report_full(FG_IC_MASTER);
			mca_log_info("fg_master not report full, send force_report_full cmd again...");
		} else {
			count = 0;
			mca_log_info("fg_master check report full ok");
			return;
		}
	} else if (fg->batt_rsoc != 100) {
		platform_fg_ops_set_force_report_full(FG_IC_MASTER);
...
static void strategy_slave_force_report_full_work(struct work_struct *work)
{
	struct strategy_fg *fg = container_of(work, struct strategy_fg, slave_force_report_full_work.work);
	static int count = 0;
	...
	if (fg->cfg.fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX) {
		if (fg->slave_batt_info.rsoc != 100) {
			platform_fg_ops_set_force_report_full(FG_IC_SLAVE);
			mca_log_info("fg_slave not report full, send force_report_full cmd again...");
		} else {
			count = 0;
			mca_log_info("fg_slave check report full ok");
			return;
		}
	}
...
static int strategy_fg_ops_get_thermal_temp(void *data, int *temp)
{
	...
	if (!fg || !fg->fg_init_flag)
		*temp = STRATEGY_FG_ERROR_FAKE_TEMP;
		...
		if (fg->cfg.support_pnc)
			*temp = fg->batt_original_temperature;
		else
			*temp = fg->batt_temperature;
	}
...
static int strategy_fg_parse_dt(struct strategy_fg *fg)
{
	...
	if (fg->cfg.fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX) {
		...
		ret = mca_parse_dts_u32_array(node, "sdp_decrease_iterm", fg->cfg.sdp_decrease_iterm, FG_IC_MAX);
		if (ret < 0) {
			mca_log_err("parse sdp decrease iterm failed %d\n", ret);
			return 0;
		}
	}
	fg->cfg.support_pnc = of_property_read_bool(node, "support-pnc");
	fg->support_normal_force_full= of_property_read_bool(node, "support_normal_force_full");
...
static int strategy_fg_ops_set_charging_done(void *data, bool charging_done)
{
	...
	if (charging_done) {
		...
		mca_vote(fg->charge_limit_voter, "drop_fcc", false, 0);
		...
		if (fg->cfg.fg_type <= MCA_FG_TYPE_SINGLE_NUM_MAX) {
			...
			if (fg->cfg.support_base_flip) {
				if (adapt_power == 55 && adapt_power_slave == 15)
					adapt_power = 67;
				else if (adapt_power == 60 && adapt_power_slave == 60)
					adapt_power = 120;
			} else {
...
static int strategy_fg_ops_get_auth_event(void *data, bool *auth_event_flag)
{
	struct strategy_fg *fg = (struct strategy_fg *)data;
	...
	*auth_event_flag = (fg->fg_batt_master_auth || fg->fg_batt_slave_auth);
...
int strategy_fg_dual_ops_is_chip_ok(void *data, int index)
{
	...
	if (index == FG_IC_MASTER)
		return fg->fg_master_init_flag && !fg->fg_master_error;
	else if (index == FG_IC_SLAVE)
		return fg->fg_slave_init_flag && !fg->fg_slave_error;
...
int strategy_fg_ops_get_batt_ctr_sts(void *data, int role, bool *batt_ctr_sts)
{
	struct strategy_fg *fg = (struct strategy_fg *)data;
	...
	if (role == FG_IC_MASTER)
		*batt_ctr_sts = fg->fg1_batt_ctr_enabled;
	else if (role == FG_IC_SLAVE)
		*batt_ctr_sts = fg->fg2_batt_ctr_enabled;

	return 0;
}

int strategy_fg_ops_get_final_iterm(void *data, int *iterm)
{
	struct strategy_fg *fg = (struct strategy_fg *)data;
...
static struct strategy_fg_class_ops g_strategy_fg_ops = {
	...
	.strategy_fg_get_auth_event = strategy_fg_ops_get_auth_event,
	...
	.strategy_fg_get_batt_ctr_sts = strategy_fg_ops_get_batt_ctr_sts,
	.strategy_fg_get_final_iterm = strategy_fg_ops_get_final_iterm,
};
...
static int strategy_fg_process_event(int event, int value, void *data)
{
	...
	case MCA_EVENT_WIRELESS_DISCONNECT:
		...
		if (info->cfg.fg_type > MCA_FG_TYPE_SINGLE_NUM_MAX) {
			...
			mca_vote(info->charge_limit_voter, "drop_fcc", false, 0);
			strategy_fg_reset_co_to_default(info);
...
static int strategy_fg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&fg->master_force_report_full_work, strategy_master_force_report_full_work);
	INIT_DELAYED_WORK(&fg->slave_force_report_full_work, strategy_slave_force_report_full_work);
