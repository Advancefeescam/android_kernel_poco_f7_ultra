			...
			char *buf, size_t size)
{
	...
	panel = dsi_display->panel;
	config = &(dsi_display->panel->mi_panel.wp_config);

	if (strlen(config->wp_info) < 32 && config->wp_info) {
		strcpy(oled_wp_info_str, config->wp_info);
		oled_wp_info_str[strlen(config->wp_info)] = '\0';
	}
	...
	if (!strlen(wp_info_str) || strstr(wp_info_str, "00000000") != NULL) {
		...
		if (mi_get_panel_id_by_dsi_panel(panel) == O81A_PANEL_PA) {
			DISP_INFO("failed to get wp_info from uefi, default wp_info is %s\n", wp_info_str);
		} else {
			...
			char *buf, size_t size)
{
	...
	config = &(dsi_display->panel->mi_panel.cell_id_config);
	panel = dsi_display->panel;

	if (strlen(config->cell_id) < 32 && config->cell_id) {
		strcpy(cell_id_info_buffer, config->cell_id);
		cell_id_info_buffer[strlen(config->cell_id)] = '\0';
		DISP_INFO("cell_id form uefi is: %s\n", config->cell_id);
	}

	if (!strlen(cell_id_info_str) &&
			(!strlen(cell_id_info_buffer) || strstr(cell_id_info_buffer, "00000000") != NULL)) {
		if (mi_get_panel_id_by_dsi_panel(panel) == O81A_PANEL_PA) {
			DISP_INFO("failed to get cell_id from uefi, default cell_ id is %s\n", cell_id_info_buffer);
		} else {
			mi_dsi_acquire_wakelock(dsi_display->panel);
