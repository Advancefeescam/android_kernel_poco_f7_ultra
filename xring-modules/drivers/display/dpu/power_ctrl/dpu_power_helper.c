		...
		struct drm_crtc_state *new_state)
{
	...
	for_each_connector_per_crtc(connector, new_state, mask) {
		if (connector->connector_type == DRM_MODE_CONNECTOR_DSI) {
			...
			display = to_dsi_connector(connector)->display;
		...
		struct drm_crtc_state *new_state)
{
	...
	for_each_connector_per_crtc(connector, new_state, mask) {
		...
		if (is_cont_display_enabled()) {
			dsi_display->is_panel_power_up = false;
			continue;
		}

		if (power_state->dsi_panel_need_power_on) {
			dsi_display->is_panel_power_up = true;
			continue;
		} else if (dsi_display->is_panel_power_up) {
			/* trigger backlight power on after power on frame */
			dsi_display_backlight_power_on(dsi_display);
			dsi_display->is_panel_power_up = false;
		}
	}
		...
		struct drm_crtc_state *new_state)
{
	...
	dpu_power_finish_for_connector(dpu_kms->power_mgr, power_state, old_state, new_state);
