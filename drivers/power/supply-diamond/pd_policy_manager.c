...
static bool usbpd_get_pps_status(struct usbpd_pm *pdpm)
{
    ...
    while (1) {
		...
		if(pdpm->apdo_max_volt == 11000 && pdpm->apdo_max_curr > apdo_cap.ma) {
...
//		msleep(300);
		usbpd_set_iio_channel(pdpm, MAIN, MAIN_CHAGER_HZ, false);
	}
...
#define CHG_CUR_VOLT_NORMAL     4450
#define CHG_CUR_VOLT1       	4200
#define CHG_CUR_VOLT1_1       	4125
#define CHG_CUR_VOLT2           4300
#define CHG_CUR_VOLT3           4400
#define CHG_CUR_VOLT4           4480
#define CHG_VOLT1_MAX_CUR       12200
#define CHG_VOLT2_MAX_CUR       8820
#define CHG_VOLT3_MAX_CUR       7350
#define CHG_VOLT4_MAX_CUR       5880
#define CHG_VOLT5_MAX_CUR       3430
...
//	int step_temp = 0;
	...
	if (bat_temp >= CHG_BAT_TEMP_MIN && bat_temp < CHG_BAT_TEMP_MAX) {
		...
		else if (therm_curr <= 2000 && pdpm->cp_sec.charge_enabled) {
			pdpm->cp_sec_stopped = true;
			pdpm->cp_sec.charge_enabled = false;
			usbpd_pm_enable_cp_sec(pdpm, false);
		} else if (therm_curr >= 2100 && !pdpm->cp_sec.charge_enabled) {
			pdpm->cp_sec_stopped = false;
			pdpm->cp_sec.charge_enabled = true;
...
//      if(val.intval >= 79 || pps_change_to_stop() || (therm_curr < 2000))
        if(val.intval >= 79 || (therm_curr < 2000))
            pdpm->pps_leave = true;
        else
...
*/
   ...
   //     if(pdpm->state == PD_PM_STATE_FC2_TUNE)
    ...
    case PD_PM_STATE_ENTRY:
        ...
        if (pdpm->cp.vbat_volt < pm_config.min_vbat_for_cp) {
            ...
            pr_notice("batt_volt-%d is too high for cp,\
                    ...
                    pdpm->cp.vbat_volt);
            usbpd_pm_move_state(pdpm, PD_PM_STATE_FC2_EXIT);
        } else if (therm_curr <=1500) {
            pr_notice("therm_curr-%d is too low for cp", therm_curr);
        } else {
	...
	pr_err("PD_PM_STATE_FC2_ENTRY3 pdpm->cp.charge_enabled %d, pm_config.cp_sec_enable %d, pdpm->cp.charge_enabled %d, pdpm->cp_sec.charge_enabled %d\n",
        ...
        if (pdpm->cp.charge_enabled) {
            if ((pm_config.cp_sec_enable && (pdpm->cp_sec.charge_enabled || therm_curr <= 2000))
    ...
    case PD_PM_STATE_FC2_TUNE:
        ...
        if (ret == PM_ALGO_RET_THERM_FAULT) {
         ...
         //   if (pdpm->request_voltage_old != pdpm->request_voltage)
         	if(!pdpm->apdo_max_curr) {
	            pr_notice("PD abnormal stop charging:%d\n", pdpm->sw.charge_enabled);
	            stop_sw = true;
	            usbpd_pm_move_state(pdpm, PD_PM_STATE_FC2_EXIT);
			} else {
...
static void usbpd_pm_workfunc(struct work_struct *work)
{
	...
	if ((abs(pdpm->cp.set_ibat_cur - pdpm->cp.ibat_curr) > 2000) && (pdpm->cp.vbat_volt < pm_config.bat_volt_lp_lmt - 50))
		time = 300;
	else if ((abs(pdpm->cp.set_ibat_cur - pdpm->cp.ibat_curr) <= 2000)
		&& (abs(pdpm->cp.set_ibat_cur - pdpm->cp.ibat_curr) > 1500)
